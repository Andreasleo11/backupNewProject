...
Route::get("/push-overtime-detail/{detailId}", [
    FormOvertimeController::class,
    "pushSingleDetailToJPayroll",
]);
Route::post("/overtime/push-all/{headerId}", [
    FormOvertimeController::class,
    "pushAllDetailsToJPayroll",
]);
Route::get("/user-list", [UserRoleController::class, "User"]);

Route::get("/depthead/report/{employee_id}", [
    EmployeeDailyReportController::class,
    "showDepthead",
])->name("reports.depthead.show");

Route::get("/upload-daily-report", [EmployeeDailyReportController::class, "showUploadForm"])->name(
    "daily-report.form",
);
Route::post("/upload-daily-report", [EmployeeDailyReportController::class, "upload"])->name(
    "daily-report.upload",
);
Route::get("/employee-daily-reports", [EmployeeDailyReportController::class, "index"]);
Route::post("/daily-report/confirm-upload", [
    EmployeeDailyReportController::class,
    "confirmUpload",
])->name("daily-report.confirm-upload");

Route::get("/login-daily-employee", [EmployeeDailyReportController::class, "showLoginForm"])->name(
    "employee-login",
);
Route::post("/login-de", [EmployeeDailyReportController::class, "login"])->name("employee.login");
Route::get("/dashboard-daily-report", [
    EmployeeDailyReportController::class,
    "dashboardDailyReport",
])->name("daily-report.user");
Route::post("/logout-daily-employee", [EmployeeDailyReportController::class, "logout"])->name(
    "employee.logout",
);
...
Route::get("/change-password", [PasswordChangeController::class, "showChangePasswordForm"])->name(
    "change.password.show",
);
Route::post("/change-password", [PasswordChangeController::class, "changePassword"])->name(
    "change.password",
);
...
Route::middleware(["checkUserRole:1,2", "checkSessionId"])->group(function () {
    ...
    Route::get("/notification", function () {
        ...
        $detail = [
            ...
            "actionURL" =>
                env("APP_URL", "http://116.254.114.93:2420/") .
                "monthlyBudgetReport/" .
                $report->id,
        ];
        ...
        return (new App\Notifications\MonthlyBudgetReportRequestSign($report, $detail))->toMail(
            $report->user,
        );
    });
    ...
    Route::get("/ot/notification", function () {
        ...
        return (new App\Notifications\FormOvertimeNotification($report, $details))->toMail(
            $report->user,
        );
    ...
    Route::get("/training-notification", function () {
        ...
        return (new \App\Notifications\TrainingReminderNotification($training))->toMail(
            auth()->user(),
        );
    });
...
Route::middleware(["auth", "is.head.or.management"])->group(function () {
    ...
    Route::get("employees", [EmployeeDashboardController::class, "getEmployeesData"])->name(
        "employee-dashboard.getEmployeesData",
    );
    Route::get("/get-weekly-evaluation-data/{year}/{week}", [
        EmployeeDashboardController::class,
        "getWeeklyEvaluationData",
    ])->name("getWeeklyEvaluationData");
    Route::get("/get-employees-by-category-week/{department}/{category}/{year}/{week}", [
        EmployeeDashboardController::class,
        "getEmployeesByCategoryAndWeek",
    ])->name("getEmployeesByCategoryAndWeek");
});
