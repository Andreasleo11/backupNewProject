...
<div class="mb-4 border-b border-gray-100 pb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex items-center gap-2">
        ...
        @if ($filtersApplied && $currentColumn && $currentAction && $currentValue)
            <div
                class="hidden sm:inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700">
                Active filter:
    ...
    <form id="filterForm" method="GET" action="{{ $filterRoute }}" class="mb-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select id="columnSelect" name="filterColumn"
                    ...
                    @foreach ($filterColumns as $value => $label)
                        <option value="{{ $value }}" {{ $currentColumn == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="actionSelect" name="filterAction"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                               px-3 py-2
                               focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <option value="startswith" {{ $currentAction == 'startswith' ? 'selected' : '' }}>Starts With
                    </option>
            ...
            <div>
                ...
                <input type="text" id="filterValue" name="filterValue" value="{{ $currentValue }}"
                    placeholder="Value"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                              px-3 py-2
                              focus:border-indigo-500 focus:ring-indigo-500">
