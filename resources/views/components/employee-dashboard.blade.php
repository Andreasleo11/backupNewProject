  ...
  <div class="row">
    <div class="container-fluid">
      <div class="row">
        ...
        <text class="text-secondary">Last updated at
          <strong>{{ $latestUpdatedAt ?? 'No Data' }}</strong></text>
        <div class="col">
                        ...
                        <div class="d-flex mb-3 justify-content-between align-items-center">
                          ...
                          <div>
                            <button type="submit" class="btn btn-outline-primary"
                              data-bs-target="#allEmployeesModal" data-bs-toggle="modal">Employee
                              List ></button>
                          </div>
                        </div>
                              ...
                              <tr>
                                ...
                                @php
                                  ...
                                  $uniqueStatuses = collect($departmentEmployeeCounts)
                                      ->flatMap(
                                          fn($dept) => array_keys($dept['breakdown']->toArray()),
                                      )
                                      ->unique();

                                  // Initialize totals array
                                  $statusTotals = array_fill_keys($uniqueStatuses->toArray(), 0);
                                  $grandTotal = 0;
                                @endphp
                            ...
                            <tbody>
                              @foreach ($departmentEmployeeCounts as $department => $counts)
                                <tr>
                                  ...
                                  @foreach ($uniqueStatuses as $status)
                                    @php
                                      $countValue = $counts['breakdown']->get($status, 0);
            ...
            <div class="col-md-8">
              <div class="mb-2">
                <div class="d-flex flex-wrap">
                  ...
                  <span class="badge bg-secondary-subtle text-secondary p-2 me-2 my-1">Legend:
                    <span id="currentLegendFilter">Status</span></span>
                  <span class="badge bg-secondary-subtle text-secondary p-2 me-2 my-1">Branch:
                    <span id="currentBranchFilter">All</span></span>
                  <span class="badge bg-secondary-subtle text-secondary p-2 me-2 my-1">Dept: <span
                      id="currentDeptFilter">All</span></span>
                  <span class="badge bg-secondary-subtle text-secondary p-2 me-2 my-1">Status:
                    <span id="currentStatusFilter">All</span></span>
                  <span class="badge bg-secondary-subtle text-secondary p-2 me-2 my-1">Gender:
                    <span id="currentGenderFilter">All</span></span>
                </div>
              </div>
              <!-- Pie Chart -->
              <div>
...
<script type="module">
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchWeeklyEvaluation(weekValue) {
      ...
      let url =
        `{{ route('getWeeklyEvaluationData', ['year' => '__YEAR__', 'week' => '__WEEK__']) }}`;
...
<script type="module">
  ...
  const legendFilter = document.getElementById(
    'legendFilter'); // Optional if legend selection is added
  const ctx = document.getElementById('pieChart').getContext('2d');
  const employeeTableBody = document.getElementById('employeewithevaluation-table').querySelector(
    'tbody');
  ...
  function getFilteredData() {
    ...
    if (ratio > 50) {
      riskText.textContent =
        `Ratio karyawan Kontrak dengan status karyawan lainnya: ${ratio.toFixed(2)}%`;
  ...
  function updateChart() {
    ...
    const dominantCategory = dominantCount === -Infinity ? 'undefined' : labels[data.indexOf(
      dominantCount)];
  ...
  function updateDropdowns() {
    ...
    if (selectedLegend !== "Branch") {
      ...
      branchFilter.value = selectedBranch && branchOptions.includes(selectedBranch) ?
        selectedBranch : "";
    }
    ...
    if (selectedLegend !== "Dept") {
      ...
      deptFilter.value = selectedDept && deptOptions.some(dept => JSON.parse(dept).dept_no ===
          selectedDept) ?
        selectedDept :
        "";
    }
    ...
    if (selectedLegend !== "Status") {
      ...
      statusFilter.value = selectedStatus && statusOptions.includes(selectedStatus) ?
        selectedStatus : "";
    }
