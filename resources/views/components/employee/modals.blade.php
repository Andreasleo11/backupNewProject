...
@php
    ...
    $phases       = $modalData['phases'] ?? ['employees'];
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                                @foreach ($phases as $ph)
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest {{ $phaseColors[$ph] ?? 'bg-slate-100 text-slate-500' }}">
                                        {{ $phaseLabels[$ph] ?? $ph }}
                                    </span>
                                @endforeach
                            </div>
                        ...
                        <div class="flex items-center gap-3 p-3 bg-white rounded-2xl border border-blue-100 shadow-sm">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-black text-slate-900 mt-0.5">
                                    <span class="text-emerald-600">+{{ $emp['new'] ?? 0 }}</span>
                                    <span class="text-slate-400 mx-1">·</span>
                                    <span class="text-blue-600">~{{ $emp['updated'] ?? 0 }}</span>
                                    <span class="text-slate-400 mx-1">·</span>
                            ...
                            <div class="p-0 max-h-[50vh] overflow-y-auto custom-scrollbar border-t border-slate-200">
                                <table class="w-full text-left border-collapse border-separate border-spacing-0">
                                    <thead class="sticky top-0 z-20 bg-slate-50 shadow-sm">
                                        <tr>
                                            <th class="px-8 py-4 text-[11px] font-bold text-slate-600 uppercase tracking-wider border-b border-slate-200 sticky left-0 bg-slate-50 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                    ...
                                    <tbody class="divide-y divide-slate-200">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="3" class="px-8 py-20 text-center text-sm text-slate-400">No matching records</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                ...
                <div class="px-8 py-6 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                    <div class="text-xs font-bold text-slate-500 italic">
                        {{ $isHistorical
                            ...
                            : 'Review the phase summary above before committing.' }}
                    </div>
                    <div class="flex items-center gap-4">
                        <button wire:click="{{ $isHistorical ? 'closeLog' : 'cancelSync' }}"
