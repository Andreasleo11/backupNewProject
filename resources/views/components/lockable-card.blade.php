...
<div
    {{ $attributes->class(['card', 'position-relative', 'shadow', $isSaved ? 'border-success' : '', $locked ? 'opacity-50' : '']) }}>
    {{-- Body: clickable when unlocked, blocked when locked --}}
    <div class="card-body position-relative {{ $locked ? 'pe-none' : '' }} z-1">
        @isset($title)
            <h5 class="text-primary fw-bold mt-1 pb-2 border-bottom {{ $isSaved ? 'text-success border-success' : '' }}">
                {!! $title !!}</h5>
        @endisset
        {{ $slot }}
    </div>
