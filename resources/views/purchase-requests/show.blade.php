...
@section('content')
    ...
    <div>
        ...
        <div class="mb-8 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div class="space-y-2">
                ...
                <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-500">
                    <div class="flex items-center gap-2">
                        <div
                            class="flex h-6 w-6 items-center justify-center rounded-full bg-indigo-100 text-[10px] font-bold text-indigo-700">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <!-- {{-- If it's pure draft, show delete --}}
                    @if (
                        ($purchaseRequest->status === 'draft' || $purchaseRequest->status === 0) &&
                            auth()->id() === $purchaseRequest->created_by)
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div class="space-y-8 lg:col-span-2">
                ...
                <div class="glass-card overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                        <div class="p-6 space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <p class="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Date Required
                                    </p>
                                    <p class="mt-1 text-sm font-bold text-slate-800">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="mt-1">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-bold {{ $purchaseRequest->branch === 'JAKARTA' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800' }}">
                                    ...
                                    <div class="mt-1">
                                        @if ($flags['isImportType'])
                                            @if ($flags['showImportToggle'] && $purchaseRequest->is_import)
                                                <div
                                                    class="flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-0.5 text-[10px] font-bold text-amber-600 uppercase border border-amber-200">
                                                    <i class="bi bi-globe-americas"></i> Import
                                                </div>
                                            @elseif($purchaseRequest->is_import === false || $purchaseRequest->is_import === 0)
                                                <span
                                                    class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-100 px-2.5 py-1.5 text-xs font-bold text-emerald-800">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm font-semibold text-slate-800 truncate"
                                        title="{{ $purchaseRequest->supplier }}">
                            ...
                            @if ($purchaseRequest->po_number)
                                <div class="pt-4 border-t border-slate-100">
                                    <p class="text-[10px] uppercase tracking-wider text-slate-400 font-bold mb-1">PO Number
                                    </p>
                                    <div
                                        class="inline-flex items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-sm font-bold text-indigo-700 border border-indigo-100">
                    ...
                    @if ($purchaseRequest->remark)
                        <div class="border-t border-slate-100 bg-slate-50/30 p-6">
                            ...
                            <div
                                class="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600 italic shadow-sm">
                ...
                <div class="glass-card overflow-hidden">
                    <div
                        class="border-b border-slate-100 bg-slate-50/50 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
                        <h3 class="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-800">
                    ...
                    <div class="overflow-x-auto custom-scrollbar-thin pb-4">
                        <table class="w-full text-sm text-left min-w-[800px] lg:min-w-max">
                            <thead>
                                <tr
                                    class="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500 font-bold">
                                ...
                                @forelse($filteredItemDetail as $detail)
                                    ...
                                    <tr class="{{ $rowClass }}">
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <span
                                                class="inline-flex items-center justify-center rounded-lg bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">
                                                {{ $detail->quantity }} <span class="mx-1 text-slate-300">|</span>
                                                {{ $detail->uom }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-4 py-4 text-center align-middle">
                                            <div class="flex flex-col items-center gap-2">
                                                ...
                                                <div
                                                    class="flex items-center gap-1 text-[10px] uppercase font-bold tracking-wider text-slate-400">
                                                    @php
                                                        $steps = $approval?->steps
                                                            ->sortBy('sequence')
                                                            ->filter(fn($s) => !is_null($s->item_approver_type));
                                                    @endphp
                                                        ...
                                                        @foreach ($steps as $step)
                                                            ...
                                                            @if ($column)
                                                                <div class="flex flex-col items-center gap-0.5"
                                                                    title="{{ $step->approver_label }}">
                                                                    <span class="text-[9px]">{{ $label }}</span>
                                                                    @if ($detail->$column === 1)
                                                                        <i
                                                                            class="bi bi-check-circle-fill text-emerald-500 text-base"></i>
                                                                    @elseif($detail->$column === 0)
                                                                        <i
                                                                            class="bi bi-x-circle-fill text-rose-500 text-base"></i>
                                                                    @else
                                                                        <i
                                                                            class="bi bi-circle text-slate-300 text-base"></i>
                                                                    @endif
                                                                </div>
                                                ...
                                                @can('approve', $detail)
                                                    <div class="flex items-center gap-2 mt-1">
                                                        <form method="POST"
                                                            action="{{ route('purchase-requests.items.approve', $detail) }}">
                                                            @csrf
                                                            <button type="submit"
                                                                onclick="return confirm('Approve {{ $detail->item_name }}?')"
                                                                class="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
                                                                title="Approve">
                                                                <i class="bi bi-check-lg text-sm"></i>
                                                            </button>
                                                        </form>
                                                        <form method="POST"
                                                            action="{{ route('purchase-requests.items.reject', $detail) }}">
                                                            @csrf
                                                            <button type="submit"
                                                                onclick="return confirm('Reject {{ $detail->item_name }}?')"
                                                                class="flex h-7 w-7 items-center justify-center rounded-full bg-rose-100 text-rose-600 hover:bg-rose-600 hover:text-white transition-all shadow-sm"
                                                                title="Reject">
                                    ...
                                    <tr>
                                        <td colspan="10" class="py-8 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="mt-2 text-sm font-medium text-slate-500">No items found in this
                                                    request.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right text-base font-black text-slate-800">
                                        ...
                                        @if ($isThereAnyCurrencyDifference)
                                            <span class="block text-[10px] font-normal text-amber-600">*Mixed
                                                Currencies</span>
                                        @else
                ...
                @if ($canApprove)
                    @php
                        ...
                        if ($currentStep) {
                            ...
                            if ($approverType) {
                                $itemStats = app(
                                    \App\Domain\PurchaseRequest\Services\PurchaseRequestItemValidationService::class,
                    ...
                    <div
                        class="glass-card border-l-4 {{ isset($itemStats['pending']) && $itemStats['pending'] > 0 ? 'border-l-amber-500' : 'border-l-emerald-500' }} p-6 shadow-lg">
                        ...
                        @if ($itemStats)
                            @php
                                ...
                                $cardColor = $hasPending
                                    ? 'bg-amber-50 text-amber-800'
                                    : ($allRejected
                                        ? 'bg-rose-50 text-rose-800'
                                        : 'bg-emerald-50 text-emerald-800');
                                $icon = $hasPending
                                    ? 'bi-exclamation-triangle-fill text-amber-500'
                                    : ($allRejected
                                        ? 'bi-x-circle-fill text-rose-500'
                            ...
                            <div class="mb-6 rounded-xl {{ $cardColor }} p-4">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs font-bold uppercase tracking-wide opacity-80">Item Review Status
                                        </p>
                                        ...
                                        <div class="mt-2 flex h-1.5 w-full overflow-hidden rounded-full bg-black/10">
                                            @php
                                                $reviewedPercent =
                                                    $itemStats['total'] > 0
                                                        ? (($itemStats['total'] - $itemStats['pending']) /
                                                                $itemStats['total']) *
                                                            100
                                                        : 0;
                                                $barColor = $hasPending
    ...
    @can('approval.view-log')
        @push('modals')
            <div x-data="{ open: false }" @open-audit-drawer.window="open = true" x-init="$watch('open', value => {
                if (value) {
                ...
                style="display: none;" class="relative z-[100]" aria-labelledby="slide-over-title" role="dialog"
                aria-modal="true">
                                ...
                                <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-xl"
                                    ...
                                    <div class="bg-indigo-700 px-4 py-6 sm:px-6">
                                        <div class="flex items-center justify-between">
                                            <h2 class="text-base font-semibold leading-6 text-white" id="slide-over-title">Audit
                                                Log History</h2>
                                                            ...
                                                            <div class="flex flex-col min-w-0 flex-1 justify-between gap-2">
                                                                <div>
                                                                    <p class="text-xs text-slate-500">
                                                                        <span
                                                                            class="font-medium text-slate-900">{{ $activity->causer->name ?? 'System' }}</span>
                                                                        {{ $activity->description }}
                                                                    </p>
                                                                    @if ($activity->properties->has('attributes'))
                                                                        <div
                                                                            class="mt-2 rounded-lg bg-slate-50 p-2 text-[10px] text-slate-600 font-mono overflow-auto border border-slate-100">
                                                                            @foreach ($activity->properties['attributes'] as $key => $val)
                                                                                @if ($key !== 'updated_at')
                                                                                    <div class="flex gap-2">
                                                                                        <span
                                                                                            class="font-bold text-slate-700">{{ $key }}:</span>
                                                                                        <span
                                                                                            class="truncate">{{ is_array($val) ? json_encode($val) : $val }}</span>
                                                                                    </div>
                                                                                @endif
                                                                            @endforeach
                                                                        </div>
                                                                ...
                                                                <div
                                                                    class="whitespace-nowrap text-right text-[10px] text-slate-400">
                                                                    <time
                                                                        datetime="{{ $activity->created_at }}">{{ $activity->created_at->format('M d, H:i') }}</time>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
