    ...
    <div class="relative px-2 py-2">
        ...
        <div class="space-y-0">
            @foreach ($steps as $step)
                @php
                    ...
                    $dotColor = match (true) {
                        ...
                        $step->status == 'CANCELED'
                            => 'bg-rose-100 ring-rose-50 border-2 border-rose-500', // Canceled step
                        $isCurrent && $approval->status == 'CANCELED'
                            => 'bg-rose-500 ring-rose-100', // Current but canceled
                        $isCurrent => 'bg-white border-2 border-indigo-600 ring-indigo-50',
                        $approval->status == 'REJECTED' && $isCurrent
                            => 'bg-rose-500 ring-rose-100', // If rejected at this step
                        default => 'bg-slate-200 ring-slate-50',
                    };

                    $icon = match (true) {
                        $isCompleted => '<i class="bi bi-check text-white text-xs"></i>',
                        $isCurrent && $approval->status == 'RETURNED'
                            => '<i class="bi bi-arrow-return-left text-white text-xs"></i>',
                        $step->status == 'CANCELED' || ($isCurrent && $approval->status == 'CANCELED')
                            => '<i class="bi bi-x text-rose-600 text-xs"></i>',
                ...
                <div class="relative flex gap-4 pb-8 {{ $isLast ? 'pb-0' : '' }}">
                    ...
                    {{-- Dot/Icon --}}
                    <div
                        class="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full ring-4 {{ $dotColor }} transition-all">
                        {!! $icon !!}
                    </div>
                    ...
                    <div class="pt-0.5 w-full">
                        <div class="flex flex-col">
                            ...
                            @if ($step->remarks)
                                <div
                                    class="mt-2 rounded-lg bg-slate-50 p-2 text-[10px] italic text-slate-600 border border-slate-100">
                                    "{{ $step->remarks }}"
                                </div>
                            @endif
                            ...
                            @if ($step->return_reason)
                                <div
                                    class="mt-2 rounded-lg bg-orange-50 p-2 text-[10px] text-orange-800 border border-orange-100">
        ...
        @if ($approval->status == 'APPROVED')
            <div class="relative flex gap-4 mt-8">
                <div
                    class="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-600 ring-4 ring-emerald-100">
        ...
        @elseif($approval->status == 'REJECTED')
            <div class="relative flex gap-4 mt-8">
                <div
                    class="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-rose-600 ring-4 ring-rose-100">
        ...
        @elseif($approval->status == 'RETURNED')
            <div class="relative flex gap-4 mt-8">
                <div
                    class="relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-500 ring-4 ring-orange-100">
                    <i class="bi bi-arrow-return-left text-white text-xs"></i>
                </div>
                <div class="pt-1">
