@if ($pr->is_cancel)
  ...
  <a href="{{ route('purchaserequest.exportToPdf', $pr->id) }}"
    class="my-1 btn btn-outline-success my-1 ">
    ...
    <ul class="dropdown-menu">
      ...
      <li>
        @if ($pr->status === 4 && $user->specification->name === 'PURCHASER')
          <button data-bs-target="#edit-purchase-request-po-number-{{ $pr->id }}"
            data-bs-toggle="modal" class="my-1 btn btn-primary dropdown-item"><i
              class='bx bx-edit'></i> Edit PO
            Number</button>
        @endif
      </li>
    </ul>
