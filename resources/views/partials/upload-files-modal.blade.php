...
<div x-data="{ 
    ...
    {{-- Modal Content --}}
    <div class="relative w-full max-w-lg mx-4 transform transition-all"
        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden">
            ...
            <form x-ref="uploadForm" action="{{ route('file.upload') }}" method="post" enctype="multipart/form-data" class="flex flex-col">
                @csrf
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="relative group cursor-pointer"
                        ...
                        <input type="file" x-ref="dummyInput" class="hidden" multiple @change="handleFileSelect($event)">
                        ...
                        <div class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl transition-all duration-300 bg-slate-50/50"
                             :class="isDragging ? 'border-indigo-500 bg-indigo-50/30 scale-[1.02]' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50'">
                            
                            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-slate-100 mb-3 group-hover:scale-110 transition-transform duration-300">
                                <i class="bi bi-cloud-arrow-up text-2xl" 
                                   :class="isDragging ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-500'"></i>
                            </div>
                            
                            <p class="text-sm font-semibold text-slate-700" :class="isDragging ? 'text-indigo-700' : ''">
                    ...
                    <template x-if="files.length > 0">
                        <div class="space-y-3" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translateY-4" x-transition:enter-end="opacity-100 translateY-0">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400">Selected Files (<span x-text="files.length"></span>)</h4>
                            
                            <div class="max-h-48 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                                <template x-for="(file, index) in files" :key="index">
                                    <div class="flex items-center gap-3 p-2 rounded-lg border border-slate-100 bg-white shadow-sm group hover:border-indigo-100 transition-colors">
                                        {{-- Preview / Icon --}}
                                        <div class="h-10 w-10 flex-shrink-0 rounded-lg bg-slate-50 flex items-center justify-center overflow-hidden border border-slate-100">
