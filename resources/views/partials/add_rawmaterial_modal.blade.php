<div class="modal fade" id="add-rawmaterial-modal-{{ $detail->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('save.rawmaterial', $detail->id) }}">
                    @csrf
                    <input type="hidden" name="detail_id" value="{{ $detail->id }}"> <!-- Include the detail ID -->
                    ...
                    <select name="MasterId" class="form-select">
                        @foreach($masterDataCollection as $masterData)
                            @if($masterData->fg_code == $final)
                                <option value="{{ $masterData->id }}">{{ $masterData->rm_code }} (Quantity: {{ $masterData->rm_quantity }})(ID: {{ $masterData->id }})</option>
                                @php
                ...
                <select id="rm_warehouse" name="rm_warehouse">
