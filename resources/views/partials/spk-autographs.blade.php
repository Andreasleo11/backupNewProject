      ...
      <div class="row px-4 d-flex justify-content-center g-2 gx-4">
        <div class="col-auto">
          <form action="{{ route('spk.save.autograph', $report->id) }}" method="POST"
            ...
            @csrf @method('PUT')
            <input type="hidden" name="creator_autograph"
  ...
  <div class="col my-3 {{ $report->to_department === 'MAINTENANCE MACHINE' ? '' : 'd-none' }}">
    ...
    @if ($showPpicAutographButtons)
      <div class="row px-4 d-flex justify-content-center g-2 gx-4">
        ...
        <div class="col-auto">
          <form action="{{ route('spk.save.autograph', $report->id) }}" method="POST"
            id="formPpicAutograph">
            @csrf @method('PUT')
            <input type="hidden" name="ppic_autograph"
              value="{{ ucwords($authUser->name) . '.png' }}">
          </form>
          @include('partials.confirmation-modal', [
              'id' => '6',
      ...
      <div class="row px-4 d-flex justify-content-center g-2 gx-4">
        <div class="col-auto">
          <form action="{{ route('spk.save.autograph', $report->id) }}" method="POST"
            ...
            @csrf @method('PUT')
            <input type="hidden" name="admin_autograph"
              value="{{ ucwords($authUser->name) . '.png' }}">
          </form>
          @include('partials.confirmation-modal', [
              'id' => '3',
      ...
      <div class="row px-4 d-flex justify-content-center g-2 gx-4">
        <div class="col-auto">
          <form action="{{ route('spk.save.autograph', $report->id) }}" method="POST"
            id="formPicAutograph">
            @csrf @method('PUT')
            <input type="hidden" name="pic_autograph"
              value="{{ ucwords($authUser->name) . '.png' }}">
          </form>
          @include('partials.confirmation-modal', [
              'id' => '4',
