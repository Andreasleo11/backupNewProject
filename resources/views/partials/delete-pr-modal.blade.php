<div x-data="{
    ...
    @open-delete-pr-modal.window="open($event)" @keydown.escape.window="show = false" x-show="show"
    class="relative z-[100]" aria-labelledby="deletePrModal" role="dialog" aria-modal="true" x-cloak>
            ...
            <div x-show="show" @click.away="show = false" x-transition:enter="ease-out duration-300"
                ...
                <div class="p-5 bg-slate-50 relative">
                    <p class="text-slate-600 text-sm leading-relaxed">
                        ...
                        <span x-text="doc"
                            class="font-bold text-slate-800 text-base inline-block mt-2 px-3 py-1 bg-white border border-slate-200 rounded-lg shadow-sm"></span>
                        ?
                    </p>
                ...
                <div
                    ...
                    <button type="button" @click="show = false"
                        class="bg-slate-100 text-slate-600 hover:bg-slate-200 border-0 rounded-lg text-sm px-4 py-2 font-medium transition-colors">Cancel</button>
                    <form :action="`/purchase-requests/${id}`" method="POST" class="m-0">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="bg-rose-500 hover:bg-rose-600 hover:shadow-lg hover:shadow-rose-200 text-white border-0 rounded-lg text-sm px-4 py-2 font-medium transition-all flex items-center gap-1.5 cursor-pointer">
