<div class="mt-8 border-t border-slate-100 px-6 py-6">
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        @foreach ($record->workflow_signatures as $signature)
            ...
            <div
                class="group relative flex flex-col items-center justify-center rounded-xl border p-4 transition-all {{ $cardClasses }}">

                {{-- Status Badge --}}
                @if ($status === 'rejected')
                    <span
                        class="absolute right-2 top-2 rounded bg-red-100 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide text-red-600">
                        Rejected
                    </span>
                @endif
                ...
                @if ($signature['is_current'] ?? false)
                    <span
                        class="absolute right-2 top-2 rounded bg-amber-100 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide text-amber-600 animate-pulse">
                        Current
                    </span>
                @endif
                ...
                <div
                    class="mb-3 flex h-24 w-full items-center justify-center overflow-hidden rounded-lg transition-opacity {{ $status === 'signed' ? 'bg-slate-50 opacity-90 mix-blend-multiply group-hover:opacity-100' : '' }}">
                    @if ($status === 'signed' && $signature['image'])
                        <img src="{{ $signature['image'] }}" alt="Signature"
                            class="max-w-full h-auto max-h-20 object-contain mix-blend-darken">
                ...
                <div class="text-center">
                    <p
                        class="text-[10px] font-bold uppercase tracking-wider mb-0.5 {{ $status === 'signed' ? 'text-indigo-600' : 'text-slate-500' }}">
                        {{ $roleLabel }}
                    </p>
                ...
                @if ($status === 'signed')
                    <div
                        class="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm opacity-0 transition-all group-hover:opacity-100 group-hover:scale-110">
