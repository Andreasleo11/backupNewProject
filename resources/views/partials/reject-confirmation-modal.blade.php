    ...
    <form action="{{ $route }}" method="POST">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="reject-title-modal">Confirmation Reject</h1>
        </div>
        <div class="modal-body text-start">
          <div class="text-center" id="prompt">
            <label for="description" class="form-label">Are you sure you want to reject <span
                class="fw-semibold">{{ $doc_num }}</span>?</label>
          </div>
          <div class="description-form-group d-none">
            <label for="description" class="form-label">Description</label>
            <textarea name="description" class="form-control"
              placeholder="Tell us why you rejecting this report..." required></textarea>
          </div>
        </div>
        <div class="modal-footer">
          <button id="btnNo" type="button" class="btn btn-secondary"
...
<script>
  const modalTitle = document.querySelector('.modal-title#reject-title-modal');
