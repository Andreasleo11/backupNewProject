@php
    ...
    $baseClasses = 'relative inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest border transition-all overflow-hidden';
    $icon = '';
    ...
    if ($model && !empty($model->workflow_status)) {
        ...
        $actionStep = $approval && isset($approval->steps) ? $approval->steps->firstWhere('sequence', $currentStepSeq) : null;
    ...
    <div class="inline-flex items-center gap-2">
        <span class="{{ $baseClasses }} {{ $cls }}" @if($tooltip) title="{{ $tooltip }}" @endif>
            ...
            @if($progress > 0)
                ...
                <div class="absolute bottom-0 left-0 h-0.5 bg-current opacity-60 transition-all duration-1000" style="width: {{ $progress }}%"></div>
            @endif
        </span>
    </div>
@endif
