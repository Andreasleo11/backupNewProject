            ...
            <div class="flex flex-col items-center rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-1">
                    @if ($approval->status === 'approved')
                        <span
                            class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-semibold text-emerald-800">
                            ✓ Signed {{ optional($approval->signed_at)->diffForHumans() }}
                        </span>
                    @elseif ($approval->status === 'rejected')
                        <span
                            class="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2.5 py-1 text-[11px] font-semibold text-rose-800">
                            ✗ Rejected
                        </span>
                    @else
                        <span
                ...
                {{-- Signature box --}}
                <div
                    class="mt-3 flex h-24 w-52 items-center justify-center overflow-hidden rounded-md border border-slate-200 bg-slate-50">
                    @if ($approval->signature_path)
                        <img src="{{ asset('autographs/' . $approval->signature_path) }}" alt="Signature"
                    ...
                    @if ($approval->approver)
                        <div class="space-y-1 text-xs">
                            <div class="flex items-center justify-center gap-1 text-slate-500">
                                ...
                                <span>
                                    {{ optional($approval->signed_at) ? $approval->signed_at->timezone('Asia/Jakarta')->format('d-m-Y · H:i') : '—' }}
                                </span>
                            </div>
                        ...
                        @if ($showApprovalButtons)
                            ...
                            <div x-data="{ openReject: false }" class="mt-2 flex flex-col items-center gap-2">
                                ...
                                <div x-show="openReject" x-cloak x-transition.opacity
                                    ...
                                    <div @click.away="openReject = false" x-transition
                                        ...
                                        <div class="mb-3 flex items-start justify-between gap-3">
                                            <div>
                                                ...
                                                <p class="mt-1 text-xs text-slate-500">
                                                    Please provide a short explanation why this overtime form is
                                                    rejected.
                                                </p>
                                            </div>
                                        ...
                                        <form action="{{ route('overtime.reject', $header->id) }}" method="POST"
                                            ...
                                            <div>
                                                ...
                                                <textarea id="description-{{ $approval->id }}" name="description" required rows="4"
