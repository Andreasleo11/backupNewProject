<div x-data="{
    ...
    @open-edit-po-modal.window="open($event)" @keydown.escape.window="show = false" x-show="show" class="relative z-[100]"
    aria-labelledby="editPoModal" role="dialog" aria-modal="true" x-cloak>
            ...
            <div x-show="show" @click.away="show = false" x-transition:enter="ease-out duration-300"
                ...
                <form :action="`/purchase-requests/${id}/po-number`" method="post" class="m-0">
                        ...
                        <div class="mb-2">
                            <label for="po_number_input"
                                class="form-label text-sm font-bold text-slate-700 mb-1 block">PO Number for <span
                                    x-text="doc" class="text-indigo-600"></span></label>
                            <p class="text-xs text-slate-500 mb-3">Update the Purchase Order number associated with this
                                request.</p>

                            <div class="relative">
                                <i
                                    class="bx bx-hash absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg"></i>
                    ...
                    <div
                        ...
                        <button type="submit"
                            class="bg-indigo-600 hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-200 text-white border-0 rounded-lg text-sm px-4 py-2 font-medium transition-all flex items-center gap-1.5 cursor-pointer">
