...
<div class="space-y-4">
    ...
    @if($files->isEmpty())
        <div class="flex flex-col items-center justify-center py-8 text-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/50">
    ...
    @else
        <div class="grid {{ $gridCols }} gap-4">
            @foreach($files as $file)
                ...
                @endphp
                <div class="group flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-3 shadow-sm transition-all duration-300 hover:border-indigo-100 hover:shadow-md hover:shadow-indigo-500/5 hover:-translate-y-0.5">
                    <div class="flex items-center gap-3 overflow-hidden">
                        {{-- File Icon --}}
                        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-slate-50 shadow-inner group-hover:bg-indigo-50 transition-colors">
                            ...
                            @elseif(Str::contains($file->mime_type, 'spreadsheet') || Str::contains($file->mime_type, 'excel') || in_array($extension, ['xls', 'xlsx', 'csv']))
                        ...
                        <div class="min-w-0">
                            <p class="truncate text-sm font-bold text-slate-700 group-hover:text-indigo-600 transition-colors" title="{{ $file->name }}">
                                {{ $file->name }}
                            </p>
                    ...
                    <div class="flex items-center gap-1.5 pl-2">
                        ...
                        @if($showDelete)
                            <form action="{{ route('file.destroy', $file->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this file?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex h-9 w-9 items-center justify-center rounded-xl text-rose-400 transition-all hover:bg-rose-600 hover:text-white hover:shadow-lg hover:shadow-rose-200" title="Delete">
