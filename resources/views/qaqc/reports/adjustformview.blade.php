        ...
        <div class="row text-center mt-5">
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->department->name == 'QC' && $currentUser->specification->name == 'HEAD')
                    <button id="btn1" class="btn btn-primary" onclick="addAutograph(1, {{ $datas->id }})">Acc QC
                        Head</button>
                @endif
            </div>
            ...
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->department->name == 'STORE' && $currentUser->specification->name == 'HEAD')
                    <button id="btn3" class="btn btn-primary" onclick="addAutograph(3, {{ $datas->id }})">Acc Store
                        Head</button>
                @endif
            </div>
            ...
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->department->name == 'LOGISTIC' && $currentUser->specification->name == 'HEAD')
                    <button id="btn4" class="btn btn-primary" onclick="addAutograph(4, {{ $datas->id }})">Acc
                        Logistic Head</button>
                @endif
            </div>
            ...
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->department->name == 'PLASTIC INJECTION' && $currentUser->is_gm == 1)
                    <button id="btn5" class="btn btn-primary" onclick="addAutograph(5, {{ $datas->id }})">Acc GM
                    </button>
                @endif
            </div>
            ...
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->department->name == 'ACCOUNTING' && $currentUser->specification->name == 'HEAD')
                    <button id="btn6" class="btn btn-primary" onclick="addAutograph(6, {{ $datas->id }})">Acc
                        Accounting Head</button>
                @endif
            </div>
            ...
            <div class="col">
                ...
                @if (Auth::check() && $currentUser->specification->name == 'DIRECTOR')
                    <button id="btn7" class="btn btn-primary" onclick="addAutograph(7, {{ $datas->id }})">Acc
                        Director</button>
                @endif
            </div>
        </div>
        ...
        function checkAutographStatus(reportId) {
            ...
            for (var key in autographs) {
                if (autographs.hasOwnProperty(key)) {
                    ...
                    autographNames['autograph_name_' + autographNumber] =
                        autographName; // Append .png to autograph name
                }
            }
