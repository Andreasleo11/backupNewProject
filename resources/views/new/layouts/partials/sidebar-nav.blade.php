            ...
            @elseif ($item['type'] === 'single')
                ...
                <div class="relative group/nav-item" x-data="{ hover: false, flyoutTop: 0, myIdx: 'si-{{ $loop->index }}', pinned: false, pinLoading: false, flyoutTimer: null }"
                    ...
                    <a href="{{ route($item['route'], $item['params'] ?? []) }}"
                        ...
                        role="menuitem" tabindex="0" aria-current="{{ $isActive ? 'page' : 'false' }}">
                        ...
                        <div class="relative flex items-center justify-center shrink-0">
                            ...
                            @if ($isActive)
                                <span class="absolute -right-0.5 -top-0.5 flex h-2.5 w-2.5">
                                    <span
                                        class="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-500 ring-[2.5px] ring-white shadow-[0_0_8px_rgba(59,130,246,0.6)]"></span>
                                </span>
                            @endif
                        </div>

                        <span class="truncate text-[13px] tracking-tight flex-1 pt-0.5"
                        ...
                        @if (isset($item['badge']) && $item['badge'] > 0)
                            <span
                                class="ml-auto bg-rose-500 text-white text-[10px] px-1.5 py-0.5 rounded-lg min-w-[18px] text-center font-bold shadow-sm shadow-rose-200"
                                x-show="!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }}">
                                {{ $item['badge'] }}
                            </span>
                        @endif
                    ...
                    @if (!$isMobile)
                        <template x-teleport="body">
                            <div x-show="sidebarCollapsed && hover"
                                ...
                                x-transition:enter-end="opacity-100 translate-x-0" x-cloak :id="'flyout-' + myIdx"
                                ...
                                @if (isset($item['description']))
                                    <p class="text-[11px] leading-relaxed text-slate-500 font-medium max-w-[180px]">
                                        {{ $item['description'] }}</p>
                                @endif
                            </div>
                        </template>
            ...
            @elseif ($item['type'] === 'group')
                ...
                <div class="relative overflow-hidden" x-data="{
                    ...
                    <button type="button" @click="open = !open"
                        ...
                        role="menuitem" :aria-expanded="open">
                        <div class="flex items-center gap-3">
                            <span
                            ...
                            <span x-show="!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }}"
                                class="tracking-tight pt-0.5 flex-1 text-left">{{ $groupLabel }}</span>
                        </div>
                        <svg xmlns="http://www.w3.org/2000/svg"
                            :class="open ? 'rotate-90 text-blue-600' : 'text-slate-300'"
                            class="h-4 w-4 transition-transform duration-300" viewBox="0 0 20 20" fill="currentColor"
                            x-show="!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }}">
                    ...
                    <div x-show="(!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }})"
                        ...
                        <div class="overflow-hidden relative">
                            ...
                            <div class="absolute left-[30px] top-0 bottom-2 w-[2px] bg-slate-100/80 rounded-full z-0">
                            </div>
                                    ...
                                    <div role="none" class="relative group/child" x-data="{ pinned: false, pinLoading: false }">
                                            ...
                                            <div class="w-[60px] flex justify-center shrink-0">
                                                <div
                                                    class="w-1.5 h-1.5 rounded-full shrink-0 transition-all duration-300
                                                           {{ $childActive ? 'bg-blue-500 scale-125 shadow-[0_0_8px_rgba(59,130,246,0.6)]' : 'bg-slate-300 group-hover/child:bg-blue-400 group-hover/child:scale-110' }}">
                                                </div>
                                            </div>
                                            <span
                                                class="truncate tracking-tight flex-1 pt-0.5 pr-3">{{ $child['label'] }}</span>
                                        </a>
                                        {{-- ⭐ Pin Button --}}
                                        <button :class="{ '!opacity-100': pinned }" :disabled="pinLoading"
                                            ...
                                            <svg class="h-3 w-3 transition-colors duration-200"
                                                :class="pinned ? 'text-amber-500 fill-amber-400' :
                    ...
                    @if (!$isMobile)
                        <template x-teleport="body">
                            <div x-show="sidebarCollapsed && flyoutOpen"
                                ...
                                x-transition:enter-end="opacity-100 scale-100 translate-x-0" x-cloak
                                :id="'flyout-' + myIdx"
                                ...
                                <div class="px-3 py-3 border-b border-slate-100/80 mb-1">
                                    <div
                                        class="font-black text-blue-950 uppercase tracking-widest text-[10px] flex items-center gap-2">
                                    ...
                                    @foreach ($children as $child)
                                        ...
                                        <li>
                                            <a href="{{ route($child['route'], $child['params'] ?? []) }}"
                                                ...
                                                <span
                                                    class="flex h-6 w-6 items-center justify-center rounded-lg transition-all duration-300
