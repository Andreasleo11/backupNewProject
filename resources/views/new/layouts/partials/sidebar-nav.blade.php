            ...
            @elseif ($item['type'] === 'single')
                ...
                <li class="relative group/nav-item"
                    ...
                    <button x-show="!sidebarCollapsed && !{{ $isMobile ? 'true' : 'false' }}"
                            x-cloak
                            :class="pinned ? 'opacity-100' : 'opacity-0 group-hover/nav-item:opacity-100'"
                            :disabled="pinLoading"
                            @click.prevent="
                                pinLoading = true;
                                if (pinned) {
                        ...
                        <svg class="h-3.5 w-3.5 transition-colors duration-200"
                             :class="pinned ? 'text-amber-500 fill-amber-400' : 'text-slate-300 hover:text-amber-400'"
                    ...
                    <ul x-show="open && (!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }})"
                        ...
                        @foreach ($children as $child)
                            ...
                            <li role="none" class="relative group/child"
                                ...
                                {{-- ⭐ Pin Button --}}
                                <button :class="pinned ? 'opacity-100' : 'opacity-0 group-hover/child:opacity-100'"
                                        :disabled="pinLoading"
                                        @click.prevent="
                                            pinLoading = true;
                                            if (pinned) {
