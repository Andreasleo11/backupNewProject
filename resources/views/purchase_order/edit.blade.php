      ...
      <form action="{{ route('po.update', $po->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
          ...
          <input type="text" name="invoice_date" id="invoice_date" class="form-control"
            value="{{ old('invoice_date', $po->invoice_date) }}" required
        ...
        <div class="form-group mb-3">
          ...
          <select name="purchase_order_category_id" id="purchase_order_category_id"
            class="form-select">
          ...
          <div class="input-group">
            <div class="col-auto">
              <select name="currency" id="currency" class="form-select" required>
                <option value="IDR"
                  {{ old('currency', $po->currency) == 'IDR' ? 'selected' : '' }}>Rp
                </option>
                <option value="YUAN"
                  {{ old('currency', $po->currency) == 'YUAN' ? 'selected' : '' }}>¥
                </option>
                <option value="USD"
                  {{ old('currency', $po->currency) == 'USD' ? 'selected' : '' }}>$
                </option>
              </select>
            </div>
            <div class="col ms-1">
        ...
        <div class="form-group mb-3">
          ...
          @if ($po->filename)
            <div class="mb-2">
              <p><strong>Current File:</strong> <a
          ...
          <input type="file" name="pdf_file" id="pdf_file" accept="application/pdf"
            class="form-control" aria-describedby="pdfFileHelp">
