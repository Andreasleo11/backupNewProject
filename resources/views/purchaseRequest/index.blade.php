        ...
        <div class="card mt-5">
            <div class="card-body pe-4">
                <div class="table-responsive p-4">
                    <div class="mb-3">
                        <button id="approve-selected-btn" data-approve-url="{{ route('director.pr.approveSelected') }}" class="btn btn-primary">Approve Selected</button>
                        @include('partials.approve-confirmation-modal')
                        <button id="reject-selected-btn" data-reject-url="{{ route('director.pr.rejectSelected') }}" class="btn btn-danger">Reject Selected</button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        document.getElementById('confirmApprove').addEventListener('click', function(){
            ...
            if(checkboxes.length>0){
                ...
                showInfoModal('Cannot Approve', 'You cannot approve because there is no selected purchase request.');
            }
        });
    ...
    function getSelectedReportIdsWithStatus() {
        ...
        checkboxes.forEach(function(checkbox) {
            ...
            reportIdWithStatus.push({ id: reportId, status: status, docNum: docNum});
        });
    ...
    var approvalStatusMap = {/* Map of report IDs to their approval status (e.g., 'approved', 'rejected') */};
    ...
    approveSelectedButton.addEventListener('click', function(){
        ...
        if (hasApprovedOrRejected) {
            showInfoModal('Cannot Approve', 'You cannot approve or reject the selected documents because there are already approved or rejected documents among them.');
        } else {
            ...
            console.log('Executing approval process for selected purchase request:', selectedReportIdsWithStatus);
        }
    ...
    rejectSelectedButton.addEventListener('click', function(){
        ...
        // If any selected report has an approval status of 'approved' or 'rejected', show the modal
        if (hasApprovedOrRejected) {
            showInfoModal('Cannot Reject', 'You cannot approve or reject the selected documents because there are already approved or rejected documents among them.');
        } else {
            ...
            if(checkboxes.length > 0){
                ...
                showInfoModal('Cannot Reject', 'You cannot reject because there is no selected purchase request.');
            }
            console.log('Executing rejection process for selected purchase request:', selectedReportIdsWithStatus);
        }
    });
    ...
    function showRejectModal(){
        const modal = new bootstrap.Modal(document.getElementById('reject-selected-modal'), { backdrop: 'static' })
        modal.show();
    }

    function showApproveModal(){
        const modal = new bootstrap.Modal(document.getElementById('approve-confirmation-modal'), { backdrop: 'static' })
        modal.show();
    }
