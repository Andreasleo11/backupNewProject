...
<div class="max-w-7xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
    ...
    <div class="flex-1 w-full lg:max-w-4xl space-y-6">
        ...
        <div class="mb-2">
            ...
            <p class="text-sm text-slate-500 mt-1">Specify branch, vehicle, and destinations carefully. Estimations will
                be processed through the approval workflow.</p>
        </div>
        ...
        <form wire:submit.prevent="submit" novalidate id="dn-form" class="space-y-6">
            ...
            {{-- Draft toggle --}}
            <label for="is_draft"
                class="flex items-start gap-3 rounded-xl border border-slate-200 bg-gradient-to-r from-slate-50 to-white px-5 py-4 cursor-pointer hover:border-indigo-300 transition-colors shadow-sm">
...
@enderror">
                            ...
                            <ul x-show="show && filtered().length" x-transition
                                ...
                                <template x-for="item in filtered()" :key="item.id">
                                    <li @click="select(item)"
                                        class="cursor-pointer px-4 py-2 hover:bg-slate-50 border-b border-slate-50 last:border-0 transition-colors">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between mt-8 mb-2">
                    ...
                    <span
                        class="text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 shadow-sm">
                ...
                @foreach ($destinations as $index => $dest)
                    <fieldset
                        class="relative rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow transition-shadow overflow-hidden group">

                        <div
                            class="absolute left-0 top-0 bottom-0 w-1.5 bg-indigo-500 group-hover:bg-indigo-600 transition-colors">
                        </div>

                        <div
                            class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                            <p
                                class="text-sm font-bold uppercase tracking-wider text-slate-600 flex items-center gap-2">
                                <span
                                    class="flex h-5 w-5 items-center justify-center rounded-full bg-indigo-100 text-[10px] text-indigo-700 ring-1 ring-inset ring-indigo-200">{{ $index + 1 }}</span>
                                Stop Location
                            </p>
                            <button type="button"
                                class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50 hover:text-red-700 transition"
                                ...
                                <div x-data="{
                                    ...
                                    <div class="relative">
                                        <div
                                            ...
                                            <svg class="h-4 w-4 text-slate-400" viewBox="0 0 20 20"
                                                fill="currentColor">
                                                <path fill-rule="evenodd"
                            ...
                            <div x-data="{
                                ...
                                <template x-if="orders.length > 0">
                                    <div class="space-y-2 mb-2">
                                        <template x-for="(chunk, rowIndex) in chunkedOrders()"
                                            :key="rowIndex">
                                            <div class="grid gap-2 grid-cols-2">
                                                <template x-for="(order, i) in chunk" :key="i">
                                                    <div
                                                        class="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-md pr-1 overflow-hidden focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
                                                        <div
                                                            class="bg-slate-100 px-2 py-2 border-r border-slate-200 text-slate-500 text-xs font-semibold">
                                                            #
                                                        </div>
                                                        <input type="text"
                        ...
                        <div class="space-y-1.5 lg:col-span-2">
                            ...
                            <textarea rows="2" wire:model="destinations.{{ $index }}.remarks"
                                placeholder="Optional notes for this leg of the trip..."
                        ...
                        <div class="lg:col-span-2 mt-2" x-data="{ showCosts: {{ $dest['driver_cost'] > 0 || $dest['kenek_cost'] > 0 || $dest['balikan_cost'] > 0 ? 'true' : 'false' }} }">
                            <button type="button" @click="showCosts = !showCosts"
                                ...
                                <svg x-show="!showCosts" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <span
                                    x-text="showCosts ? 'Hide Cost Estimations' : 'Add Cost Estimations (Optional)'"></span>
                            </button>

                            <div x-show="showCosts" x-transition:enter="transition ease-out duration-200"
                                x-transition:enter-start="opacity-0 translate-y-2"
                                x-transition:enter-end="opacity-100 translate-y-0" style="display: none;"
                                class="mt-4 rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                                <h3
                                    class="text-xs font-bold text-slate-800 uppercase tracking-wide mb-3 flex items-center gap-2">
...
<aside class="w-full lg:w-80 flex-shrink-0 mt-6 lg:mt-0">
    <div
        class="sticky top-8 rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-200/40 overflow-hidden flex flex-col">
        ...
        <div class="px-6 py-5 flex-1 relative">
            ...
            <div class="absolute right-0 top-0 opacity-[0.03] pointer-events-none">
                <svg width="120" height="120" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M12 2L2 22h20L12 2zm0 4.5l6.5 13h-13L12 6.5z" />
                </svg>
            </div>
            ...
            <div class="mt-8 rounded-xl bg-slate-50 border border-slate-100 p-4 relative overflow-hidden group">
                <div
                    class="absolute -right-4 -top-4 w-16 h-16 bg-emerald-100 rounded-full blur-xl scale-0 group-hover:scale-100 transition-transform duration-700">
                </div>
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest relative z-10">Total Est.
                    Cost</p>
                <p
                    class="mt-1 text-2xl font-black text-emerald-600 tracking-tight flex items-baseline gap-1 relative z-10">
            ...
            @if ($this->total_cost > 10000000)
                <div class="mt-4 flex items-start gap-2 rounded-lg bg-red-50 p-3 border border-red-100">
                    <svg class="h-4 w-4 text-red-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @else
                <div class="mt-4 flex items-start gap-2 rounded-lg bg-indigo-50 p-3 border border-indigo-100">
                    <svg class="h-4 w-4 text-indigo-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white border-t border-slate-100 p-5 space-y-3">
            <button type="submit" form="dn-form"
                class="relative group w-full flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-6 py-3.5 text-sm font-bold text-white shadow-md hover:shadow-xl hover:bg-black transition-all overflow-hidden focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2">
                <span
            ...
            <a href="{{ route('delivery-notes.index') }}"
                class="w-full flex justify-center items-center py-3 text-sm font-bold text-slate-500 hover:text-slate-800 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl transition-colors">
                Discard Changes
            </a>
        </div>
