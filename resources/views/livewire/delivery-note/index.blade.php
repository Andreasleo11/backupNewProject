...
<div x-data="{ showFilters: false }" class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    @if (session()->has('success'))
        <div
            class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 shadow-sm flex items-start gap-3">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="relative w-full lg:max-w-md group">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg class="h-4 w-4 text-slate-400 group-focus-within:text-indigo-500 transition-colors" fill="none"
    ...
    <div x-show="showFilters" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
            ...
            <div>
                ...
                <select wire:model.defer="inputStatus"
                    class="block w-full rounded-lg border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                ...
                <select wire:model.defer="inputBranch"
                    class="block w-full rounded-lg border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                ...
                <select wire:model.defer="inputRitasi"
                    class="block w-full rounded-lg border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500">
            ...
            {{-- From Date --}}
            <div>
                <label class="block text-xs font-semibold tracking-wide text-slate-600 mb-1.5 uppercase">From
                    Date</label>
                <input type="date" wire:model.defer="inputFromDate"
                    class="block w-full rounded-lg border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            ...
            <div>
                ...
                <input type="date" wire:model.defer="inputToDate"
                    class="block w-full rounded-lg border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500">
            </div>
        </div>

        <div class="mt-6 flex justify-end border-t border-slate-100 pt-5">
            <button type="button" wire:click="applyFilters" @click="showFilters = false"
                class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-slate-900 px-6 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 transition focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2">
    ...
    @if ($filterStatus !== 'all' || $filterBranch !== 'all' || $filterRitasi !== 'all' || $fromDate || $toDate || $searchAll)
        <div class="flex flex-wrap items-center gap-2 mt-2">
            ...
            @if ($filterStatus !== 'all')
                <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 border border-slate-200 shadow-sm text-xs">
            ...
            @if ($searchAll)
                <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-3 py-1 border border-indigo-100 shadow-sm text-xs">
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col mt-4">
        <div class="border-b border-slate-100 bg-slate-50/50 px-5 py-3.5 flex items-center justify-between">
            ...
            <span
                class="text-xs font-semibold bg-white border border-slate-200 py-1 px-3 rounded-full text-slate-500 shadow-sm">
