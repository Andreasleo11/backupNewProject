    ...
    <div class="grid lg:grid-cols-5 gap-4 mb-5">
        ...
        <div class="glass-card px-5 py-5 lg:col-span-2">
            ...
            <div class="flex h-4 rounded-full overflow-hidden gap-0.5">
                ...
                @endif
                @if ($dist['p100'] > 0)
                    <div class="bg-emerald-400 transition-all {{ !$dist['p0_49'] && !$dist['p50_99'] ? 'rounded-l-full' : '' }} rounded-r-full"
                        style="width:{{ $dist['p100'] }}%" title="100%: {{ $dist['c100'] }}"></div>
                @endif
            </div>
            <div class="flex justify-between items-center mt-3 text-xs">
                <div class="flex items-center gap-1.5">
                    <span class="h-2 w-2 rounded-full bg-rose-400 inline-block"></span>
                    <span class="text-slate-500">0–49%: <strong
                            class="text-slate-700">{{ $dist['c0_49'] }}</strong></span>
                </div>
                <div class="flex items-center gap-1.5">
                    <span class="h-2 w-2 rounded-full bg-amber-400 inline-block"></span>
                    <span class="text-slate-500">50–99%: <strong
                            class="text-slate-700">{{ $dist['c50_99'] }}</strong></span>
                </div>
                <div class="flex items-center gap-1.5">
                    <span class="h-2 w-2 rounded-full bg-emerald-400 inline-block"></span>
                    <span class="text-slate-500">100%: <strong
                            class="text-slate-700">{{ $dist['c100'] }}</strong></span>
                </div>
            </div>
        </div>
            ...
            <div class="divide-y divide-slate-50">
                @forelse($pending as $u)
                    <div class="flex items-start gap-3 px-5 py-3">
                        ...
                        <span
                            class="text-xs text-slate-400 shrink-0 pt-0.5">{{ $u->created_at->diffForHumans() }}</span>
                    </div>
                @empty
            ...
            <div class="divide-y divide-slate-50">
                @forelse($expiring as $u)
                    ...
                    <div class="flex items-start gap-3 px-5 py-3">
                        ...
                        <span
                            class="text-xs font-semibold shrink-0 pt-0.5
                ...
                succeed(({
                    ...
                    requestAnimationFrame(() => {
                        if (window._complianceTrend) {
                            window._complianceTrend.data.labels =
                                @json($trendLabels);
                            window._complianceTrend.data.datasets[0].data =
                                @json($trendValues);
