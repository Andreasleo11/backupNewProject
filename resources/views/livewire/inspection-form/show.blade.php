@php
    /** @var \App\Models\InspectionReport $inspectionReport */
    $h = $inspectionReport; // shorthand
    $r = $inspectionReport; // shorthand
    $periods = $r->detailInspectionReports->pluck('period')->sort();
@endphp

<div class="container py-4">
    @php $qs = request()->getQueryString(); @endphp
    <a href="{{ route('inspection-reports.index') }}@if ($qs) ?{{ $qs }} @endif"
        class="text-decoration-none small" onclick="if (history.length > 1) { history.back(); return false; }">
    ...
    <ul class="nav nav-pills my-4" id="qTab" role="tablist">
        @foreach (range(1, 4) as $p)
            @php $hasData = in_array($p, $filledPeriods, true); @endphp
            <li class="nav-item">
                <button
                    class="nav-link me-2 border {{ $hasData ? 'border-primary' : 'border-secondary text-muted opacity-50' }} {{ $loop->first ? 'active' : '' }}"
    ...
    {{-- Problems (per shift) --}}
    @php
        $problemRows = $r->problemData ?? collect();
    @endphp
