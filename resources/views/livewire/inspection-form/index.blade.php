<div class="container py-4" wire:init="load">
                    ...
                    @foreach ($showCol as $key => $val)
                        <label class="dropdown-item d-flex align-items-center gap-2">
                            <input type="checkbox" class="form-check-input"
                                wire:model.live="showCol.{{ $key }}">
                            <span class="text-capitalize">{{ str_replace('_', ' ', $key) }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
            <button class="btn btn-outline-primary" wire:click="exportCsv" wire:loading.attr="disabled"
    ...
    <div class="table-responsive">
        <table class="table table-striped align-middle mb-0" aria-describedby="reports-caption"
            ...
            <thead class="table-light">
                ...
                <tr class="small">
                    ...
                    @if ($showCol['document_number'])
                        <th>
                            <input type="text" class="form-control form-control-sm" placeholder="Doc no…"
                                wire:key="f-doc" wire:model.live.debounce.500ms="filters.document_number">
                        </th>
                    @endif
                    ...
                    @if ($showCol['inspection_date'])
                        <th>
                            <div class="d-flex gap-1">
                                <input type="date" class="form-control form-control-sm" wire:key="f-date_from"
                                    wire:model.live="filters.date_from" title="From">
                                <input type="date" class="form-control form-control-sm" wire:key="f-date_to"
                    ...
                    @if ($showCol['shift'])
                        <th>
                            <select class="form-select form-select-sm" wire:model.live="filters.shift"
                                wire:loading.attr="disabled">
                    ...
                    @if ($showCol['customer'])
                        <th>
                            <input type="text" class="form-control form-control-sm" placeholder="Customer…"
                                wire:key="f-cust" wire:model.live.debounce.500ms="filters.customer">
                        </th>
                    @endif

                    @if ($showCol['part_number'])
                        <th>
                            <input type="text" class="form-control form-control-sm" placeholder="Part no…"
                                wire:key="f-part" wire:model.live.debounce.500ms="filters.part_number">
                        </th>
                    @endif
            ...
            <tbody wire:loading.class="opacity-50" role="rowgroup">
                ...
                @if (!$ready)
                    @php
                        ...
                        $sk = fn($w = '100%') => '<span class="placeholder col-12" style="max-width:' .
                            $w .
                            '"></span>';
                    @endphp
