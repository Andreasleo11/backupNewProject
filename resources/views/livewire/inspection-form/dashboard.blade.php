<div class="insp-dash">
    ...
    <style>
        ...
        .dash-header h1 {
            ...
            font-weight: 700;
            margin: 0;
            background: linear-gradient(135deg, var(--dash-accent1), var(--dash-accent2));
        ...
        .kpi-card.c1::before {
            background: linear-gradient(90deg, var(--dash-accent1), #818cf8);
        }

        .kpi-card.c2::before {
            background: linear-gradient(90deg, var(--dash-accent2), #22d3ee);
        }

        .kpi-card.c3::before {
            background: linear-gradient(90deg, var(--dash-accent3), #facc15);
        }

        .kpi-card.c4::before {
            background: linear-gradient(90deg, var(--dash-accent4), #e879f9);
        }

        .kpi-card.c5::before {
            background: linear-gradient(90deg, #818cf8, var(--dash-accent1));
        }

        .kpi-card.c6::before {
            background: linear-gradient(90deg, #f87171, var(--dash-accent3));
        }

        .kpi-card.c7::before {
            background: linear-gradient(90deg, #a78bfa, var(--dash-accent4));
        }
        ...
        .badge-shift.s1 {
            background: var(--dash-accent1);
        }

        .badge-shift.s2 {
            background: var(--dash-accent3);
        }
        ...
        .btn-sm-dash {
            ...
            padding: .2rem .6rem;
            border-radius: 6px;
            border: 1px solid var(--dash-accent1);
            color: var(--dash-accent1);
    ...
    <div class="dash-header">
        <div>
            ...
            <p class="dash-subtitle">Quality overview &mdash;
                {{ $dateFrom ? \Carbon\Carbon::parse($dateFrom)->format('d M Y') : '—' }} to
                {{ $dateTo ? \Carbon\Carbon::parse($dateTo)->format('d M Y') : '—' }}</p>
        </div>
    ...
    <div class="filter-bar">
        <div>
            ...
            <input type="date" class="form-control form-control-sm" :value="$wire.dateFrom"
                @change="$wire.set('dateFrom', $event.target.value)" style="min-width:140px">
        </div>
        <div>
            <label>Date To</label>
            <input type="date" class="form-control form-control-sm" :value="$wire.dateTo"
                @change="$wire.set('dateTo', $event.target.value)" style="min-width:140px">
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" class="form-control form-control-sm" placeholder="Search part…"
                wire:model.live.debounce.400ms="partNumber" style="min-width:140px">
        </div>
    ...
    <div class="kpi-grid">
        <div class="kpi-card c1">
            ...
            <div class="kpi-value" style="color:var(--dash-accent1)">{{ number_format($kpi['total_reports'] ?? 0) }}
            </div>
            <div class="kpi-unit">in selected period</div>
        </div>
        <div class="kpi-card c2">
            <div class="kpi-label">Total Output</div>
            <div class="kpi-value" style="color:var(--dash-accent2)">{{ number_format($kpi['total_output'] ?? 0) }}
            </div>
        ...
        <div class="kpi-card c4">
            ...
            <div class="kpi-value" style="color:var(--dash-accent2)">{{ $kpi['avg_pass_rate'] ?? 0 }}<span
                    style="font-size:1rem">%</span></div>
            <div class="kpi-unit">across all reports</div>
        </div>
        <div class="kpi-card c3">
            <div class="kpi-label">Avg Reject Rate</div>
            <div class="kpi-value" style="color:var(--dash-accent3)">{{ $kpi['avg_reject_rate'] ?? 0 }}<span
                    style="font-size:1rem">%</span></div>
            <div class="kpi-unit">across all reports</div>
        </div>
        <div class="kpi-card c5">
            <div class="kpi-label">Avg NG Sample Rate</div>
            <div class="kpi-value" style="color:#818cf8">{{ $kpi['avg_ng_sample_rate'] ?? 0 }}<span
                    style="font-size:1rem">%</span></div>
                        ...
                        <tbody>
                            @foreach ($topFailingParts as $i => $part)
                                <tr>
                                    ...
                                    <td class="font-monospace" style="font-size:.78rem">{{ $part['part_number'] }}
                                    </td>
                                    <td style="color:var(--dash-muted);font-size:.78rem">
                                        {{ Str::limit($part['part_name'], 24) }}</td>
                                    <td>{{ $part['reports'] }}</td>
                                    <td>
                                        <span
                                            style="color:{{ $part['avg_reject'] > 5 ? '#f87171' : ($part['avg_reject'] > 2 ? 'var(--dash-accent3)' : 'var(--dash-accent2)') }}; font-weight:700">
                                            {{ $part['avg_reject'] }}%
                                        </span>
                                        <div class="rate-bar">
                                            <div class="rate-bar-fill"
                                                style="width:{{ min($part['avg_reject'] * 5, 100) }}%;background:{{ $part['avg_reject'] > 5 ? '#f87171' : 'var(--dash-accent3)' }}">
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            @foreach ($topProblemTypes as $i => $prob)
                                <tr>
                                    ...
                                    <td>
                                        <div class="rate-bar">
                                            <div class="rate-bar-fill"
                                                style="width:{{ $maxCount ? round(($prob['count'] / $maxCount) * 100) : 0 }}%;background:var(--dash-accent4)">
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <table class="dash-table">
                        ...
                        <tbody>
                            @foreach ($latestReports as $r)
                                <tr>
                                    <td style="white-space:nowrap;color:var(--dash-muted);font-size:.78rem">
                                        {{ $r['inspection_date'] }}</td>
                                    <td class="font-monospace" style="font-size:.76rem">{{ $r['document_number'] }}
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('inspection-reports.show', $r['id']) }}"
                                            class="btn-sm-dash">View</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                            ...
                            @foreach ($dimensionFailures as $i => $dim)
                                <tr>
                                    ...
                                    <td>
                                        <div class="rate-bar">
                                            <div class="rate-bar-fill"
                                                style="width:{{ $maxNg ? round(($dim['ng_count'] / $maxNg) * 100) : 0 }}%;background:#f87171">
                                            </div>
                                        </div>
                                    </td>
                                </tr>
