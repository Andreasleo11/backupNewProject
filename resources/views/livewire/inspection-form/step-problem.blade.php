  ...
  <x-lockable-card :locked="$locked" :overlay="$overlay" title="Problems" :isSaved=$isSaved class="mt-4">
    <div data-step-problem>
      <div wire:ignore
           ...
           x-init="
             const root = $el.closest('[data-step-problem]');; 
             ...
             Livewire.on('problemsSaved', e => { dirty = false; saved = true; ts = e?.savedAt ?? new Date().toISOString() });
             Livewire.on('problemsReset', () => { dirty = false; saved = false; ts = null });
           "
          ...
          <div class="card-body">
            ...
            <input type="text"
              wire:model.blur="problems.{{ $index }}.inspection_report_document_number" readonly
               @class([
                     'form-control','bg-secondary-subtle',
                     'is-invalid' => $errors->has("problems.$index.inspection_report_document_number"),
                     'is-valid'   => $rk && $this->isRowFieldSaved($rk, 'inspection_report_document_number') && !$errors->has("problems.$index.inspection_report_document_number"),
                   ])>
          ...
          <div class="row g-3">
            <div class="col">
              ...
              <select wire:model.blur="problems.{{ $index }}.type"
                @class([
                  ...
                  'is-valid'   => $rk && $this->isRowFieldSaved($rk, 'type') && !$errors->has("problems.$index.type"),
                ])>
            ...
            <div class="col">
              ...
              $watch('v', val => fp.setDate(val, false));">
                <input type="text" x-ref="t" readonly
                  @class([
                    'form-control',
                    'is-invalid' => $errors->has('problems.' . $index . '.time'),
                    'is-valid'   => $rk && $this->isRowFieldSaved($rk, 'time') && !$errors->has('problems.' . $index . '.time'),
                  ])>
              </div>
            ...
            <div class="col">
              ...
              <input type="number" wire:model.blur="problems.{{ $index }}.cycle_time"
                ...
                @class([
                  ...
                  'is-valid'   => $rk && $this->isRowFieldSaved($rk, 'cycle_time') && !$errors->has("problems.$index.cycle_time"),
                ])>
            ...
            <div class="col">
              ...
              <input type="text" wire:model.blur="problems.{{ $index }}.remarks"
                  ...
                  @class([
                    ...
                    'is-valid'   => $rk && $this->isRowFieldSaved($rk, 'remarks') && !$errors->has("problems.$index.remarks"),
                  ]) required>
