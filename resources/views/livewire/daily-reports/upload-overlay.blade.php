            ...
            <div x-show="isOpen" @click.away="isOpen = false" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="absolute right-0 top-0 hidden pr-4 pt-4 sm:block">
                    <button @click="isOpen = false" type="button"
                        class="rounded-xl bg-white text-slate-400 hover:text-slate-500 transition-colors">
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-left w-full">
                            ...
                            <p class="text-sm font-medium text-slate-500 mb-6 border-b border-slate-100 pb-4">
                                @if ($step === 'upload')
                                    Unggah file Excel atau CSV untuk pratinjau.
                                @elseif($step === 'preview')
                            ...
                            @if ($step === 'upload')
                                <div x-data="{ dragging: false, fileName: '' }" class="space-y-6">
                                    <div class="relative flex justify-center rounded-3xl border-2 border-dashed px-6 py-16 transition-all cursor-pointer"
                                        :class="dragging ? 'border-indigo-500 bg-indigo-50/50 scale-[0.98]' :
                                        ...
                                        <div class="text-center">
                                            ...
                                            <div x-show="fileName"
                                                class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-600 mb-4"
                                                style="display: none;">
                                                <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <div class="flex text-sm text-slate-600 font-bold mb-1">
                                                ...
                                                <span x-show="!fileName" class="ml-1 font-medium">atau tarik file ke
                                                    sini</span>
                                            </div>
                                            <p class="text-xs text-slate-400 font-medium">XLSX, CSV (Maks 10MB)</p>
                                        </div>
                                        <input type="file" wire:model="report_file" x-ref="fileInput" class="hidden"
                                            @change="fileName = $event.target.files[0]?.name || ''">
                                    </div>

                                    @error('report_file')
                                        <span
                                            class="text-xs font-bold text-rose-500 bg-rose-50 px-3 py-2 rounded-xl flex items-center gap-2">
                                    ...
                                    <div class="flex items-center justify-end border-t border-slate-50 pt-6">
                                        <button wire:click="processUpload" wire:loading.attr="disabled"
                                            ...
                                            <svg wire:loading.remove class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="space-y-6">
                                    <div class="overflow-x-auto rounded-2xl border border-slate-100 bg-slate-50/50">
                                        <table class="min-w-full text-left text-sm">
                                            <thead
                                                class="bg-slate-50 text-[10px] uppercase tracking-widest font-black text-slate-400">
                                                <tr>
                                            ...
                                            <tbody class="divide-y divide-slate-100">
                                                @foreach (array_slice($previewData, 0, 10) as $row)
                                                    <tr>
                                                        <td
                                                            class="px-4 py-3 font-bold text-slate-700 whitespace-nowrap">
                                                            {{ $row['work_date'] }}</td>
                                                        <td
                                                        ...
                                                        <td class="px-4 py-3">
                                                            <div class="font-black text-indigo-600">
                                                                {{ $row['employee_name'] }}</div>
                                                            <div class="text-[10px] font-bold text-slate-400">
                                                                {{ $row['employee_id'] }}</div>
                                                        </td>
                                                        <td class="px-4 py-3 text-slate-600 line-clamp-2">
                                                            {{ $row['work_description'] }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                        @if (count($previewData) > 10)
                                            <div
                                                class="px-4 py-3 text-center text-xs font-bold text-slate-400 bg-white">
                                    ...
                                    <div class="flex items-center justify-between border-t border-slate-50 pt-6">
                                        <button wire:click="$set('step', 'upload')"
                                            class="text-sm font-bold text-slate-400 hover:text-slate-600">Kembali</button>
                                ...
                                <div class="space-y-6">
                                    <div class="grid grid-cols-3 gap-4 mb-4">
                                        <div class="bg-emerald-50 rounded-2xl p-4 text-center">
                                            <div class="text-2xl font-black text-emerald-600">
                                                {{ collect($logs)->where('status', 'Berhasil')->count() }}</div>
                                            <div
                                        ...
                                        <div class="bg-amber-50 rounded-2xl p-4 text-center">
                                            <div class="text-2xl font-black text-amber-600">
                                                {{ collect($logs)->filter(fn($l) => str_contains($l['status'], 'Duplikat'))->count() }}
                                            </div>
                                            <div
                                        ...
                                        <div class="bg-rose-50 rounded-2xl p-4 text-center">
                                            <div class="text-2xl font-black text-rose-600">
                                                {{ collect($logs)->where('status', 'Gagal')->count() }}</div>
                                            <div
                                                class="text-[10px] font-black text-rose-700 uppercase tracking-widest">
                                                Gagal</div>
                                        </div>
                                    </div>

                                    <div
                                        class="max-h-64 overflow-y-auto rounded-2xl border border-slate-100 divide-y divide-slate-100">
                                        @foreach ($logs as $log)
                                            <div class="px-4 py-3 flex items-center justify-between text-xs">
                                                <div>
                                                    <span
                                                        class="font-black text-slate-700">{{ $log['employee_name'] }}</span>
                                                    <span
                                                        class="text-slate-400 font-medium ml-1">({{ $log['work_date'] }})</span>
                                                </div>
                                                <span
                                                    class="font-black @if ($log['status'] === 'Berhasil') text-emerald-600 @else text-amber-600 @endif">{{ $log['status'] }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="flex justify-end pt-4">
                                        <button @click="isOpen = false"
                                            class="rounded-2xl bg-slate-900 px-8 py-3.5 text-sm font-black text-white shadow-xl transition-all hover:bg-slate-800">Tutup</button>
                                    </div>
                                </div>
                            @endif
                        </div>
