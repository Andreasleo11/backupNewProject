<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8" x-data="{ showFilters: false }">
    ...
    <div class="relative z-50 rounded-3xl bg-slate-900 shadow-2xl">
        ...
        <div class="relative px-8 py-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            ...
            <div class="flex items-start gap-6">
                <a href="{{ route('daily-reports.index') }}" wire:navigate class="mt-1 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-white shadow-sm ring-1 ring-white/20 transition-all hover:bg-white/20 hover:scale-105 shrink-0" title="Kembali">
                ...
                <div class="flex flex-col gap-1">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="inline-flex items-center rounded-full bg-indigo-500/20 px-3 py-1 text-xs font-bold text-indigo-300 ring-1 ring-inset ring-indigo-500/30 backdrop-blur-md">
                            {{ $this->employee->nik }}
                        </span>
                    </div>
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium text-slate-400">
                        <span class="flex items-center gap-2">
                            <svg class="h-4 w-4 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-7h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
        ...
        <div class="rounded-b-3xl border border-t-0 border-slate-200 bg-white p-6 shadow-xl">
            <div class="grid gap-5 md:grid-cols-12 md:items-end">
                <div class="md:col-span-4">
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Tanggal Mulai</label>
                    <input type="date" wire:model.live="filter_start_date" class="block w-full rounded-xl border-slate-200 bg-slate-50 py-2.5 px-3 text-sm font-medium text-slate-700 focus:border-indigo-500 focus:ring-indigo-500 transition-colors">
                </div>
                <div class="md:col-span-4">
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Tanggal Selesai</label>
                    <input type="date" wire:model.live="filter_end_date" class="block w-full rounded-xl border-slate-200 bg-slate-50 py-2.5 px-3 text-sm font-medium text-slate-700 focus:border-indigo-500 focus:ring-indigo-500 transition-colors">
                </div>
                <div class="md:col-span-4 flex items-center justify-end gap-3">
                    <button type="button" wire:click="resetFilters" class="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-200 hover:bg-slate-50">Reset</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl">
            ...
            <div class="relative flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-3 text-3xl font-black text-slate-900 leading-none">{{ $submittedDates->count() }} <span class="text-sm font-semibold text-slate-500">Hari</span></p>
                </div>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-600 shadow-inner">
        ...
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl">
            ...
            <div class="relative flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-3 text-3xl font-black text-rose-600 leading-none">{{ $missingDates->count() }} <span class="text-sm font-semibold text-slate-500">Hari</span></p>
                </div>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-rose-100 text-rose-600 shadow-inner">
        ...
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl">
            ...
            <div class="relative flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-3 text-3xl font-black text-slate-900 leading-none">{{ $reports->count() }} <span class="text-sm font-semibold text-slate-500">Task</span></p>
                </div>
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-100 text-sky-600 shadow-inner">
        ...
        <div class="space-y-8 lg:col-span-8">
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/40">
                <div class="border-b border-slate-100 bg-slate-50/50 px-6 py-4 flex items-center justify-between">
                    ...
                    <span class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-600">{{ $reports->count() }} Entri</span>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead>
                            <tr class="bg-slate-50/30">
                                <th class="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500">Tanggal</th>
                                <th class="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500">Jam / Sesi</th>
                                <th class="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500">Aktivitas</th>
                                <th class="px-6 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500">Bukti</th>
                            </tr>
                        </thead>
                            ...
                            @forelse ($reports as $report)
                                <tr class="transition-colors hover:bg-slate-50/50 group">
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="text-sm font-bold text-slate-900">{{ \Carbon\Carbon::parse($report->work_date)->translatedFormat('d M Y') }}</div>
                                        <div class="text-[10px] font-medium text-slate-400">{{ \Carbon\Carbon::parse($report->work_date)->diffForHumans() }}</div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 ring-1 ring-inset ring-slate-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($report->proof_url)
                                            <a href="{{ $report->proof_url }}" target="_blank" class="inline-flex items-center gap-1.5 rounded-xl bg-indigo-50 px-3 py-2 text-xs font-black text-indigo-700 transition-all hover:bg-indigo-100">
                                        ...
                                        @else
                                            <span class="text-[10px] font-bold text-slate-300 uppercase italic">Tidak Ada</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-400 italic text-sm">Tidak ada data ditemukan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
