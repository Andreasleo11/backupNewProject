...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8" x-data="{ showFilters: false }">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto relative z-50">
                <div class="relative w-full sm:w-64 z-20">
                    ...
                    <input type="text"
                        ...
                        placeholder="Cari NIK / Nama..." wire:model.live.debounce.400ms="search">
                </div>
                ...
                <button type="button"
                    class="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white shadow-xl shadow-indigo-500/20 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all w-full sm:w-auto z-10"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div
            class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md group">
            <div
                class="absolute right-0 top-0 -mr-4 -mt-4 h-20 w-20 rounded-full bg-indigo-50 transition-transform group-hover:scale-110">
            </div>
            <div class="relative flex items-center justify-between">
                <div>
                    <h3 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Kepatuhan Tim Hari Ini
                    </h3>
                    <div class="mt-2 flex items-baseline gap-2">
                        <span class="text-3xl font-black text-slate-900">{{ $this->teamStats['rate'] }}%</span>
                        <div class="flex h-1.5 w-24 rounded-full bg-slate-100 overflow-hidden">
                            <div class="h-full bg-indigo-500 rounded-full"
                                style="width: {{ $this->teamStats['rate'] }}%"></div>
                        </div>
                    </div>
                </div>
        ...
        <div
            class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md group">
            <div
                class="absolute right-0 top-0 -mr-4 -mt-4 h-20 w-20 rounded-full bg-emerald-50 transition-transform group-hover:scale-110">
            </div>
            <div class="relative flex items-center justify-between">
                <div>
                    <h3 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Laporan Masuk</h3>
                    <div class="mt-2 text-3xl font-black text-slate-900">{{ $this->teamStats['submitted'] }} <span
                            class="text-sm font-semibold text-slate-500">Karyawan</span></div>
                </div>
        ...
        <div
            class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md group">
            <div
                class="absolute right-0 top-0 -mr-4 -mt-4 h-20 w-20 rounded-full bg-slate-50 transition-transform group-hover:scale-110">
            </div>
            <div class="relative flex items-center justify-between">
                <div>
                    <h3 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Karyawan</h3>
                    <div class="mt-2 text-3xl font-black text-slate-900">{{ $this->teamStats['total'] }} <span
                            class="text-sm font-semibold text-slate-500">Orang</span></div>
                </div>
    ...
    @if ($employees->isEmpty())
        <div
            class="mt-8 rounded-3xl border border-amber-200/60 bg-gradient-to-br from-amber-50 to-orange-50 px-6 py-8 text-center shadow-sm">
            <div
                class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-amber-100/50 text-amber-500 mb-4">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button type="button"
                class="mt-5 inline-flex items-center rounded-xl bg-white px-4 py-2 text-xs font-semibold text-amber-700 shadow-sm ring-1 ring-inset ring-amber-200 hover:bg-amber-50"
                wire:click="resetFilters">
                Bersihkan Filter
            </button>
        </div>
                            ...
                            <tr class="transition-colors hover:bg-slate-50/80 group">
                                ...
                                <td class="px-6 py-4">
                                    @if (!empty($employee->position))
                                        <span
                                            class="inline-flex items-center rounded-full border border-sky-100 bg-sky-50 px-3 py-1 text-xs font-bold text-sky-700">
                                            {{ $employee->position }}
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    @if ($latest)
                                        <div class="flex flex-col">
                                            <span class="text-sm font-bold text-slate-700 flex items-center gap-1.5">
                                                <div
                                                    class="h-2 w-2 rounded-full {{ $latest->isToday() ? 'bg-emerald-500' : 'bg-amber-400' }}">
                                                </div>
                                                {{ $latest->translatedFormat('d M Y') }} •
                                                {{ $latest->format('H:i') }}
                                            </span>
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-500 bg-rose-50 px-3 py-1.5 rounded-xl border border-rose-100/50">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('daily-reports.show', $employee->nik) }}" wire:navigate
                                            class="inline-flex items-center gap-1.5 rounded-xl bg-white px-4 py-2 text-xs font-black text-slate-700 shadow-sm ring-1 ring-inset ring-slate-200 transition-all hover:bg-slate-50 hover:shadow-md group/btn">
                                            <span>Lihat Detail</span>
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="h-3.5 w-3.5 text-slate-400 transition-transform group-hover/btn:translate-x-0.5"
