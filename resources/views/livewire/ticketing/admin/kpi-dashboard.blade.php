<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm font-medium text-slate-500 mt-1">Aggregated performance and support metrics across all IT staff.</p>
        </div>
        <div>
            <a href="{{ route('ticketing.list') ?? '#' }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
    ...
    @forelse($picMetrics as $metrics)
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden mb-6">
            <div class="bg-slate-50/80 px-6 py-4 border-b border-slate-100 flex items-center gap-4">
                <div class="h-10 w-10 rounded-full bg-slate-900 text-white flex items-center justify-center text-sm font-black uppercase tracking-wider">
                    {{ $initials }}
                </div>
                <div>
                    <h2 class="text-lg font-bold text-slate-900">{{ $pic->name ?? 'Unknown' }}</h2>
                    <div class="text-xs font-semibold text-slate-500">Period: {{ \Carbon\Carbon::parse($metrics['period']['start'])->format('M Y') }}</div>
                </div>
            </div>
            ...
            <div class="p-6 grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- SLA Compliance --}}
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-slate-50/50 border border-slate-100">
                        <div class="h-12 w-12 rounded-xl @if($metrics['sla_compliance'] >= 90) bg-emerald-50 text-emerald-600 @else bg-rose-50 text-rose-600 @endif flex items-center justify-center shrink-0">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        </div>
                        <div>
                            <div class="text-2xl font-black @if($metrics['sla_compliance'] >= 90) text-emerald-600 @else text-rose-600 @endif">
                                {{ $metrics['sla_compliance'] }}%
                            </div>
                            <div class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">SLA Targets</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-slate-50/50 border border-slate-100">
                        ...
                        <div>
                            <div class="text-2xl font-black @if($metrics['reopen_rate'] > 5) text-rose-600 @else text-slate-900 @endif">
                                {{ $metrics['reopen_rate'] }}%
                            </div>
                            <div class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Re-open Rate</div>
                        </div>
                    </div>
                ...
                new Chart(document.getElementById('{{ $chartId }}'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data: [{{ $metrics['throughput'] + 2 }}, {{ $metrics['throughput'] - 1 }}, {{ $metrics['throughput'] + 5 }}, {{ $metrics['throughput'] }}],
                            borderColor: '#4f46e5',
