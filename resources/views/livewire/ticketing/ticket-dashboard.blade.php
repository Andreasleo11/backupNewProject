    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm font-medium text-slate-500 mt-1">
                Auditing metrics for {{ auth()->user()->name }} ({{ \Carbon\Carbon::parse($metrics['period']['start'])->format('M Y') }})
            </p>
        </div>
        <div>
            <a href="{{ route('ticketing.list') ?? '#' }}" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 transition-colors">
            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <div class="mt-1 flex items-baseline gap-1">
                        <span class="text-3xl font-black @if($metrics['sla_compliance'] >= 90) text-emerald-600 @else text-rose-600 @endif">
                            {{ $metrics['sla_compliance'] }}%
                        </span>
                    </div>
                </div>
                <div class="p-2 @if($metrics['sla_compliance'] >= 90) bg-emerald-50 text-emerald-600 @else bg-rose-50 text-rose-600 @endif rounded-lg">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm relative overflow-hidden">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <div class="mt-1 text-3xl font-black @if($metrics['reopen_rate'] > 5) text-rose-600 @else text-slate-900 @endif">
                        {{ $metrics['reopen_rate'] }}%
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1 flex items-center gap-2">
                        ...
                        <div class="flex h-2.5 w-24 rounded-full bg-slate-100 overflow-hidden">
                            ...
                            <div class="h-full @if($utilPct > 80) bg-rose-500 @else bg-sky-500 @endif rounded-full" style="width: {{ $utilPct }}%"></div>
                        </div>
                    </div>
                </div>
        ...
        if (mainCtx) {
            new Chart(mainCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Metrics',
                        data: [{{ $metrics['sla_compliance'] }}, {{ $metrics['throughput'] }}, {{ $metrics['reopen_rate'] }}, {{ $metrics['utilization'] }}],
                        backgroundColor: [
                            'rgba(16, 185, 129, 0.2)',
                            'rgba(99, 102, 241, 0.2)',
                            'rgba(245, 158, 11, 0.2)',
