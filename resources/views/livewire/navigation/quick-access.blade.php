<div x-show="!sidebarCollapsed || {{ $isMobile ? 'true' : 'false' }}"
    ...
    <div class="space-y-1">
        @forelse ($items as $quickItem)
            <div class="group/qi relative"
                 wire:key="qa-{{ $quickItem['route'] }}"
                ...
                <a href="{{ route($quickItem['route']) }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all duration-300
                          ...
                          {{ $quickItem['active']
                             ...
                             : 'text-slate-600' }}">
                    <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-all duration-300
                                 {{ $quickItem['active']
                    ...
                    {{-- Filled star if pinned --}}
                    @if($quickItem['pinned'])
                        <svg class="h-3.5 w-3.5 text-amber-500 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/>
                        </svg>
                    @endif
                </a>

                {{-- Unpin button: only on pinned items, visible on hover --}}
                @if($quickItem['pinned'])
                    <button :disabled="loading"
                            @click.prevent="
                                loading = true;
                                $dispatch('nav-unpin', { routeName: '{{ $quickItem['route'] }}' });
                                setTimeout(() => loading = false, 500);
                            "
                            class="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover/qi:opacity-100
                                   transition-opacity duration-200 p-1 rounded-lg hover:bg-rose-50 text-amber-400 hover:text-rose-500"
                            title="Unpin">
