            ...
            <div class="group/qi relative"
                ...
                <a href="{{ route($quickItem['route']) }}"
                   class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all duration-300
                    ...
                    @if($quickItem['pinned'])
                        <svg class="h-3 w-3 text-amber-400 shrink-0 group-hover/qi:opacity-0 transition-opacity duration-150" viewBox="0 0 24 24" fill="currentColor">
                ...
                <button :disabled="loading"
                        @click.prevent="
                            ...
                            @else
                                $dispatch('nav-remove-visit', { routeName: '{{ $quickItem['route'] }}' });
                            @endif
