                            ...
                            <div>
                                ...
                                @if ($requirement && $department)
                                    <p class="text-[11px] font-semibold text-slate-500 mt-1">
                                        <span class="text-indigo-600">{{ $requirement->code }}</span> • {{ $department->name }}
                                    </p>
                                @endif
                            </div>
                            <button @click="open = false" class="text-slate-400 hover:text-rose-500 transition-colors shrink-0">
                                        ...
                                        <div class="p-5 hover:bg-slate-50/50 transition-colors group">
                                            <div class="flex items-start justify-between gap-4 mb-3">
                                                <div class="min-w-0 flex-1">
                                                    <p class="text-sm font-bold text-slate-800 truncate" title="{{ $u->original_name }}">{{ $u->original_name }}</p>
                                                    <p class="text-[11px] text-slate-500 mt-0.5">
                                                        {{ str_replace('application/', '', $u->mime_type) }} • {{ number_format($u->size / 1024, 1) }} KB
                                                    </p>
                                                </div>
                                                <span class="inline-flex shrink-0 items-center justify-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider {{ $badge }}">
                                                    {{ $u->status }}
                                                </span>
                                            </div>

                                            <div class="flex items-center gap-4 text-[10px] font-medium text-slate-400 mb-3">
                                                <div class="flex items-center gap-1">
                                                    <i class="bx bx-time text-slate-300"></i> {{ $u->created_at->format('d M y, H:i') }}
                                                </div>
                                                @if ($u->valid_until)
                                                    <div class="flex items-center gap-1 text-slate-500">
                                                        <i class="bx bx-calendar text-slate-300"></i> Valid ≤ {{ $u->valid_until->format('d M y') }}
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            @if ($u->review_notes)
                                                <div class="rounded bg-slate-100 border border-slate-200 p-2 mb-3">
                                                    <div class="flex items-start gap-1.5">
                                                        ...
                                                        <p class="text-xs text-slate-600 leading-tight">{{ $u->review_notes }}</p>
                                                    </div>
                                                </div>
                                            @endif

                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ $downloadUrl }}" class="inline-flex items-center justify-center h-7 px-3 rounded border border-slate-200 bg-white text-slate-600 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 text-[11px] font-bold transition-colors">
                                                    <i class="bx bx-download mr-1 text-sm"></i> Download
                                                </a>
                                                @if($previewable)
                                                    <a href="{{ $publicUrl }}" target="_blank" rel="noopener" class="inline-flex items-center justify-center h-7 px-3 rounded border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white text-[11px] font-bold transition-all">
