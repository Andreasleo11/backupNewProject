...
<div>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('requirements.edit', $req) }}" class="ml-1 text-sm font-medium text-slate-500 hover:text-indigo-600 md:ml-2 transition-colors">
                        {{ $req->code }}
                    </a>
                </div>
            </li>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2 mt-1.5">
                    <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100">
                        Min {{ $req->min_count }}
                    </span>
                    <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold bg-slate-100 text-slate-700 border border-slate-200">
                    ...
                    @endphp
                    <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $freqColor }} border">
                        {{ ucfirst($req->frequency) }}
                    </span>
                    @if ($req->requires_approval)
                        <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold bg-violet-50 text-violet-700 border border-violet-200">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        <div class="glass-card px-5 py-4">
            ...
            <p class="text-3xl font-bold text-slate-800 mt-1">{{ $summary['total'] }} <span class="text-sm font-normal text-slate-500">depts</span></p>
        </div>
    ...
    <div class="glass-card overflow-hidden">
        <div class="divide-y divide-slate-50">
            @forelse($items as $row)
                ...
                @endphp
                <div class="flex flex-wrap items-center justify-between gap-4 px-5 py-4 hover:bg-slate-50/50 transition-colors" wire:key="dept-row-{{ $row['dept']->id }}">
                    {{-- Department Info --}}
                    <div class="min-w-0 flex-1 flex items-center gap-4">
                        <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-100 to-blue-100 flex items-center justify-center shrink-0 font-bold text-indigo-600 text-sm">
                            {{ strtoupper(mb_substr($row['dept']->name, 0, 2)) }}
                        </div>
                        <div>
                            <div class="flex items-center gap-2 mb-0.5">
                                <p class="text-sm font-semibold text-slate-800">{{ $row['dept']->name }}</p>
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold {{ $sbg }} {{ $stxt }}">
                                    {{ $row['status'] }}
                                </span>
                            </div>
                    ...
                    <div class="flex flex-col items-end gap-1.5 shrink-0 w-48 hidden sm:flex">
                        <div class="flex items-center justify-between w-full">
                            ...
                            <span class="text-xs font-semibold text-slate-700">{{ $row['valid'] }} / {{ $row['min'] }}</span>
                        </div>
                        <div class="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div class="{{ $barColor }} h-full rounded-full transition-all" style="width: {{ $p }}%"></div>
                        </div>
                        @if (($row['pending'] ?? 0) > 0)
                            <span class="text-[10px] font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <button x-data x-on:click="$dispatch('open-upload', { requirementId: {{ $req->id }}, departmentId: {{ $row['dept']->id }} })"
