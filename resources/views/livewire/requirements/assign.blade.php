    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex-1 w-full lg:w-2/3 space-y-5">
            <div class="glass-card p-6">
                {{-- Top controls --}}
                <div class="flex flex-col md:flex-row gap-6 items-start md:items-end border-b border-slate-100 pb-6 mb-6">
                    <div class="flex-1 w-full">
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Requirement <span class="text-rose-500">*</span></label>
                ...
                <div class="mb-4 flex flex-wrap gap-3 items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200">
                            <button wire:click="selectAll" @disabled(!$requirement_id) class="px-3 py-1 text-xs font-semibold rounded-lg hover:bg-white hover:text-indigo-600 hover:shadow-sm disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:shadow-none disabled:cursor-not-allowed transition-all text-slate-600">All</button>
                            <button wire:click="selectNone" class="px-3 py-1 text-xs font-semibold rounded-lg hover:bg-white hover:text-indigo-600 hover:shadow-sm transition-all text-slate-600">None</button>
                            <button wire:click="selectAssigned" @disabled(!$requirement_id) class="px-3 py-1 text-xs font-semibold rounded-lg hover:bg-white hover:text-indigo-600 hover:shadow-sm disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:shadow-none disabled:cursor-not-allowed transition-all text-slate-600">Assigned</button>
                            <button wire:click="selectUnassigned" @disabled(!$requirement_id) class="px-3 py-1 text-xs font-semibold rounded-lg hover:bg-white hover:text-indigo-600 hover:shadow-sm disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:shadow-none disabled:cursor-not-allowed transition-all text-slate-600">Unassigned</button>
                        </div>
                        <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-100">
                ...
                <div class="border border-slate-200 rounded-xl overflow-y-auto max-h-[400px] bg-slate-50/50">
                    <div class="divide-y divide-slate-100">
                        @forelse($departments as $d)
                            <label class="flex items-center gap-3 p-3 hover:bg-white cursor-pointer transition-colors group">
                                ...
                                @if (in_array($d->id, $assignedDeptIds))
                                    <span class="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold bg-slate-200 text-slate-700 shrink-0">
                                        assigned
                                    </span>
                                @endif
                            </label>
                ...
                <div class="mt-6 flex flex-wrap items-center justify-end gap-3 pt-6 border-t border-slate-100">
                    ...
                    <button wire:click="selectNone" class="px-4 py-2 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-100 transition-colors">
                        Clear
                    </button>
            ...
            <div class="glass-card p-5">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        ...
                        <h2 class="text-base font-bold text-slate-800 mt-1 leading-tight">{{ $req?->name ?? 'None selected' }}</h2>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between py-2 border-b border-slate-50">
                            ...
                            @if ($req->requires_approval)
                                <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                    <i class="bx bx-shield-alt-2"></i> Required
                                </span>
                            @else
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                                    Not required
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                @forelse(($req->allowed_mimetypes ?? []) as $m)
                                    <span class="inline-flex rounded text-[10px] font-mono font-medium bg-slate-100 text-slate-600 px-1.5 py-0.5 border border-slate-200 truncate max-w-[150px]" title="{{ $m }}">
                ...
                @else
                    <div class="py-6 text-center border-t border-slate-100 mt-4">
                        ...
                        <p class="text-xs text-slate-400 mt-2">Select a requirement from the dropdown to see its specifications here.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="glass-card p-5">
                ...
                <div class="space-y-2 mb-3">
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="inline-flex items-center justify-center min-w-[24px] rounded px-1.5 py-0.5 text-xs font-bold bg-emerald-100 text-emerald-700">
                            {{ $willCreate }}
                        </span>
                    </div>
                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-600">Will update existing</span>
                        <span class="inline-flex items-center justify-center min-w-[24px] rounded px-1.5 py-0.5 text-xs font-bold bg-slate-100 text-slate-700">
                            {{ $willUpdate }}
                        </span>
                    </div>
                </div>
                <div class="rounded-lg bg-slate-50 border border-slate-100 p-3">
                    <p class="text-[11px] text-slate-500 leading-relaxed">
                        The assignments will be set as <strong class="{{ $is_mandatory ? 'text-indigo-600' : 'text-slate-700' }}">{{ $is_mandatory ? 'mandatory' : 'optional' }}</strong>.
                        @if($is_mandatory)
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] uppercase font-bold {{ $ra->is_mandatory ? 'text-indigo-500' : 'text-slate-400' }}">
                                            {{ $ra->is_mandatory ? 'Mandatory' : 'Optional' }}
                                        </p>
                                    </div>
                                </div>
                                <span class="text-[11px] text-slate-400 whitespace-nowrap">{{ $ra->created_at?->diffForHumans() }}</span>
                            </div>
                        @endforeach
                    </div>
                @elseif($req)
