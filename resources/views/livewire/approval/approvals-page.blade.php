<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8" x-data="{ search: @entangle('search') }">
    ...
    <div class="glass-card shadow-xl overflow-hidden relative pt-10 pb-8 px-8 border-none">
        <div
            class="absolute inset-0 bg-gradient-to-br from-indigo-600/10 via-purple-600/5 to-transparent pointer-events-none">
        </div>
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                <div
                    class="h-16 w-16 rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-700 flex items-center justify-center text-white shadow-2xl shadow-indigo-200">
                ...
                <div>
                    ...
                    <p class="text-slate-500 font-medium mt-1">Review and action all pending workflow requests across the
                        system.</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                <div
                    class="px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center gap-3 shadow-sm">
    ...
    {{-- Filter Bar --}}
    <div
        class="glass-card p-4 flex flex-col sm:flex-row items-center gap-4 bg-white/60 backdrop-blur-md shadow-sm border border-slate-100 rounded-3xl">
        <div class="relative w-full sm:w-80 group">
            ...
            <input type="text" wire:model.debounce.500ms="search" placeholder="Search ID, Ref, or Type..."
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-100">
                        <th class="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Request
                            Details</th>
                        <th class="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Department
                            / Branch</th>
                        <th class="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Submitted
                            At</th>
                        <th class="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Workflow
                            Status</th>
                        <th
                        ...
                        <tr class="group hover:bg-indigo-50/30 transition-colors">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors shadow-inner">
                                        <i class="bi bi-file-earmark-text text-xl"></i>
                                    </div>
                                    <div>
                                        <p class="text-sm font-black text-slate-800">
                                            {{ $approvable->getApprovableTypeLabel() }}</p>
                                        <p class="text-xs text-slate-400 font-bold mt-0.5 tracking-tight">Ref:
                                            {{ $approvable->getApprovableIdentifier() }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="space-y-1">
                                    <p class="text-sm font-bold text-slate-700">
                                        {{ $approvable->getApprovableDepartmentName() ?? 'General' }}</p>
                                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">
                                        {{ $approvable->getApprovableBranchValue() ?? 'HO' }}</p>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex flex-col items-start gap-2">
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-amber-50 text-amber-700 border border-amber-200 shadow-sm">
                                        Level {{ $approvalStep->sequence }} of {{ $request->steps_count ?? '?' }}
                                    </span>
                                    <p
                                        class="text-[10px] text-slate-500 font-bold italic tracking-wide group-hover:text-slate-700 transition-colors">
                                ...
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    @php
                                        $quickViewUrl = match (get_class($approvable)) {
                                            'App\Models\PurchaseRequest' => route(
                                                'purchase-requests.quick-view',
                                                $approvable->id,
                                            ),
                                            'App\Domain\Overtime\Models\OvertimeForm' => route(
                                                'overtime.detail',
                                                $approvable->id,
                                            ),
                                            default => null,
                                    ...
                                    @if ($quickViewUrl)
                                        <button
                                            ...
                                            title="Quick Preview">
                                            <i
                                                class="bi bi-eye-fill text-lg group-hover/view:scale-110 transition-transform"></i>
                                        </button>
                                    @endif
                                    ...
                                    <a href="{{ $approvable->getApprovableShowUrl() }}"
                                        ...
                                        <i
                                            class="bi bi-arrow-right group-hover/btn:translate-x-1 transition-transform"></i>
                                    </a>
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center justify-center max-w-xs mx-auto">
                                    <div
                                        class="h-20 w-20 rounded-full bg-slate-50 flex items-center justify-center text-slate-200 mb-6">
                                        <i class="bi bi-check-all text-5xl"></i>
                                    </div>
                                    <h4 class="text-xl font-bold text-slate-800">Inbox Clean!</h4>
                                    <p class="text-slate-400 text-sm mt-2">You don't have any pending approvals at the
                                        moment. Great job!</p>
                                    @if ($search || $filterType)
                                        <button wire:click="resetFilters"
                                            class="mt-6 text-indigo-600 font-bold text-sm hover:underline">Clear all
                                            filters</button>
                                    @endif
                                </div>
                            </td>
    ...
    {{-- QUICK VIEW MODAL --}}
    <div x-data="{ open: false }" @open-quick-view-modal.window="open = true"
        @close-quick-view-modal.window="open = false" x-show="open" class="fixed inset-0 z-[100] overflow-y-auto"
        style="display: none;">
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300"
                ...
                <div class="flex items-center justify-between border-b border-slate-100 px-8 py-6">
                    <div class="flex items-center gap-4">
                        <div
                            class="h-12 w-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center shadow-inner">
                            <i class="bi bi-eye text-2xl"></i>
                        </div>
                        <div>
                            <h3 class="text-xl font-black text-slate-900">Quick Preview</h3>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Reference ID:
                                {{ $selectedId }}</p>
                        </div>
                    </div>
                    <button @click="open = false; $wire.closeQuickView()"
                        class="h-10 w-10 flex items-center justify-center rounded-xl hover:bg-slate-50 text-slate-400 transition-colors">
                    ...
                    @if ($selectedId && $selectedType)
                        ...
                        @else
                            <div class="flex flex-col items-center justify-center h-full py-20 text-center">
                                <div
                                    class="h-16 w-16 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center mb-4">
                                    <i class="bi bi-exclamation-triangle text-3xl"></i>
                                </div>
                                <h4 class="text-lg font-bold text-slate-800">No Preview Available</h4>
                                <p class="text-slate-400 max-w-xs mx-auto mt-2 text-sm italic">Detailed preview for
                                    this module is still in development. Please use the full review page.</p>
                            </div>
                        @endif
                    @else
                ...
                <div
                    class="px-8 py-6 bg-white rounded-b-[2rem] flex items-center justify-between border-t border-slate-100">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic">Best practice:
                        Always verify line items before signing off.</p>
                    <div class="flex items-center gap-3">
                        <button @click="open = false; $wire.closeQuickView()"
                            class="px-6 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition-all">Close</button>
                        <a href="{{ $selectedId ? '#' : '' }}"
                            class="px-6 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">Open
                            Full Detail</a>
                    </div>
                </div>
            </div>
