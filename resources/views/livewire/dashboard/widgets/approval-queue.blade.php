<div
    class="h-full flex flex-col rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden group hover:shadow-xl hover:border-blue-100 transition-all duration-500">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
        @forelse ($approvals as $step)
            ...
            <div
                class="relative mb-4 last:mb-0 p-4 rounded-2xl border border-slate-100 bg-white transition-all hover:bg-slate-50/50 hover:scale-[1.01] hover:shadow-md group/item overflow-hidden">
                <div
                    class="absolute inset-0 bg-gradient-to-r {{ $bgGradient }} opacity-0 group-hover/item:opacity-100 transition-opacity">
                </div>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex flex-col">
                            ...
                            <span class="text-[11px] text-slate-500 mt-0.5">
                                Submitted {{ $request->submitted_at?->diffForHumans() ?? 'recently' }} by <span
                                    class="font-bold text-slate-700">{{ $request->submittingUser?->name ?? 'User' }}</span>
                            </span>
                        </div>
                    </div>
    ...
    <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 mt-auto">
        <a href="{{ route('approvals') }}"
            class="text-[11px] font-black uppercase tracking-widest text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-2 group/all">
            View All Requests
                ...
                <div x-show="open" x-transition:enter="ease-out duration-300"
                        ...
                        @if ($selectedId && $selectedType)
                            ...
                            @else
                                <div class="flex flex-col items-center justify-center h-full py-20 text-center">
                                    ...
                                    <p class="text-slate-400 max-w-xs mx-auto mt-2 text-sm italic">Detailed preview for this
                                        module is still in development. Please use the full review page.</p>
                                </div>
                            @endif
                        @else
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ $selectedId ? '#' : '' }}"
                                class="px-6 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all">Open
                                Full Detail</a>
                        </div>
                    </div>
                </div>
