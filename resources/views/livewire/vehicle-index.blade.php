    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3">
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col h-full">
                ...
                {{-- Toolbar --}}
                <div class="border-b border-slate-100 bg-slate-50/50 px-5 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="relative w-full sm:max-w-xs group">
                        ...
                        <div wire:loading wire:target="search" class="absolute inset-y-0 right-0 flex items-center pr-3">
                            <div class="h-4 w-4 rounded-full border-2 border-slate-200 border-t-indigo-500 animate-spin"></div>
                        </div>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="bg-white border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($vehicles as $v)
                                <tr class="hover:bg-slate-50/60 transition-colors group">
                                    <td class="px-5 py-3">
                                        <div class="flex items-center gap-3">
                                            <div class="h-9 w-9 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 ring-1 ring-inset ring-indigo-100">
                                            ...
                                            <div>
                                                <p class="font-bold text-slate-900 tracking-tight">{{ $v->plate_number }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-5 py-3">
                                        <div class="flex items-center gap-2">
                                            <div class="h-6 w-6 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0">
                                                <span class="text-[10px] font-bold text-slate-600">{{ $v->driver_name ? substr($v->driver_name, 0, 1) : '-' }}</span>
                                            </div>
                                            <span class="font-medium text-slate-700">{{ $v->driver_name ?: 'Unassigned' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-5 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ route('vehicles.edit', $v->id) }}"
                                                class="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 transition" title="Edit Vehicle">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                            </a>
                                            <button wire:click="delete({{ $v->id }})" wire:confirm="Are you sure you want to delete this vehicle?"
                                                class="rounded-md p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 transition" title="Delete Vehicle">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-5 py-12 text-center">
                                        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 mb-3">
                                            <svg class="h-8 w-8 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                                        </div>
                                        <h3 class="text-sm font-semibold text-slate-900">No vehicles found</h3>
                                        <p class="mt-1 text-sm text-slate-500 max-w-sm mx-auto">Get started by registering a new vehicle to the fleet management system.</p>
                                        @if($search)
                                            <button wire:click="$set('search', '')" class="mt-4 text-sm font-semibold text-indigo-600 hover:text-indigo-500">Clear search filter</button>
                                        @else
                                            <a href="{{ route('vehicles.create') }}" class="mt-4 inline-block text-sm font-semibold text-indigo-600 hover:text-indigo-500">Register First Vehicle</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm text-center relative overflow-hidden">
                <div class="absolute -right-6 -top-6 w-24 h-24 bg-indigo-50 rounded-full"></div>
                
                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 relative z-10">Total Active Fleet</h3>
                <p class="mt-2 text-4xl font-black text-slate-800 tracking-tighter relative z-10">{{ $vehicles->count() }}</p>
            ...
            <div class="rounded-xl bg-gradient-to-b from-slate-800 to-slate-900 p-5 shadow-lg relative overflow-hidden text-white">
                <div class="absolute -right-4 top-4 text-white/5">
                    <svg class="w-24 h-24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 22h20L12 2zm0 4.5l6.5 13h-13L12 6.5z"/></svg>
                </div>
                ...
                <p class="mt-2 text-xs text-slate-300 leading-relaxed font-medium relative z-10">
                    Ensure all vehicle plates are accurate to prevent issues when assigning fleet vehicles to new Delivery Notes in the Operations domain.
                </p>
            </div>
        </div>
