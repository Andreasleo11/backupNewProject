...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8">
    ...
    <div class="relative z-0 grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="relative">
                ...
                <h3 class="mt-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">Selected Period</h3>
                <p class="mt-2 text-2xl font-black text-slate-900 leading-none truncate" title="{{ $monthLabel }}">{{ $monthLabel }}</p>
            </div>
        </div>
        ...
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="relative">
                ...
                <h3 class="mt-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">Cumulative Total</h3>
                <p class="mt-2 text-2xl font-black text-slate-900 leading-none truncate" title="Rp {{ number_format($grandTotal, 0, ',', '.') }}">
        ...
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
            ...
            <div class="relative">
                ...
                <h3 class="mt-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">Departments Count</h3>
                <p class="mt-2 text-2xl font-black text-slate-900 leading-none truncate">{{ $this->totals()->count() }} active</p>
            </div>
        </div>

        {{-- Selected Status --}}
        <div class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
            <div class="absolute right-0 top-0 -mr-4 -mt-4 h-24 w-24 rounded-full bg-amber-50 transition-all group-hover:scale-150"></div>
            <div class="relative">
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl {{ $deptSelected || $prSigner ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400' }}">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                    </svg>
                </div>
                <h3 class="mt-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none">Data Filter Context</h3>
                @if ($deptSelected)
                    <p class="mt-2 text-2xl font-black text-slate-900 leading-none truncate" title="{{ $deptSelected }}">{{ $deptSelected }}</p>
                @elseif ($prSigner)
                    <p class="mt-2 text-2xl font-black text-slate-900 leading-none truncate" title="{{ $prSigner }}">{{ $prSigner }}</p>
                @else
    ...
    >
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Aggregate comparison of {{ \Illuminate\Support\Carbon::parse($month . '-01')->isoFormat('MMMM YYYY') }}.</p>
            </div>
            ...
            <div class="relative h-[380px]">
                ...
                <div
                    class="pointer-events-none absolute inset-0 bg-white/60 backdrop-blur-sm flex items-center justify-center transition-opacity duration-300 opacity-0"
                    wire:loading.class="opacity-100"
                    wire:target="month,prSigner,showDetail,clearDetail"
                >
                    <div class="flex items-end gap-2 h-32 opacity-50">
                        <div class="w-8 bg-indigo-200 animate-pulse rounded-t-sm h-12"></div>
                        <div class="w-8 bg-indigo-200 animate-pulse rounded-t-sm h-24" style="animation-delay: 150ms"></div>
                        <div class="w-8 bg-indigo-200 animate-pulse rounded-t-sm h-32" style="animation-delay: 300ms"></div>
                        <div class="w-8 bg-indigo-200 animate-pulse rounded-t-sm h-16" style="animation-delay: 450ms"></div>
                        <div class="w-8 bg-indigo-200 animate-pulse rounded-t-sm h-20" style="animation-delay: 600ms"></div>
                    </div>
                </div>
            </div>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        ...
        <div class="border-b border-slate-100 bg-slate-50/50 p-6">
            <div class="inline-flex items-center space-x-2 p-1 bg-slate-200/50 backdrop-blur-sm rounded-xl border border-slate-200/50">
                <button
                    type="button"
                ...
                >
                    ...
                    <span class="flex items-center gap-2">
                        ...
                        @if ($activeTab === 'detail' && $deptSelected)
                            <span class="hidden sm:inline-flex bg-indigo-100 text-indigo-800 text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-sm">{{ $deptSelected }}</span>
                        @endif
                    </span>
                </button>
        ...
        <div class="p-6 relative z-20 min-h-[400px]">
            ...
            <div wire:loading.delay wire:target="activeTab" class="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-b-2xl">
                ...
                <div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-left text-sm text-slate-700">
                            <thead>
                                <tr class="border-b border-slate-200 text-xs font-semibold uppercase tracking-wider text-slate-500 bg-slate-50/50">
            ...
            @if ($activeTab === 'compare')
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8" x-data="compareChart()" x-init="mount()">
                        <div>
                            <h3 class="text-xl font-bold tracking-tight text-slate-900">Compare Analytics</h3>
                            <p class="text-sm font-medium text-slate-500 mt-1">Cross-reference spending across timeframes.</p>
                        </div>

                        <div class="flex flex-wrap items-center gap-4">
                            {{-- Mode toggle --}}
                            <div class="inline-flex items-center space-x-1 p-1 bg-slate-100 rounded-xl border border-slate-200">
                                <label class="cursor-pointer">
                                    <input type="radio" class="sr-only" value="range" wire:model.live="compareMode">
                                    <div class="px-3 py-1.5 rounded-lg text-sm font-bold transition-all {{ $compareMode === 'range' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                                        <i class="bi bi-calendar-range mr-1"></i> Range
                                    </div>
                                </label>
                                <label class="cursor-pointer">
                                    <input type="radio" class="sr-only" value="rolling" wire:model.live="compareMode">
                                    <div class="px-3 py-1.5 rounded-lg text-sm font-bold transition-all {{ $compareMode === 'rolling' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                            ...
                            @if ($compareMode === 'range')
                                <div class="flex flex-wrap items-center gap-3 bg-slate-50 p-1.5 rounded-xl border border-slate-200/60 relative z-50">
                            ...
                            <div class="hidden sm:flex items-center gap-4 bg-slate-50 p-2 rounded-xl border border-slate-200/60 ml-auto">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="checkbox" class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600 transition-shadow" x-model="stacked" @change="update()">
                                    <span class="text-sm font-semibold text-slate-600 group-hover:text-slate-900 transition-colors">Stacked</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="checkbox" class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600 transition-shadow" x-model="normalized" @change="update()">
                                    <span class="text-sm font-semibold text-slate-600 group-hover:text-slate-900 transition-colors">Normalize (%)</span>
                                </label>
                            </div>
                        </div>

                    {{-- Compare chart --}}
                    <div class="relative mt-8 h-[450px] w-full rounded-2xl bg-slate-50 border border-slate-100 p-4">
                        <div wire:ignore class="h-full w-full">
                            <canvas x-ref="canvas"></canvas>
                        </div>
                        <div
                            class="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-white/60 backdrop-blur-sm transition-opacity duration-300"
                            wire:loading
                            wire:target="compareMode,startMonth,endMonth,rollingN,prSigner"
                        >
                            <div class="flex items-center gap-3 rounded-full bg-white px-5 py-2.5 text-sm font-bold text-indigo-600 shadow-lg ring-1 ring-slate-100">
                            ...
                            scales: {
                                y: {
                                    ...
                                    ticks: {
                                        ...
                                        callback: (v) => 'Rp ' + (v >= 1000000 ? (v/1000000).toFixed(0) + 'M' : fmt.format(v))
                                    }
                                },
                                x: {
                                    grid: { display: false },
                                    ...
                                    ticks: {
                                        ...
                                        font: { size: 11, font: 'Inter, sans-serif' },
                                        maxRotation: 45,
                                        minRotation: 0
                                    }
                                }
        ...
        function compareChart() {
            return {
                ...
                color(ctx, datasetIdx) {
                    ...
                    if (!ctx) return { bg: colorSet[0], hoverBg: colorSet[1] };
                        ...
                        options: {
                            ...
                            scales: {
                                x: {
                                    ...
                                    ticks: {
                                        color: '#475569',
                                        font: { size: 11, family: 'Inter, sans-serif' }, // Changed 'font' to 'family'
                                        maxRotation: 45,
                                        minRotation: 0
                                    }
                                },
                                y: {
                                    ...
                                    ticks: {
                                        color: '#64748b',
                                        font: { size: 11, family: 'Inter, sans-serif' },
                                        callback: (v) => this.normalized ? v + '%' : 'Rp ' + (v >= 1000000 ? (v/1000000).toFixed(0) + 'M' : fmt.format(v))
                                    }
                                }
                            },
