...
<div x-data="{ decisionPanelOpen: false }" @open-decision-modal.window="decisionPanelOpen = true"
        ...
        <div class="flex flex-wrap justify-between items-center gap-3 pt-3 border-t border-slate-100">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <div class="flex items-center ml-2 bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                    <button wire:click="setRange('7d')"
                        class="px-2.5 py-1 text-xs font-medium rounded-md hover:bg-white hover:text-indigo-600 transition-colors {{ request('range') == '7d' ? 'bg-white shadow text-indigo-700' : 'text-slate-600' }}">7d</button>
                    <button wire:click="setRange('30d')"
                        class="px-2.5 py-1 text-xs font-medium rounded-md hover:bg-white hover:text-indigo-600 transition-colors {{ request('range') == '30d' ? 'bg-white shadow text-indigo-700' : 'text-slate-600' }}">30d</button>
                    <button wire:click="setRange('month')"
                        class="px-2.5 py-1 text-xs font-medium rounded-md hover:bg-white hover:text-indigo-600 transition-colors {{ request('range') == 'month' ? 'bg-white shadow text-indigo-700' : 'text-slate-600' }}">Month</button>
                </div>
            ...
            <label class="flex items-center gap-2 cursor-pointer select-none">
                <div class="relative">
                    ...
                    <div
                        class="absolute top-0.5 left-0.5 h-3 w-3 rounded-full bg-white shadow transition-transform peer-checked:translate-x-4">
                    </div>
                </div>
    ...
    <div class="glass-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        class="bg-slate-50/80 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                        ...
                        <th class="px-4 py-3 cursor-pointer hover:bg-slate-100 select-none group"
                            wire:click="sortBy('requirements.name')">
                            Requirement <span class="group-hover:text-indigo-500">{!! $this->sortIcon('requirements.name') !!}</span>
                        </th>
                        <th class="px-4 py-3 cursor-pointer hover:bg-slate-100 select-none group"
                            wire:click="sortBy('departments.name')">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rows as $u)
                        ...
                        <tr wire:key="upload-{{ $u->id }}"
                            class="hover:bg-slate-50/50 transition-colors {{ in_array($u->id, $selected) ? 'bg-indigo-50/30' : '' }}">
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <p class="text-sm font-medium text-slate-700 truncate max-w-[180px]"
                                    title="{{ $u->original_name }}">{{ $u->original_name }}</p>
                                <p class="text-[11px] text-slate-400">
                                    {{ Str::limit(str_replace('application/', '', $u->mime_type), 15) }} ·
                                    {{ number_format($u->size / 1024, 1) }} KB</p>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1.5">
                                    <span
                                        class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold {{ $badge }}">
                                        {{ ucfirst($u->status) }}
                                    </span>
                                    @if ($u->review_notes)
                            ...
                            <td class="px-4 py-3">
                                <p class="text-xs text-slate-600">{{ $u->valid_from?->format('d M y') ?? '—' }} →
                                    {{ $u->valid_until?->format('d M y') ?? '—' }}</p>
                                @if (!is_null($daysLeft))
                                    @php
                                        $expColor =
                                            $daysLeft < 0
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex mt-0.5 rounded px-1.5 py-0.5 text-[10px] font-semibold {{ $expColor }}">
    ...
    @if (count($selected) > 0)
        <div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 animate-slide-up">
            <div
                class="glass-card premium-shadow px-4 py-3 rounded-2xl flex items-center gap-4 border border-slate-200/60 bg-white/90 backdrop-blur-md">
                <div class="flex items-center gap-2 border-r border-slate-200 pr-4">
                    <span
                        class="flex items-center justify-center w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold">{{ count($selected) }}</span>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="clearSelection"
                        class="ml-2 text-xs font-medium text-slate-500 hover:text-slate-700">Cancel</button>
                </div>
            </div>
        </div>
    @endif
    ...
    <template x-teleport="body">
        <div x-show="decisionPanelOpen" class="relative z-[100]" x-cloak>
            {{-- Backdrop --}}
            <div x-show="decisionPanelOpen" x-transition:enter="ease-in-out duration-300"
                                        ...
                                        <div class="space-y-6">
                                            ...
                                            <div class="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100/50">
                                                <p
                                                    ...
                                                    Requirement</p>
                                                <p class="text-sm font-bold text-slate-800">{{ $active['req_name'] }}
                                                    <span
                                                        class="font-normal text-slate-500">({{ $active['req_code'] }})</span>
                                                </p>
                                                ...
                                                <div class="mt-3">
                                                    <p
                                                        ...
                                                        Validity Claimed</p>
                                                    <p class="text-sm font-medium text-slate-800">
                                                        {{ $active['valid_from'] ?? '—' }} <span
                                                            class="text-slate-400 mx-1">→</span>
                                                        {{ $active['valid_until'] ?? '—' }}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <p
                                                    ...
                                                    Attached File</p>
                                                <div
                                                    class="flex items-start justify-between gap-4 p-3 rounded-xl border border-slate-200">
                                                    <div class="min-w-0 flex-1">
                                                        <p class="text-sm font-semibold text-slate-800 truncate"
                                                            title="{{ $active['original_name'] }}">
                                                            {{ $active['original_name'] }}</p>
                                                        <p class="text-xs text-slate-500 mt-0.5">
                                                            {{ Str::limit($active['mime_type'], 20) }} ·
                                                            {{ number_format($active['size'] / 1024, 1) }} KB</p>
                                                    </div>
                                                ...
                                                @if (Str::startsWith($active['mime_type'], 'image/'))
                                                    <div
                                                        class="mt-3 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center min-h-[150px]">
                                                        <img src="{{ $active['preview_url'] }}"
                                                            class="max-w-full h-auto object-contain" alt="preview">
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Review
    ...
    <div wire:loading.delay
        class="fixed inset-0 z-[100] bg-white/40 backdrop-blur-[2px] flex items-center justify-center">
