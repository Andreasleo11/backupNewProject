<div class="max-w-7xl mx-auto space-y-8 py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div class="flex gap-4">
                <a href="{{ route('admin.users.index') }}"
                    class="inline-flex items-center rounded-xl bg-white/10 px-4 py-2.5 text-sm font-semibold text-white backdrop-blur-md transition-all hover:bg-white/20">
                    Manage Users
                </a>
                <a href="{{ route('admin.roles.index') }}"
                    class="inline-flex items-center rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/25 transition-all hover:bg-blue-500 hover:scale-105">
                    Manage Roles
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div
            class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-xl hover:-translate-y-1">
            <div
                class="absolute right-0 top-0 -mr-4 -mt-4 h-24 w-24 rounded-full bg-violet-50 transition-all group-hover:scale-150">
            </div>
            <div class="relative">
                ...
                @foreach ($roleDistribution as $role)
                    <div class="relative pt-1">
                        <div class="flex items-center justify-between mb-2">
                            <div>
                                <span
                                    class="text-xs font-bold inline-block py-1 px-2 uppercase rounded-full text-blue-600 bg-blue-100">
                                    {{ $role->name }}
                                </span>
                            </div>
            ...
            <div class="relative">
                ...
                @if ($userSearch)
                    <button wire:click="clearSearch"
                        class="absolute right-4 top-4 text-slate-400 hover:text-rose-500">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div class="rounded-2xl bg-blue-50/50 p-6 border border-blue-100">
                        <div class="flex items-center gap-4">
                            ...
                            <span
                                class="ml-auto px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider {{ $selectedUser['active'] ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                        ...
                        <div class="mt-6 grid grid-cols-2 gap-4">
                            <div>
                                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">
                                    Assigned Roles</h4>
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($userRoles as $role)
                                        <span
                                            class="px-2 py-1 rounded-md bg-white border border-blue-100 text-[11px] font-semibold text-blue-700 shadow-sm">
                                            {{ $role }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <div>
                                <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Total
                                    Permissions</h4>
                        ...
                        <div class="mt-6">
                            <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Permission
                                Preview</h4>
                            <div class="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto custom-scrollbar p-1">
                                @forelse(array_slice($userPermissions, 0, 15) as $perm)
                                    <span
                                        class="px-2 py-0.5 rounded bg-slate-100 text-[10px] text-slate-600 font-medium">
                                        {{ $perm }}
                                    </span>
                                @empty
                                ...
                                @if (count($userPermissions) > 15)
                                    <span
                                        class="text-[10px] text-slate-400 font-bold italic">+{{ count($userPermissions) - 15 }}
                                        more</span>
                                @endif
                            </div>
                        </div>
