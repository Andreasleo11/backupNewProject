    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex space-x-2 bg-slate-100 p-1 rounded-lg">
            <button wire:click="$set('activeTab', 'compare')" 
                class="px-4 py-2 text-sm font-medium rounded-md transition-all {{ $activeTab === 'compare' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">
                Sync Preview
            </button>
            <button wire:click="$set('activeTab', 'history')" 
                class="px-4 py-2 text-sm font-medium rounded-md transition-all {{ $activeTab === 'history' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">
                History
            </button>
        </div>
    </div>
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                <div class="flex items-center">
                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-50 text-blue-600 mr-4">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                    </div>
                    <div>
                        <h3 class="font-bold text-slate-900 text-sm">Pending Changes</h3>
                ...
                <button 
                    ...
                    class="inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                @if(!empty($unmanagedRoles))
                    <div class="mt-8">
                        <div class="flex items-center mb-4">
                            <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Unmanaged Roles (Database Only)</h4>
                            <div class="ml-3 flex-grow border-t border-slate-200"></div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach($unmanagedRoles as $role => $perms)
                                <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl hover:bg-white hover:shadow-sm transition-all">
                                    <div class="flex items-center justify-between mb-2">
                                        <h5 class="font-bold text-slate-900 text-sm">{{ $role }}</h5>
                                        <svg class="w-4 h-4 text-slate-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-[10px] text-slate-500 mb-3">These roles exist in the database but are not managed by the PermissionRegistry. They will not be affected by the sync.</p>
                                    <div class="flex flex-wrap gap-1">
                                        <span class="px-2 py-0.5 bg-slate-200 text-slate-700 font-medium rounded text-[10px] border border-slate-300">{{ count($perms) }} Permissions</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
