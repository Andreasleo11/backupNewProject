            ...
            @forelse ($rules as $rule)
                <button wire:click="selectRule({{ $rule->id }})"
                    ...
                    <div class="flex justify-between items-start mb-1">
                        <span class="font-bold text-sm {{ $selectedRuleId === $rule->id ? 'text-indigo-700' : 'text-slate-700 group-hover:text-indigo-600' }}">
                            {{ $rule->code }}
                        </span>
                        @if ($rule->active)
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity {{ $selectedRuleId === $rule->id ? 'opacity-100' : '' }}">
                            <div wire:click.stop="openEditRule({{ $rule->id }})" class="p-1 text-slate-400 hover:text-indigo-600 cursor-pointer">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </div>
                            <div wire:click.stop="deleteRule({{ $rule->id }})" class="p-1 text-slate-400 hover:text-rose-600 cursor-pointer">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden bg-slate-50/50 relative">
            ...
            <div class="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6 shadow-sm z-10">
                <div class="flex items-center gap-4">
                    <h2 class="text-lg font-bold text-slate-900">
                        {{ $selectedRule->name }}
                        <span class="ml-2 text-xs font-normal text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">
                             Priority: {{ $selectedRule->priority }}
                        </span>
                    </h2>
                </div>
                <div class="flex items-center gap-3">
                     <button wire:click="openEditRule({{ $selectedRule->id }})" class="text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors">
                        Edit Settings
                    </button>
                    <div class="h-4 w-px bg-slate-300"></div>
                    <button wire:click="openCreateStep" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-indigo-500/20 transition-all hover:bg-indigo-500 hover:scale-105 active:scale-95">
                ...
                <div class="max-w-3xl mx-auto space-y-12">
                    @forelse ($steps as $index => $step)
                        <div class="relative group">
                             ...
                             @if($index > 0)
                                 <div class="absolute left-1/2 -top-12 h-12 w-0.5 bg-slate-300 transform -translate-x-1/2 z-0"></div>
                             @endif

                            <div class="relative z-10 bg-white border border-slate-200 rounded-2xl p-1 shadow-sm transition-all hover:shadow-lg hover:border-indigo-200 hover:-translate-y-1">
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div>
                                                ...
                                                <div class="text-lg font-bold text-slate-900">
                                                    {{ $step->approver_type === 'role' ? 'Role ID: ' : 'User ID: ' }} {{ $step->approver_id }}
                                                </div>
                                            </div>
                            ...
                            @if (!$loop->last)
                                <div class="absolute left-1/2 -bottom-12 h-12 w-0.5 bg-slate-300 transform -translate-x-1/2"></div>
                            @elseif($step->final)
                                 <div class="absolute left-1/2 -bottom-12 h-12 w-0.5 bg-emerald-400 transform -translate-x-1/2"></div>
                                 <div class="absolute left-1/2 -bottom-20 transform -translate-x-1/2">
                                     <span class="inline-flex items-center rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800 ring-4 ring-white shadow-lg">
                                         APPROVED
                                     </span>
                                 </div>
                            @endif
                    ...
                    @empty
                         <div class="text-center py-12 rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50">
                            ...
                            <button wire:click="openCreateStep" class="mt-4 text-xs font-bold text-indigo-600 hover:text-indigo-700 hover:underline">
        ...
        @else
            <div class="flex-1 flex flex-col items-center justify-center p-8 text-center text-slate-500">
                ...
                <p class="mt-2 text-sm max-w-sm">Select a rule from the sidebar to visualize and edit its approval workflow.</p>
            </div>
        @endif
    </div>
             ...
             <form wire:submit.prevent="saveRule" class="space-y-4">
                 ...
                 <div class="relative">
                    ...
                    <label for="rule_model_type"
                     ...
                     @error('rule_model_type') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                     ...
                     <div class="relative">
                        ...
                        @error('rule_code') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>
                     ...
                     <div class="relative">
                        ...
                        @error('rule_priority') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>
                </div>
                 ...
                 <div class="relative">
                    ...
                    @error('rule_name') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <textarea wire:model.defer="rule_match_expr_raw" rows="5"
                     ...
                     @error('rule_match_expr_raw') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>
