<div class="p-6 bg-gray-50 min-h-screen">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Asset Categories</h1>
            <p class="text-gray-600">Manage asset categories. <span class="text-sm text-gray-500">({{ $categories->total() ?? $categories->count() }})</span></p>
        </div>
                        ...
                        @foreach($categories as $category)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm font-medium">
                                    ...
                                    <button wire:click="delete({{ $category->id }})" class="text-red-600 hover:text-red-900" onclick="confirm('Delete this category? This may affect {{ $category->assets_count }} asset(s).') || event.stopImmediatePropagation()">Delete</button>
                                </td>
                            </tr>
                        @endforeach
