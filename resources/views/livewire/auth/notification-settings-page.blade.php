        ...
        <div class="glass-card p-6 shadow-sm border border-slate-200/60 overflow-hidden relative">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-medium">This serves as the default for all modules unless
                        overridden below.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <button type="button" wire:click="$set('global_mode', 'both')"
                    ...
                    <div class="text-center">
                        <span class="block text-sm font-bold text-slate-900 uppercase tracking-tight">Both <span
                                class="text-[0.6rem] ml-1 bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded uppercase tracking-wider">Default</span></span>
                        <span class="text-[10px] text-slate-500 leading-tight block mt-1">Immediate emails + morning
                            digests</span>
                    </div>
                    @if ($global_mode === 'both')
                ...
                <button type="button" wire:click="$set('global_mode', 'immediate')"
                    ...
                    <div class="text-center">
                        ...
                        <span class="text-[10px] text-slate-500 leading-tight block mt-1">Receive emails as soon as
                ...
                <button type="button" wire:click="$set('global_mode', 'daily_summary')"
                    ...
                    <div class="text-center">
                        <span class="block text-sm font-bold text-slate-900 uppercase tracking-tight">Daily
                            Summary</span>
                        <span class="text-[10px] text-slate-500 leading-tight block mt-1">Consolidated morning digest of
                            all pending tasks</span>
                    </div>
                ...
                {{-- 4. NONE --}}
                <button type="button" wire:click="$set('global_mode', 'none')"
                    ...
                    <div class="text-center">
                        ...
                        <span class="text-[10px] text-slate-500 leading-tight block mt-1">Only view notifications via
                            dashboard bell icon</span>
                    </div>
                    @if ($global_mode === 'none')
            ...
            <div class="glass-card shadow-sm border border-slate-200/60 overflow-hidden">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-slate-500 font-bold uppercase tracking-wider mt-0.5">
                                    Customize per business area</p>
                            </div>
                        </div>
                        <span
                            class="px-3 py-1 bg-slate-100 text-[10px] font-bold text-slate-500 rounded-full uppercase tracking-widest border border-slate-200">Dynamic
                            List</span>
                    </div>
                </div>
                ...
                <div class="divide-y divide-slate-100">
                    @foreach ($available_modules as $class => $label)
                        <div
                            class="p-6 transition-colors hover:bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] font-medium text-slate-400 break-all opacity-60">
                                        {{ $class }}</p>
                                </div>
                            </div>
