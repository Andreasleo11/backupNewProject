<div>
    <div x-data="{ open: false }" class="position-relative" x-cloak>
        ...
        <input type="text" class="form-control {{ $hasError ? 'is-invalid' : '' }}" placeholder="{{ $placeholder }}"
            wire:model.live="search" @keydown="open = true" @click.outside="open = false" value="{{ $value }}" />
