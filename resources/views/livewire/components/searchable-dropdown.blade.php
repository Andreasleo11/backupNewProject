<div>
    <div x-data="{ open: false }" class="position-relative" x-cloak>
        ...
        <input type="text"
            class="form-control @if ($hasError) is-invalid @endif @if ($isSaved) is-valid @endif"
            placeholder="{{ $placeholder }}" wire:model.live="search" @keydown="open = true" @click.outside="open = false"
            value="{{ $value }}" />
        ...
        <ul class="list-group position-absolute w-100 shadow"
            ...
            @foreach ($results as $item)
                <li class="list-group-item list-group-item-action" wire:click="select('{{ addslashes($item) }}')"
                    @click="open = false">
                    {{ $item }}
                </li>
            @endforeach
