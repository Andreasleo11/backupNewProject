<div x-data="focusMode()" x-show="isOpen" @open-focus-mode.window="openMode($event.detail)"
    @focus-mode-finished.window="closeMode()" style="display: none;"
    ...
    <div class="w-full flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden relative" id="focusModeScroll">
            ...
            <div class="max-w-6xl mx-auto w-full p-4 lg:p-8 pb-32">
                <div class="flex flex-col lg:flex-row gap-6 lg:gap-10 items-start">
                    ...
                    <div class="w-full lg:w-1/3 lg:sticky lg:top-8 shrink-0">
                        <div
                            class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col items-center text-center relative overflow-hidden">
                            <div class="absolute inset-x-0 top-0 h-2 bg-gradient-to-r from-indigo-500 to-purple-500">
                            </div>
                            ...
                            <h3 class="text-xl font-black text-slate-800 tracking-tight">
                                {{ $currentRecord->karyawan->name }}</h3>
                            <p class="text-sm font-semibold text-slate-500 mt-1">{{ $currentRecord->NIK }} &bull;
                                {{ $currentRecord->karyawan->dept_code ?? 'N/A' }}</p>

                            <div class="flex items-center gap-2 mt-3">
                                <span
                                    class="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-100/50 uppercase tracking-wider">
                            ...
                            {{-- Attendance Badges --}}
                            <div
                                class="flex flex-wrap items-center justify-center gap-2 mt-5 w-full pt-5 border-t border-slate-100">
                                <div class="flex-1 min-w-[60px] p-2 rounded-xl bg-rose-50 border border-rose-100/50">
                                    <p class="text-[10px] font-bold text-rose-400 uppercase tracking-widest mb-0.5">
                                        Alpha</p>
                                ...
                                <div class="flex-1 min-w-[60px] p-2 rounded-xl bg-amber-50 border border-amber-100/50">
                                    <p class="text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-0.5">
                                        Telat</p>
                                ...
                                <div class="flex-1 min-w-[60px] p-2 rounded-xl bg-sky-50 border border-sky-100/50">
                                    <p class="text-[10px] font-bold text-sky-500 uppercase tracking-widest mb-0.5">Izin
                                    </p>
                                    <p class="text-lg font-black text-sky-700">{{ $currentRecord->Izin }}</p>
                                </div>
                                <div
                                    class="flex-1 min-w-[60px] p-2 rounded-xl bg-indigo-50 border border-indigo-100/50">
                    ...
                    <div class="w-full lg:w-2/3 space-y-4">
                        ...
                        @foreach ($fields as $field => $label)
                            <div
                                class="bg-white rounded-3xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center h-full p-6 text-center">
                ...
                <button @click="closeMode()"
                    class="mt-8 px-8 py-3 bg-slate-900 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all">
        ...
        <div x-show="isOpen" x-transition:enter="transition ease-out duration-300 transform"
            ...
            <div class="max-w-6xl mx-auto p-4 lg:px-8 flex gap-3 lg:justify-end">
                ...
                <button wire:click="saveGrade"
                    class="flex-1 lg:flex-none lg:w-64 h-14 rounded-2xl bg-indigo-600 text-white font-black text-lg shadow-lg shadow-indigo-600/30 hover:bg-indigo-700 hover:shadow-indigo-600/40 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 relative overflow-hidden group">
                    <div
                ...
                closeMode() {
                    ...
                    if (typeof window.reloadEvaluationTables === 'function') {
                        ...
                        document.querySelectorAll('table.dataTable').forEach(t => {
                            const dtInstance = typeof $.fn?.dataTable?.Api === 'function' ?
                                new $.fn.dataTable.Api(t) : null;
