...
>
    ...
    <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @php
                        $metas = [
                            ...
                            ['label' => 'Created at',   'value' => $form->created_at?->timezone('Asia/Jakarta')->format('d M Y, H:i')],
                ...
                <div class="flex items-start gap-0 min-w-max">
                    ...
                    <div class="flex flex-col items-center" style="width: 180px">
                        ...
                        <div class="relative flex w-full items-center">
                            @if ($idx > 0)
                                <div class="h-1 flex-1 {{ $step['status'] === 'approved' ? 'bg-emerald-400' : 'bg-slate-200' }}"></div>
                            @else
                                <div class="flex-1"></div>
                            @endif

                            {{-- Circle Badge --}}
                            <div class="relative z-10 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl border-2 transition-all shadow-sm
                        ...
                        <div class="mt-4 text-center px-2">
                            ...
                            @if ($step['approver_name'])
                                <p class="mt-1 text-[11px] font-bold text-slate-500">{{ $step['approver_name'] }}</p>
                            @endif
                            @if ($step['signed_at'])
                                <p class="mt-0.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ \Carbon\Carbon::parse($step['signed_at'])->format('d M — H:i') }}</p>
                            @endif
                            ...
                            <div class="mt-3 flex gap-2 justify-center">
                                <button type="button" wire:click="sign({{ $step['step_id'] }})" wire:loading.attr="disabled"
                                ...
                                <button type="button" x-on:click="rejectOpen = true; $wire.openRejectModal({{ $step['approval_id'] }})"
                    ...
                    <div class="text-center w-full py-8">
                        <div class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 mb-3 border border-slate-100">
        ...
        <div class="glass-card overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-slate-200/50">
            <div class="px-6 py-4 border-b border-slate-100/60 bg-white/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-sm font-extrabold text-slate-800 tracking-tight">
                        Overtime Details
                        <span class="ml-2 rounded-md bg-slate-100 border border-slate-200 px-1.5 py-0.5 text-[10px] font-black text-slate-600">
                            {{ $form->details->count() }}
                        </span>
                    </h2>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if ($approved) <span class="rounded-lg border border-emerald-200/50 bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 uppercase tracking-wider shadow-sm"><i class='bx bx-check'></i> {{ $approved }} Approved</span> @endif
                    @if ($rejected) <span class="rounded-lg border border-rose-200/50 bg-rose-50 px-2.5 py-1 text-[10px] font-bold text-rose-700 uppercase tracking-wider shadow-sm"><i class='bx bx-x'></i> {{ $rejected }} Rejected</span> @endif
                    @if ($pending)  <span class="rounded-lg border border-amber-200/50 bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700 uppercase tracking-wider shadow-sm"><i class='bx bx-time-five'></i> {{ $pending }} Pending</span> @endif
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100/60">
                        ...
                        <tr class="{{ $rowClass }} transition-colors" wire:key="detail-{{ $detail->id }}">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center text-slate-600">
                                <div class="font-bold">{{ $detail->start_time ? substr($detail->start_time, 0, 5) : '-' }}</div>
                                <div class="text-[9px] text-slate-400 mt-0.5">{{ $detail->start_date ? \Carbon\Carbon::parse($detail->start_date)->format('d M y') : '' }}</div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-center text-slate-600">
                                <div class="font-bold">{{ $detail->end_time ? substr($detail->end_time, 0, 5) : '-' }}</div>
                                <div class="text-[9px] text-slate-400 mt-0.5">{{ $detail->end_date ? \Carbon\Carbon::parse($detail->end_date)->format('d M y') : '' }}</div>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                <span class="font-black text-[11px] rounded bg-indigo-50 px-1.5 py-0.5 border border-indigo-100 {{ $actual ? 'text-indigo-700' : 'text-slate-300' }}">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center border-l border-slate-100/60">
                                @if ($detail->status === 'Approved')
                                    <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 shadow-sm" title="Approved"><i class='bx bx-check'></i></span>
                                @elseif ($detail->status === 'Rejected')
                                    <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-rose-100 text-rose-600 shadow-sm" title="Rejected: {{ $detail->reason }}"><i class='bx bx-x'></i></span>
                                @else
                        ...
                        <tr>
                            <td colspan="12" class="px-6 py-12 text-center">
                                ...
                                <p class="text-sm font-bold text-slate-500">No detail records found in this form.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
