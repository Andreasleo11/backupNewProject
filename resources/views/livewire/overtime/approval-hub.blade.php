...
<div class="space-y-6 pb-20 font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <div
                class="h-12 w-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-xl shadow-indigo-100 ring-4 ring-indigo-50">
                <i class='bx bxs-zap text-2xl'></i>
            </div>
            <div>
                <h1 class="text-xl font-black text-slate-900 tracking-tight uppercase">Approval Hub</h1>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-0.5">Focus: Contextual
                    Roster Review</p>
            </div>
        </div>
    ...
    @if ($groups->isEmpty())
        <div
            class="bg-white rounded-[3rem] border-4 border-dashed border-slate-100 p-20 text-center animate-in fade-in duration-700">
    ...
    @else
        <div class="grid grid-cols-1 gap-4">
            @foreach ($groups as $key => $items)
                ...
                <div wire:key="pack-{{ $key }}"
                    class="group bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden transition-all {{ $isExpanded ? 'ring-4 ring-indigo-500/5 border-indigo-200 shadow-xl' : 'hover:border-slate-300' }}">
                    {{-- GROUP HEADER --}}
                    <div class="px-8 py-6 flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-6 cursor-pointer"
                            ...
                            <input type="checkbox" value="{{ $key }}" wire:model.live="selectedPackKeys"
                                @click.stop
                        ...
                        <div class="flex items-center gap-3">
                            <div class="text-right hidden sm:block mr-2">
                                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-0.5">
                                    Requested by</p>
                                <p class="text-[11px] font-black text-slate-700">{{ $first->user->name }}</p>
                            </div>
                            <button type="button" wire:click="approvePack('{{ $key }}')"
                            ...
                            <i
                                class='bx bx-chevron-down text-xl text-slate-300 transition-transform duration-300 {{ $isExpanded ? 'rotate-180' : '' }}'></i>
                        </div>
                    </div>

                    {{-- EXPANDED ROSTER --}}
                    @if ($isExpanded)
                        <div x-data x-init="$nextTick(() => { $el.scrollIntoView({ behavior: 'smooth', block: 'nearest' }) })" x-show="true" x-collapse
                                ...
                                <table class="w-full text-left border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50/80 border-b border-slate-100">
                                            <th
                                                class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                                                Employee</th>
                                            <th
                                                class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                                                Task / Work Detail</th>
                                            <th
                                                class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">
                                                Timing</th>
                                            <th
                                                class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">
                                                Context Insights</th>
                                        ...
                                        @foreach ($items->flatMap->details as $detail)
                                            ...
                                            <tr class="hover:bg-slate-50/50 transition-colors">
                                                <td class="px-6 py-4">
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-[11px] font-black text-slate-900 uppercase tracking-tight">{{ $detail->name }}</span>
                                                        <span
                                                            class="text-[9px] font-mono font-bold text-slate-400">{{ $detail->NIK }}</span>
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4">
                                                    <p
                                                        class="text-[10px] text-slate-600 font-medium leading-relaxed italic">
                                                        "{{ $detail->job_desc }}"</p>
                                                </td>
                                                <td class="px-6 py-4 text-center">
                                                    <div
                                                        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-50/50 border border-indigo-100/50 text-indigo-600 font-mono text-[10px] font-bold">
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center justify-center gap-3">
                                                        @if ($insight)
                                                            ...
                                                            <div class="group relative">
                                                                <div
                                                                    ...
                                                                    <span
                                                                        class="text-[9px] font-black uppercase">{{ $insight['monthly_hours'] }}h
                                                                        MTD</span>
                                                                </div>
                                                                <div
                                                                    class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 hidden group-hover:block bg-slate-900 text-white text-[8px] p-2 rounded-lg text-center z-10 shadow-xl">
                                                            ...
                                                            @if ($insight['streak_days'] > 2)
                                                                <div class="group relative">
                                                                    <div
                                                                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all animate-pulse
                                                                        ...
                                                                        <span
                                                                            class="text-[9px] font-black uppercase">{{ $insight['streak_days'] }}
                                                                            Days Streak</span>
                                                                    </div>
                                                                    <div
                                                                        class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 hidden group-hover:block bg-slate-900 text-white text-[8px] p-2 rounded-lg text-center z-10 shadow-xl">
                                                        ...
                                                        @else
                                                            <div
                                                                class="h-6 w-20 bg-slate-100 rounded-full animate-pulse">
                                                            </div>
                                                        @endif
                                                    </div>
                                                </td>
