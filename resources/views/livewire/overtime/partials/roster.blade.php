      ...
      <div class="relative">
          ...
          <template x-if="items.length === 0 && $wire.stagedRosterData.length === 0">
              <div
                  ...
                  <p
                      class="text-sm text-slate-400 font-bold uppercase tracking-widest mb-12 max-w-sm mx-auto leading-relaxed">
                      Choose an entry method to start your overtime request</p>
                  ...
                  <div class="flex flex-col md:flex-row items-center justify-center gap-6">
                      ...
                      <button type="button" @click="$wire.set('showBulkTray', true)"
                          ...
                          <span class="absolute -top-2 -right-2 flex h-6 w-6">
                              ...
                              <span
                                  class="relative inline-flex rounded-full h-6 w-6 bg-indigo-500 border-2 border-white flex items-center justify-center text-[10px] font-black">!</span>
                          </span>
                          <div
                          ...
                          <div class="text-left">
                              <p
                                  class="text-[10px] font-black text-indigo-200 uppercase tracking-[0.2em] leading-none mb-1">
                                  Recommended</p>
                              <p class="text-base font-black uppercase tracking-tight leading-none">Select from
                                  Directory</p>
                          </div>
                      </button>
                      ...
                      <button type="button" @click="addRow()"
                          ...
                          <div class="text-left">
                              <p
                                  class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] leading-none mb-1">
                                  Standard</p>
                              <p class="text-base font-black uppercase tracking-tight leading-none">Add Member Manually
                              </p>
                          </div>
                      </button>
                  </div>
          ...
          {{-- THE TABLE --}}
          <div x-show="items.length > 0" x-transition:enter="transition ease-out duration-300"
                  ...
                  <table class="w-full text-left border-separate border-spacing-0">
                      <thead>
                          <tr class="bg-slate-50/50">
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 w-16">
                                  #</th>
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 w-64">
                                  Employee</th>
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 min-w-[200px]">
                                  Specific Task</th>
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 w-44 text-center">
                                  Timing</th>
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 w-24 text-center">
                                  Status</th>
                              <th
                                  class="px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 w-16">
                              </th>
                              ...
                              <tr class="group hover:bg-slate-50/50 transition-all" x-data="{ open: false, q: '', editingTimes: false }">
                                  ...
                                  <td class="px-4 py-4 align-top">
                                      <div class="h-7 w-7 rounded-lg flex items-center justify-center text-[10px] font-black"
                                          :class="row.nik ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' :
                                  ...
                                  <td class="px-4 py-4 align-top overflow-visible transition-all duration-300"
                                      ...
                                      <div class="flex items-center gap-3 p-2 rounded-2xl transition-all border-2"
                                          :class="hasError(index, 'nik') ? 'border-rose-500 bg-rose-50' : (!row.nik ?
                                              'border-amber-100 bg-amber-50/30' : 'border-transparent')">
                                          ...
                                          <div class="flex-1 min-w-0 relative" @click.outside="open = false">
                                              <div class="relative">
                                                  <input type="text"
                                                      ...
                                                      :class="hasError(index, 'nik') ? 'text-rose-700' : (row.name && !row.nik ?
                                                          'text-amber-700' : 'text-slate-900')"
                                                      placeholder="Search Employee..."
                                                      x-model.debounce.750ms="row.name" @focus="open = true"
                                                      @input="q = $event.target.value; open = true;"
                                                      @keydown.escape="open = false">

                                                  <div class="flex items-center gap-2 mt-0.5">
                                                      <span
                                                          class="text-[8px] font-mono font-bold uppercase tracking-widest transition-colors"
                                                          :class="hasError(index, 'nik') ? 'text-rose-500' : (!row.nik ?
                                                              'text-amber-500 font-black' : 'text-slate-400')"
                                              ...
                                              <div x-show="open && filteredBy('name', q).length" x-cloak
                                                  ...
                                                  <template x-for="(emp, i) in filteredBy('name', q)"
                                                      :key="emp.nik">
                                                      <div class="cursor-pointer px-4 py-3 rounded-xl flex items-center gap-3 hover:bg-indigo-600 hover:text-white transition-all group/item"
                                                          ...
                                                          <div class="flex-1 min-w-0">
                                                              <p class="text-[11px] font-black truncate"
                                                                  x-text="emp.name"></p>
                                                              <p class="text-[9px] font-mono opacity-50"
                                                                  x-text="emp.nik"></p>
                                                          </div>
                                                      </div>
                                                  </template>
                                  ...
                                  <td class="px-4 py-6 align-top">
                                      <textarea x-model.debounce.1000ms="row.job_desc" rows="2"
                                          ...
                                          :class="hasError(index, 'job_desc') ?
                                              'border-rose-200 bg-rose-50/30 ring-2 ring-rose-500/5' :
                                              'border-slate-100 bg-slate-50/30 focus:bg-white focus:border-indigo-200'"
                                          placeholder="Employee's objective..."></textarea>
                                      <template x-if="hasError(index, 'job_desc')">
                                          <p class="text-[7px] font-black text-rose-500 uppercase tracking-widest mt-1 ml-1"
                                              x-text="getError(index, 'job_desc')"></p>
                                      </template>
                                  </td>

                                  {{-- TIMING CHIP --}}
                                  <td class="px-4 py-6 align-top text-center overflow-visible">
                                      <div class="relative inline-block w-full" x-data="{ editingTimes: false }"
                                          @click.outside="editingTimes = false">
                                          <button type="button" @click="editingTimes = !editingTimes"
                                              ...
                                              :class="hasTimeError(index) ?
                                                  'border-rose-400 bg-rose-50 ring-4 ring-rose-500/10' :
                                                  'border-slate-200 bg-white'">

                                              {{-- DATE RANGE (shown only if dates differ or multi-day mode) --}}
                                              <template
                                                  x-if="row.start_date !== row.end_date || row.overtime_date !== row.start_date">
                                                  <div
                                                      class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
                                                      <span x-text="row.overtime_date"></span>
                                                      <span class="mx-1 opacity-30">|</span>
                                                      <span
                                                          x-text="formatDateRange(row.start_date, row.end_date)"></span>
                                                  </div>
                                              </template>
                                          ...
                                          {{-- TIME POPOVER --}}
                                          <div x-show="editingTimes" x-cloak
                                              ...
                                              <div class="flex items-center justify-between mb-4">
                                                  <div class="flex items-center gap-2">
                                                      <span
                                                          class="text-[9px] font-black text-indigo-600 uppercase tracking-widest">Time
                                                          Override</span>
                                                      <template x-if="hasTimeError(index)">
                                                          <span
                                                              class="px-2 py-0.5 rounded-full bg-rose-50 text-rose-500 text-[7px] font-black uppercase tracking-tighter">Errors
                                                              Found</span>
                                                      </template>
                                                  </div>
                                              ...
                                              <div class="mb-6 space-y-4">
                                                  <div>
                                                      <label
                                                          class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Overtime
                                                          Date (Payroll Ref)</label>
                                                  ...
                                                  <div class="grid grid-cols-2 gap-4">
                                                      <div>
                                                          <label
                                                              class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Start
                                                              Date</label>
                                                      ...
                                                      <div>
                                                          <label
                                                              class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">End
                                                              Date</label>
                                              ...
                                              <div class="grid grid-cols-2 gap-4">
                                                  <div>
                                                      <label
                                                          class="block text-[8px] font-black uppercase text-slate-400 mb-1.5 ml-1">Start</label>
                                                      <input type="time" x-model="row.start_time"
                                                          ...
                                                          :class="hasError(index, 'start_time') ?
                                                              'border-rose-200 bg-rose-50 text-rose-500' :
                                                              'border-slate-200 bg-slate-50'">
                                                      <template x-if="hasError(index, 'start_time')">
                                                          <p class="text-[7px] font-black text-rose-500 uppercase mt-1 ml-1"
                                                              x-text="getError(index, 'start_time')"></p>
                                                      </template>
                                                  </div>
                                                  <div>
                                                      <label
                                                          class="block text-[8px] font-black uppercase text-slate-400 mb-1.5 ml-1">End</label>
                                                      <input type="time" x-model="row.end_time"
                                                          class="w-full rounded-xl py-2.5 px-3 text-xs font-mono font-black text-center transition-all border"
                                                          :class="hasError(index, 'end_time') ?
                                                              'border-rose-200 bg-rose-50 text-rose-500' :
                                                              'border-slate-200 bg-slate-50'">
                                                      <template x-if="hasError(index, 'end_time')">
                                                          <p class="text-[7px] font-black text-rose-500 uppercase mt-1 ml-1"
                                                              x-text="getError(index, 'end_time')"></p>
                                                      </template>
                                                  </div>
                                              </div>
                                              ...
                                              <div class="mt-4 pt-4 border-t border-slate-100 space-y-3">
                                                  <div class="flex items-center justify-between">
                                                      <div>
                                                          <label
                                                              class="block text-[8px] font-black uppercase text-slate-400 mb-1 ml-1">Break
                                                              (min)</label>
                                                          <input type="number" x-model="row.break"
                                                              class="w-20 rounded-xl py-2 px-3 text-[10px] font-black text-center border"
                                                              :class="hasError(index, 'break') ?
                                                                  'border-rose-200 bg-rose-50 text-rose-500' :
                                                                  'border-slate-100 bg-slate-50/50'">
                                                      </div>
                                                      <div class="text-right">
                                                          <span
                                                              class="block text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Net
                                                              OT</span>
                                                          <span class="text-lg font-black text-slate-900 leading-none"
                                                              x-text="calculateNet(row)"></span>
                                                      </div>
                                                  </div>
                                                  <template x-if="hasError(index, 'break')">
                                                      <p class="text-[7px] font-black text-rose-500 uppercase ml-1"
                                                          x-text="getError(index, 'break')"></p>
                                                  </template>
                                              </div>
                                          </div>
                                      </div>
                                  ...
                                  <td class="px-4 py-6 align-top text-center">
                                      <div class="inline-flex h-9 w-9 items-center justify-center rounded-xl transition-all"
                                          :class="row.payroll_status === 'safe' ? 'bg-emerald-50 text-emerald-500' : (row
              ...
              <template x-if="items.length === 0 && $wire.stagedRosterData.length === 0">
                  <div class="py-20 text-center">
                      ...
                      <button type="button" @click="addRow()"
                          ...
                          here to add the first person</button>
                  </div>
              </template>
          </div>
      </div>
