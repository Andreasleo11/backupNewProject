                    ...
                    <thead class="border-b border-slate-200/60 bg-slate-50/80 text-[10px] font-black uppercase tracking-widest text-slate-500">
                        <tr>
                            ...
                            <th class="w-12 px-6 py-4 bg-slate-50">
                                <input type="checkbox" :checked="isAllSelected" @change="toggleAll" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                            </th>
                            @endif
                    ...
                    <tbody class="divide-y divide-slate-100/60 bg-white/30 {{ $compact }}">
                        @forelse ($dataheader as $fot)
                            ...
                            <tr wire:key="row-{{ $fot->id }}" class="group hover:bg-indigo-50/20 transition-all duration-200"
                                ...
                                @if($canApprove)
                                <td class="px-6 py-4">
                                    <input type="checkbox" x-model="selectedIds" value="{{ $fot->id }}" class="row-checkbox rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                </td>
                                @endif
    ...
    <template x-teleport="body">
        <div x-cloak x-show="$wire.showSnapshot" class="relative z-[150]">
            ...
            <div x-show="$wire.showSnapshot" 
                 ...
                 @click="$wire.showSnapshot = false"></div>
            ...
            <div class="fixed inset-y-0 right-0 z-[160] w-full max-w-lg md:max-w-xl flex">
                <div x-show="$wire.showSnapshot"
                    ...
                    <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        ...
                        <button @click="$wire.showSnapshot = false" class="h-10 w-10 rounded-xl hover:bg-slate-100 transition-all flex items-center justify-center text-slate-400">
                    ...
                    <div class="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar space-y-8">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($snapshot['departments'] ?? [] as $name => $count)
                                        <span class="px-2 py-1 rounded-lg bg-slate-100 border border-slate-200 text-[10px] font-bold text-slate-600">
                                            {{ $name }} <span class="ml-1 opacity-50">x{{ $count }}</span>
                                        </span>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <p class="text-[10px] font-medium text-slate-400 text-center leading-relaxed italic">
                            By proceeding, your digital signature will be applied to all <span x-text="selectedIds.length"></span> selected forms. Ensure you have reviewed the aggregate data above correctly.
                        </p>
                    </div>
                    ...
                    <div class="p-8 border-t border-slate-100 flex items-center gap-3 bg-slate-50/30">
                        ...
                        <button type="button" 
                            ...
                            <span wire:loading wire:target="bulkApprove"><i class='bx bx-loader-alt animate-spin'></i> Signing...</span>
                        </button>
                    </div>
                </div>
            </div>
