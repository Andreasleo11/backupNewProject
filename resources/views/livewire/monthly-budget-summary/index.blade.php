<div class="container py-3">
        ...
        <form action="{{ route('monthly.budget.summary.report.store') }}" method="post"
          ...
          <div class="input-group">
            ...
            <input type="text" id="monthPicker" name="month" class="form-control"
                   placeholder="Select Month (mm-yyyy)" aria-label="Select Month" aria-describedby="monthPickerLabel" required>
          </div>
      ...
      <div class="card-body">
        <div class="row g-2 align-items-end">
          <div class="col-12 col-md-3">
            ...
            <input wire:model.live.debounce.400ms="search" type="text" id="search" class="form-control" placeholder="Doc # or Creator name">
          </div>

          <div class="col-6 col-md-2">
            <label for="from" class="form-label small text-secondary mb-1">Month from</label>
            <input wire:model.live="monthFrom" type="text" id="from" class="form-control" placeholder="mm-yyyy">
          </div>

          <div class="col-6 col-md-2">
            <label for="to" class="form-label small text-secondary mb-1">Month to</label>
            <input wire:model.live="monthTo" type="text" id="to" class="form-control" placeholder="mm-yyyy">
          </div>
  ...
  <div class="card mt-4 position-relative">
    ...
    {{-- Loading overlay --}}
    <div wire:loading.flex class="position-absolute top-0 start-0 w-100 h-100 bg-body bg-opacity-75
                                 align-items-center justify-content-center" aria-live="polite">
    ...
    <div class="card-body p-0">
      <div class="table-responsive">
        <table class="table table-hover table-striped align-middle mb-0">
          <thead class="bg-body">
            <tr>
              <th class="text-secondary text-uppercase small fw-semibold text-nowrap position-sticky top-0 bg-body">#</th>
              ...
              <th class="position-sticky top-0 bg-body">
                <button type="button" class="btn btn-link p-0 text-decoration-none"
                  ...
                  @include('partials.sort-icon', ['field' => 'doc_num', 'sortField' => $sortField, 'direction' => $sortDirection])
                </button>
              </th>
              ...
              <th class="position-sticky top-0 bg-body">
                <button type="button" class="btn btn-link p-0 text-decoration-none"
                  ...
                  @include('partials.sort-icon', ['field' => 'report_date', 'sortField' => $sortField, 'direction' => $sortDirection])
                </button>
              </th>
              ...
              <th class="position-sticky top-0 bg-body">
                <button type="button" class="btn btn-link p-0 text-decoration-none"
                  ...
                  @include('partials.sort-icon', ['field' => 'created_at', 'sortField' => $sortField, 'direction' => $sortDirection])
                </button>
              </th>

              <th class="text-secondary text-uppercase small fw-semibold position-sticky top-0 bg-body">Status</th>
              ...
              <th class="text-end position-sticky top-0 bg-body">
                <button type="button" class="btn btn-link p-0 text-decoration-none"
                  ...
                  @include('partials.sort-icon', ['field' => 'total', 'sortField' => $sortField, 'direction' => $sortDirection])
                </button>
              </th>

              <th class="text-secondary text-uppercase small fw-semibold text-end position-sticky top-0 bg-body">MoM</th>
              <th class="text-secondary text-uppercase small fw-semibold position-sticky top-0 bg-body">Action</th>
            </tr>
          </thead>
          ...
          <tbody class="table-group-divider">
            @forelse ($reports as $report)
              @php
                ...
                $m = $report->mom ?? ['has_prev' => false, 'direction' => 'none', 'diff' => null, 'pct' => null, 'prev' => 0];
              @endphp
              ...
              <tr>
                ...
                <td>
                  @include('partials.monthly-budget-summary-report-status', ['status' => $report->status])
                </td>
                ...
                <td class="text-end text-nowrap">
                  @if (!$m['has_prev'])
                    <span class="text-muted" data-bs-toggle="tooltip" data-bs-title="No prior month">—</span>
              ...
              <tr>
                <td colspan="8" class="py-5">
                  <div class="d-flex flex-column align-items-center text-center">
                    ...
                    <div class="text-secondary small mb-3">Adjust filters or generate a new report.</div>
                    @if ($showGenerateButton)
  ...
  @push('extraJs')
    <script type="module">
      ...
      const monthOpts = { format: "mm-yyyy", startView: "months", minViewMode: "months", autoclose: true };
  ...
  @push('extraCss')
    <style>
      thead tr > * { z-index: 2; }
      thead .btn.btn-link { color: inherit; }
      thead .btn.btn-link:hover { text-decoration: none; opacity: .8; }
    </style>
  @endpush
</div>
