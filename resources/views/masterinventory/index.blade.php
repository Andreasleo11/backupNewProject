...
@section('content')
  ...
  @php
    $authUser = auth()->user();
    $totalItems =
        $datas instanceof \Illuminate\Pagination\LengthAwarePaginator
            ? $datas->total()
            : $datas->count();
    $currentPage =
        $datas instanceof \Illuminate\Pagination\LengthAwarePaginator ? $datas->currentPage() : 1;
    $perPage =
        $datas instanceof \Illuminate\Pagination\LengthAwarePaginator
            ? $datas->perPage()
            : $totalItems;
  ...
  <div class="container">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('masterinventory.index') }}">Master Inventory</a>
        </li>
    ...
    <div class="row">
      ...
      <div class="col text-end">
        ...
        @endphp
        @if ($showCreateButton)
          <a href="{{ route('masterinventory.createpage') }}" class="btn btn-primary">New
            Inventory</a>
        @endif
        ...
        <div class="row justify-content-between align-items-center mb-3">
          <div class="col-auto d-flex align-items-center">
            ...
            <select id="itemsPerPage" class="form-select">
              <option value="10" {{ request()->get('itemsPerPage') == 10 ? 'selected' : '' }}>10
              </option>
              <option value="25" {{ request()->get('itemsPerPage') == 25 ? 'selected' : '' }}>25
              </option>
              <option value="50" {{ request()->get('itemsPerPage') == 50 ? 'selected' : '' }}>50
              </option>
              <option value="100" {{ request()->get('itemsPerPage') == 100 ? 'selected' : '' }}>100
              </option>
              <option value="all" {{ request()->get('itemsPerPage') == 'all' ? 'selected' : '' }}>
                All
              </option>
            </select>
          </div>
