...
<form action="{{ route('processAssemblyForm') }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="number" name="hm_fg" id="hm_fg" value="{{ $data[0]->val_int_asm }}" class="form-control" required>
    </div>
    <div class="form-group">
        <label for="hm_wip">Jarak H-Min WIP (hari):</label>
        <input type="number" name="hm_wip" id="hm_wip" value="{{ $data[1]->val_int_asm }}" class="form-control" required>
    </div>
    <div class="form-group">
        <label for="jarak_gudang">Jarak simpan Gudang (hari):</label>
        <input type="number" name="jarak_gudang"  id="jarak_gudang" value="{{ $data[2]->val_int_asm }}" class="form-control" required>
    </div>
    <div class="form-group">
        <label for="max_manpower">Batas Maksimal Man Power Per Hari:</label>
        <input type="number" name="max_manpower" id="max_manpower" value="{{ $data[3]->val_int_asm }}" class="form-control" required>
    </div>
    <div class="form-group">
        <label for="max_mould_change">Batas Maksimal Pergantian Mould Per Mesin Per Hari:</label>
        <input type="number" name="max_mould_change" id="max_mould_change" value="{{ $data[4]->val_int_asm }}" class="form-control" required>
    </div>
    <div class="form-group">
