...
@php
    ...
    $pageDescription =
        'Kelola database Master Employee Evaluation bulanan. Unggah dari Excel, monitor kelengkapan data, dan lakukan penghapusan massal.';
    $pageIcon = 'bx-table';
@endphp
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="evaluationDataAdmin()">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-indigo-900 shadow-xl mb-8">
            ...
            <div class="relative p-8 sm:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div class="max-w-3xl">
                    <div
                        class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-200 border border-indigo-500/30 mb-4 text-xs font-semibold tracking-wide uppercase">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-black text-white mb-2 tracking-tight">Manajemen Data <span
                            class="text-indigo-300">Bulanan</span></h1>
                    <p class="text-indigo-100/80 text-lg max-w-2xl leading-relaxed">{{ $pageDescription }}</p>
                </div>

                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto shrink-0">
                    <button @click="openUploadModal"
                        class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-indigo-900 hover:bg-indigo-50 font-bold rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 border border-white/20 whitespace-nowrap">
                        <i class='bx bx-cloud-upload text-xl'></i>
                        <span>Unggah Data Excel</span>
                    </button>
                    <button @click="confirmTruncate"
                        class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-rose-500/10 text-rose-300 hover:bg-rose-500 hover:text-white font-bold rounded-xl transition-all border border-rose-500/30 whitespace-nowrap">
        ...
        <div x-show="uploadModalOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div x-show="uploadModalOpen" x-transition:enter="ease-out duration-300"
                ...
                <div x-show="step === 1" class="w-full p-8 flex flex-col items-center">
                    <div
                        class="h-16 w-16 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center text-3xl mb-4 shadow-sm">
                    ...
                    <form @submit.prevent="submitUpload" class="w-full">
                            ...
                            <div class="text-center flex flex-col items-center">
                                <i class="bx"
                                    :class="selectedFile ? 'bx-check-circle text-emerald-500 bx-tada text-5xl' :
                                        'bx-cloud-upload text-indigo-400 group-hover:text-indigo-500 text-5xl'"
                        ...
                        <button type="submit" :disabled="!selectedFile || isUploading"
                            class="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div x-show="step === 2" x-cloak class="w-full">
                    <div class="bg-indigo-50 p-6 border-b border-indigo-100 flex items-center gap-4">
                        <div
                            class="h-12 w-12 bg-white text-indigo-600 rounded-xl flex items-center justify-center text-2xl shadow-sm border border-indigo-100 shrink-0">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <button @click="commitUpload" :disabled="isUploading"
                                class="flex-[2] py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2">
                                <span x-show="isUploading"
                                    class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-solid border-white border-r-transparent"></span>
                ...
                commitUpload() {
                    ...
                    axios.post('{{ route('admin.evaluation-data.commit') }}', {
                        ...
                        .then(res => {
                            ...
                            window.LaravelDataTables['evaluation-data-management-table'].ajax
                                .reload();
                            Swal.fire({
                                title: 'Berhasil!',
                            ...
                            axios.delete('{{ route('admin.evaluation-data.truncate') }}')
                                .then(res => {
                                    window.LaravelDataTables[
                                        'evaluation-data-management-table'].ajax
                                    .reload();
