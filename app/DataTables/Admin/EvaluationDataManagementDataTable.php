...
class EvaluationDataManagementDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('Month', function (EvaluationData $record) {
                return $record->Month ? Carbon::parse($record->Month)->translatedFormat('F Y') : '-';
            })
            ->addColumn('status', function (EvaluationData $record) {
                if ($record->total == 0) {
    ...
    {
        return $this->builder()
            ->setTableId('evaluation-data-management-table')
