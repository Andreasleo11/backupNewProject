...
class Index extends Component
{
    ...
    {
        if (in_array($name, ['startDate', 'endDate', 'dept', 'infoStatus', 'isPush', 'search', 'perPage', 'sortField', 'sortDirection'])) {
            $this->resetPage();
        }
    ...
    {
        if (!in_array($field, ['id', 'first_overtime_date', 'status'], true)) return;
    ...
    public function setRange(string $preset): void
    {
        ...
        [$start, $end] = match ($preset) {
            'today' => [$today, $today],
            '7d'    => [now()->subDays(6)->toDateString(), $today],   // inclusive last 7 (today + 6 back)
    ...
    public function exportCsv()
    {
        ...
        return response()->streamDownload(function () {
            ...
            // header row
            fputcsv($out, ['ID', 'Admin', 'Dept', 'Branch', 'First Overtime Date', 'Status', 'Type', 'After Hour', 'Approved', 'Rejected', 'Pending', 'Created At']);
        ...
        $query->where(function ($query) use ($user) {
            if ($user->role->name === 'SUPERADMIN') {
                ...
                if ($user->name === 'Umi') {
                    ...
                    $query->whereHas('department', fn($q) => $q->whereIn('name', ['PLASTIC INJECTION', 'MAINTENANCE MACHINE']));
                } else {
                    $query->whereHas('department', fn($q) => $q->where('name', $user->department->name));
                }
            }
    ...
    private function scopeFilters($query)
    {
        ...
        if ($this->dept) $query->where('dept_id', $this->dept);

        if (Auth::user()->specification->name === 'VERIFICATOR' && ($this->isPush === '0' || $this->isPush === '1')) {
