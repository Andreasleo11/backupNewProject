...
class RuleManager extends Component
{
    ...
    public function saveRule(): void
    {
        ...
        if ($this->editingRuleId) {
            ...
            $this->dispatch('toast', message: "New version (v{$rule->version_number}) created successfully.", type: 'success');
        } else {
            // Creating new rule - version_uuid will be auto-generated by the trait
            $data['created_by'] = auth()->id();
            $rule = RuleTemplate::create($data);
            $this->dispatch('toast', message: 'Rule created.', type: 'success');
        }
    ...
    public function forceCreateNewVersion(int $ruleId, array $data): void
    {
        ...
        $this->selectedRuleId = $rule->id;
        $this->dispatch('toast', message: "New version (v{$rule->version_number}) created. Active requests still use old version.", type: 'success');
    }
    ...
    private function performRuleSoftDeletion(RuleTemplate $rule): void
    {
        ...
        $this->dispatch('toast', message: 'Rule soft-deleted. It can be restored if needed.', type: 'success');
    }
    ...
    public function restoreRule(int $id): void
    {
        ...
        $this->dispatch('toast', message: 'Rule restored.', type: 'success');
    }
    ...
    public function forceDeleteRulePermanently(int $id): void
    {
        ...
        $this->dispatch('toast', message: 'Rule permanently deleted.', type: 'success');
    }
    ...
    public function openCreateStep(): void
    {
        if (! $this->selectedRuleId) {
            $this->dispatch('toast', message: 'Select a rule first.', type: 'error');
