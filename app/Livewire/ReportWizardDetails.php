...
class ReportWizardDetails extends Component
{
        ...
        if ($this->details) {
            foreach ($this->details as &$detail) {
                ...
                if (!$detail['part_code'] && !empty($detail['part_name'])) {
                    ...
                    $part_name = $parts[1];
                    // dd($part_name);
                    $part = MasterDataPart::select('item_no')->where('description', $part_name)->first();
                    if ($part) {
    ...
    private function resolvePartByCode(?string $args): ?array
    {
        ...
        $row = MasterDataPart::select('item_no', 'description')->where('item_no', $part_no)->first();
    ...
    private function loadSuggestions($index, $search)
    {
        if (strlen($search) > 0) {
            ...
            $this->partSuggestions[$index] = MasterDataRogPartName::where('name', 'like', "%{$search}%")
                ->orderBy('name')
                ->limit($limit)
                ->pluck('name')
                ->toArray();
