...
class ReportWizardDefects extends Component
{
    ...
    {
        $this->existingDetails = collect(Session::get("report.details", []))->map(function (
            $item,
            $i,
        ) {
            $item["id"] = $i;
    ...
    public function removeDefectFromSession($key)
    {
        ...
        if (isset($report["details"][$this->activeDetailId]["defects"][$key])) {
            ...
            $report["details"][$this->activeDetailId]["defects"] = array_values(
                $report["details"][$this->activeDetailId]["defects"],
            ); // reindex
