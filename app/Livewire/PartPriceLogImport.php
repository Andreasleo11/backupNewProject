...
class PartPriceLogImport extends Component
{
    ...
    private function looksLikeHeader(array $header): bool
    {
        ...
        return Str::contains($joined, 'parent') || Str::contains($joined, 'item') || Str::contains($joined, 'total') || Str::contains($joined, 'original');
    }

    private function makeHeaderMap(array $header): array
    {
        // Normalize header names
        $normalized = array_map(fn($h) => strtolower(preg_replace('/\s+/', ' ', trim((string)$h))), $header);

        $find = function ($keys) use ($normalized) {
            foreach ($keys as $k) {
                foreach ($normalized as $i => $h) {
                    if (Str::contains($h, $k)) return $i;
                }
            }
            return null;
        };
        ...
        return [
            'parent_item' => $find(['parent item', 'parent', 'item code', 'part', 'part item']) ?? 1,
    ...
    private function between(string $s, string $start, string $end): string
    {
        $p1 = strpos($s, $start);
        $p2 = strrpos($s, $end);
        if ($p1 === false || $p2 === false || $p2 <= $p1) return $s;
