...
class DailyReportIndex extends Component
{
    ...
    {
        if (in_array($name, ['search', 'employeeId', 'departmentNo', 'jabatan', 'dateFrom', 'dateTo'], true)) {
            $this->resetPage();
        }
    }
    ...
    public function render()
    {
        ...
        $base = DB::table('employee_daily_reports as dr')
            ...
            ->leftJoinSub($latestSub, 'lr', fn($join) => $join->on('lr.employee_id', '=', 'dr.employee_id'))
            ->when(
                !empty($this->validNiks),
                fn($q) =>
                $q->whereIn('e.NIK', $this->validNiks)
            )
            // Filters
            ->when($this->employeeId, fn($q) => $q->where('dr.employee_id', $this->employeeId))
            ->when($this->departmentNo, fn($q) => $q->where('dr.departement_id', $this->departmentNo))
            ->when($this->jabatan, fn($q) => $q->where('e.jabatan', $this->jabatan))
            ->when($this->search, function ($q) {
                $term = '%' . trim($this->search) . '%';
                $q->where(function ($w) use ($term) {
                    $w->where('dr.employee_id', 'like', $term)
                        ->orWhere('e.Nama', 'like', $term);
                });
            })
