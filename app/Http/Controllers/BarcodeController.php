...
class BarcodeController extends Controller
{
    ...
    public function missingbarcodegenerator(Request $request)
    {
        ...
        return view('barcodeinandout.missingbarcode', [
            'barcodes' => $barcodes,
            'partno' => $partNo,
        ]);
    }
    ...
    public function generateBarcode(Request $request)
    {
        ...
        for ($i = 0; $i <= $looping; $i++) {
            ...
            $barcodeData =
                $partNumber."\t".$defaultquantity."\t".$defaultwarehouse."\t".$startnum;
            // Generate the barcode using DNS1D (1D Barcode)
            $barcode = new DNS1D;

            // Use $spkNumber in the filename
            $filename =
                preg_replace('/[()#,.\\s&]+(?<!png)/i', '', $partNumber).
                '-'.
                $defaultquantity.
                '-'.
                $defaultwarehouse.
    ...
    public function processInAndOut(Request $request)
    {
        ...
        return view(
            'barcodeinandout.scanpage',
            compact('noDokumen', 'tanggalScanFull', 'position', 'HeaderScan'),
        );
    }
        ...
        while (isset($data['partno'.$counter])) {
            ...
            if (! $exists) {
                BarcodePackagingDetail::create([
                    ...
                    'scantime' => \Carbon\Carbon::parse($data['scantime'.$counter])->format(
                        'Y-m-d H:i:s',
                    ),
                ]);
            }
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->filled('dateScan')) {
            $dateScan = \Carbon\Carbon::createFromFormat(
                'Y-m-d',
                $request->input('dateScan'),
            )->startOfDay();
    ...
    {
        $query = BarcodePackagingMaster::with([
            'detailbarcode' => function ($query) use ($request) {
    ...
    public function stockall($location = 'Jakarta')
    {
        ...
        foreach ($partNos as $partNo) {
            ...
            $locationQuantity = $datas
                ->where('partNo', $partNo)
                ->where('position', $locationData['position'])
                ->sum('quantity');

            $customerQuantity = $datas
                ->where('partNo', $partNo)
