...
use App\Models\ProdplanAsmDelitem;
use App\Models\ProdplanAsmDelraw;
use App\Models\ProdplanAsmDelsched;
use App\Models\ProdplanAsmItem;
use App\Models\ProdplanAsmLinelist;
...
class PPSAssemblyController extends Controller
{
    ...
    public function processAssemblyForm(Request $request)
    {
          ...
          return redirect()->route('assemblyprocess1');
    }
    ...
    public function process1()
    {
		...
		$val_lead_fg = $tab_scenario_lead_fg->val_int_asm;
		
		$tab_scenario_lead_wip = DB::table('prodplan_scenarios')->where('id',2)->first();
		$val_lead_wip = $tab_scenario_lead_wip->val_int_asm;
		
		$tab_date_list = DB::table('uti_date_list')->where('id',17)->first();
		...
		foreach($tab_delsched_final as $delsched_final){
			...
			if(empty($tab_delsched_final_wip->id)){
				...
				if(empty($val_item_code)){
					...
					ProdplanAsmDelraw::insert($ins_delraw);
				}
				
			} else {
				
				$tab_delsched_final_wip_link = DB::table('delsched_finalwip')->where('fglink_id',$val_id_fg)->where('departement','362')->get();
		...
		//Tarik ke delitem
		$tab_delraw_itemonly_paired = DB::table('prodplan_asm_delraws')->select('item_code','item_pair')->distinct()->get();
		...
		foreach($tab_delraw_itemonly_paired as $delraw_itemonly_paired){
			...
			if(empty($val_item_pair)){
				...
				ProdplanAsmDelitem::insert($ins_delitem);
			}
		}
		
		$tab_delraw_itemonly_nopair = DB::table('prodplan_asm_delraws')->select('item_code','item_pair')->distinct()->get();
		...
		foreach($tab_delraw_itemonly_nopair as $delraw_itemonly_nopair){
			...
			$tab_delitem = DB::table('prodplan_snd_delitems')->where('item_pair',$val_item_code_ii)->first();
        ...
        return redirect()->route('assemblyprocess2');

    }

    public function process2()
    {
        $tab_delraw_date = DB::table('prodplan_asm_delraws')->select('delivery_date')->orderBy('delivery_date','asc')->distinct()->get();
		
		foreach($tab_delraw_date as $delraw_date){
			
			$val_delivery_date = $delraw_date->delivery_date;
			
			$tab_delitem = DB::table('prodplan_asm_delitems')->get();
			
			foreach($tab_delitem as $delitem){
				
				$val_item_code = $delitem->item_code;
			
				$sum_del_qty = DB::table('prodplan_asm_delraws')->where('item_code',$val_item_code)->where('delivery_date',$val_delivery_date)->sum('quantity');							
				...
				if($sum_del_qty>0){			
					...
					if(empty($delitem->item_pair)){												
						...
						ProdplanAsmDelsched::insert($ins_delsched);
						
					} else {
					
						$val_pair_code = $delitem->item_pair;
						
						$sum_del_pair_qty = DB::table('prodplan_asm_delraws')->where('item_code',$val_pair_code)->where('delivery_date',$val_delivery_date)->sum('quantity');
        ...
        return redirect()->route('assemblyprocess3');
    }

    public function process3()
    {

        $tab_delsched = DB::table('prodplan_asm_delscheds')->get();
		...
		foreach($tab_delsched as $delsched){
			...
			$val_lead_time_fg = $tab_scen_lead_time_fg->val_int_asm;
			
			$tab_scen_lead_time_wip = DB::table('prodplan_scenarios')->where('id',2)->first();
			$val_lead_time_wip = $tab_scen_lead_time_wip->val_int_asm;
			...
			DB::table('prodplan_asm_delscheds')->where('id',$val_delsched_id)->update([	
    ...
    public function deliveryassembly(ProdplanAsmDelschedDataTable $dataTable)
    {
        // return view("pps.assemblydelivery");
        return $dataTable->render("pps.assemblydelivery");
    }
    ...
    public function process4()
    {
		...
		$tab_delsched_itemonly = DB::table('prodplan_asm_delscheds')->select('item_code')->distinct()->get();
		
		foreach($tab_delsched_itemonly as $delsched_itemonly){
						
			$ins_items = array(					
				'item_code' => $delsched_itemonly->item_code);
			ProdplanAsmItem::insert($ins_items);
			
		}
		
		$tab_items = DB::table('prodplan_asm_items')->get();
		
		foreach($tab_items as $items){
			
			$val_items_id = $items->id;
			$val_item_code = $items->item_code;
			$tab_delsched = DB::table('prodplan_asm_delscheds')->where('item_code',$val_item_code)->first();
			...
			$sum_outstanding = DB::table('prodplan_asm_delscheds')->where('item_code',$val_item_code)->sum('outstanding');
			
			//Update data di tabel items
			DB::table('prodplan_asm_items')->where('id',$val_items_id)->update([			
				'pair_code' => $val_pair_code,
				'bom_level' => $val_bom_level,
		...
		$tab_items_up = DB::table('prodplan_asm_items')->get();
        ...
        return redirect()->route('itemassembly');
    }

    public function itemassembly(ProdplanAsmItemDataTable $dataTable)
    {
        // return view("pps.assemblyitem");
        return $dataTable->render("pps.assemblyitem");
    }
    ...
    public function process5()
	{
		...
		$data = InvLineList::where('departement', 362)->get();
		// dd($data);

		foreach($data as $item)
		{
			ProdplanAsmLinelist::create([
				'area' => $item->area,
