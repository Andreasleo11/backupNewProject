...
class PPSAssemblyController extends Controller
{
    ...
    public function process1()
    {
        ...
        // Tarik ke delitem
        $tab_delraw_itemonly_paired = DB::table('prodplan_asm_delraws')
        ...
        $tab_delraw_itemonly_nopair = DB::table('prodplan_asm_delraws')
            ->select('item_code', 'item_pair')
            ->distinct()
            ->get();
    ...
    {
        $tab_delraw_date = DB::table('prodplan_asm_delraws')
        ...
        foreach ($tab_delraw_date as $delraw_date) {
            ...
            foreach ($tab_delitem as $delitem) {
                ...
                $sum_del_qty = DB::table('prodplan_asm_delraws')
                    ->where('item_code', $val_item_code)
                ...
                if ($sum_del_qty > 0) {
                    if (empty($delitem->item_pair)) {
                        ...
                        $sum_del_pair_qty = DB::table('prodplan_asm_delraws')
    ...
    public function process3()
    {
        ...
        foreach ($tab_delsched as $delsched) {
            ...
            DB::table('prodplan_asm_delscheds')
    ...
    public function process4()
    {
        ...
        $tab_delsched_itemonly = DB::table('prodplan_asm_delscheds')
            ->select('item_code')
            ->distinct()
            ->get();
        ...
        foreach ($tab_items as $items) {
            ...
            $tab_delsched = DB::table('prodplan_asm_delscheds')
                ->where('item_code', $val_item_code)
                ->first();
            ...
            $sum_outstanding = DB::table('prodplan_asm_delscheds')
                ->where('item_code', $val_item_code)
                ->sum('outstanding');

            // Update data di tabel items
            DB::table('prodplan_asm_items')
