...
class PPSSecondController extends Controller
{
    ...
    public function process1()
    {
		...
		foreach($tab_delsched_final as $delsched_final){
			...
			if(empty($tab_delsched_final_wip->id)){
				...
				$tab_delsched_final_wip_link = DB::table('delsched_finalwip')->where('fglink_id',$val_id_fg)->where('departement','361')->get();
		...
		//Tarik ke delitem
		$tab_delraw_itemonly_paired = DB::table('prodplan_snd_delraws')->select('item_code','item_pair')->distinct()->get();
		...
		$tab_delraw_itemonly_nopair = DB::table('prodplan_snd_delraws')->select('item_code','item_pair')->distinct()->get();
    ...
    {
        $tab_delraw_date = DB::table('prodplan_snd_delraws')->select('delivery_date')->orderBy('delivery_date','asc')->distinct()->get();
		...
		foreach($tab_delraw_date as $delraw_date){
			...
			foreach($tab_delitem as $delitem){
				...
				$sum_del_qty = DB::table('prodplan_snd_delraws')->where('item_code',$val_item_code)->where('delivery_date',$val_delivery_date)->sum('quantity');							
				...
				if($sum_del_qty>0){			
					...
					if(empty($delitem->item_pair)){												
						...
						$sum_del_pair_qty = DB::table('prodplan_snd_delraws')->where('item_code',$val_pair_code)->where('delivery_date',$val_delivery_date)->sum('quantity');
    ...
    public function process3()
    {
		...
		foreach($tab_delsched as $delsched){
			...
			DB::table('prodplan_snd_delscheds')->where('id',$val_delsched_id)->update([	
    ...
    public function process4()
    {
		...
		$tab_delsched_itemonly = DB::table('prodplan_snd_delscheds')->select('item_code')->distinct()->get();
		...
		foreach($tab_items as $items){
			...
			$tab_delsched = DB::table('prodplan_snd_delscheds')->where('item_code',$val_item_code)->first();
			...
			$sum_outstanding = DB::table('prodplan_snd_delscheds')->where('item_code',$val_item_code)->sum('outstanding');
			
			//Update data di tabel items
			DB::table('prodplan_snd_items')->where('id',$val_items_id)->update([			
				'pair_code' => $val_pair_code,
				'bom_level' => $val_bom_level,
