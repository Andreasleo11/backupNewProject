...
class PPSKarawangController extends Controller
{
    ...
    public function process1()
    {
		...
		$tab_delsched_final = DB::table('delsched_final')->where('outstanding_stk','>',0)->whereBetween('delivery_date',[$val_past_date , $val_advanced_date_fg])->where('item_code', 'like', 'K-%')->get();
		...
		//Tarik ke delitem
		$tab_delraw_itemonly_paired = DB::table('prodplan_kri_delraws')->select('item_code','item_pair')->distinct()->get();
		...
		$tab_delraw_itemonly_nopair = DB::table('prodplan_kri_delraws')->select('item_code','item_pair')->distinct()->get();
		...
		foreach($tab_delraw_itemonly_nopair as $delraw_itemonly_nopair){
			...
			$tab_delitem = DB::table('prodplan_kri_delitems')->where('item_pair',$val_item_code_ii)->first();
    ...
    {
        $tab_delraw_date = DB::table('prodplan_kri_delraws')->select('delivery_date')->orderBy('delivery_date','asc')->distinct()->get();
		...
		foreach($tab_delraw_date as $delraw_date){
			...
			foreach($tab_delitem as $delitem){
				...
				$sum_del_qty = DB::table('prodplan_kri_delraws')->where('item_code',$val_item_code)->where('delivery_date',$val_delivery_date)->sum('quantity');	
				...
				if($sum_del_qty>0){			
					...
					if(empty($delitem->item_pair)){												
						...
						$sum_del_pair_qty = DB::table('prodplan_kri_delraws')->where('item_code',$val_pair_code)->where('delivery_date',$val_delivery_date)->sum('quantity');
    ...
    public function process3()
    {
		...
		foreach($tab_delsched as $delsched){
			...
			DB::table('prodplan_kri_delscheds')->where('id',$val_delsched_id)->update([	
    ...
    public function process4()
    {
		...
		$tab_delsched_itemonly = DB::table('prodplan_kri_delscheds')->select('item_code')->distinct()->get();
		...
		foreach($tab_items as $items){
			...
			$tab_delsched = DB::table('prodplan_kri_delscheds')->where('item_code',$val_item_code)->first();
			...
			$sum_outstanding = DB::table('prodplan_kri_delscheds')->where('item_code',$val_item_code)->sum('outstanding');
    ...
    public function process5()
	{
		...
		$data = InvLineList::where('departement', 390)->whereIn('line_code', ['0650A', '0900A', '0900B', '1400A', '1400B', '1400C'])
        ->get();
		// dd($data);
