...
class MasterTintaController extends Controller
{
    ...
    public function storetransaction(Request $request)
    {
        ...
        if($transactiontype ===  'out')
        {
            ...
            foreach ($itemNames as $itemName) {
                StockTransaction::where('unique_code', $itemName)->where('stock_id', $stockId)
                    ->update([
                        'dept_id' => $department,
                        'out_time' => now(),
                        'is_out' => true,
                    ...
                    StockRequest::where('stock_id', $stockId)->where('dept_id', $department)->whereYear('created_at', Carbon::now()->year)
                    ->whereMonth('created_at', Carbon::now()->month) 
                    ->latest()->decrement('quantity_available', 1);
            }
        }
        else{
            foreach ($itemNames as $itemName) {
    ...
    public function requeststore(Request $request)
    {
        ...
        if (!$masterStock) {
            return redirect()->back()->withErrors(['masterStock' => 'Invalid MasterStock ID.']);
        }

        $sumRequested = StockRequest::where('stock_id', $validatedData['masterStock'])
            ->whereMonth('month', $month)->sum('quantity_available');
    ...
    public function getAvailableQuantity($stock_id, $department_id)
    {
        ...
        return response()->json([
            'available_quantity' => $latestTransaction->quantity_available ? $latestTransaction->quantity_available : 0
        ]);
    }
