...
class NewBusController extends Controller
{
    ...
    {
        $tab_delivery_schedule = DB::table('delsched_final')->get();

        $tab_date_delsched = DB::table('uti_date_list')->where('id', 13)->first();
        $val_last_update = $tab_date_delsched->last_update;

        return view('program/new_business/delivery_schedule/bus_del_delsched', [
            'delivery_schedule' => $tab_delivery_schedule,
            'last_update' => $val_last_update,
        ]);
    }
    ...
    public function delivery_schedule_proses_1()
    {
        ...
        // 1. Menarik data dari tab SAP ke Delsched

        // 1.1 Data tabel sap_delsched ke delsched_final
        ...
        foreach ($tab_sap_delsched as $sap_delsched) {
            ...
            $tab_sap_customer = DB::table('sap_fg_customer')
                ->where('item_code', $val_item_code_i)
                ->first();
        ...
        // 1.2 Data tabel sap_delso ke delsched_solist
        ...
        foreach ($tab_sap_delso as $sap_delso) {
            ...
            $ins_solist = [
                'so_number' => $val_so_num_ii,
                'so_status' => $val_so_status_ii,
                'item_code' => $val_item_code_ii,
                'so_qty' => $val_so_quantity_ii,
                'delivered_qty' => $val_delivered_qty_ii,
                'row_status' => $val_row_status_ii,
            ];
        ...
        // 1.3 Data tabel sap_delactual ke delsched_delfilter dengan ketentuan SO yang closed di solist tidak ditarik
        ...
        foreach ($tab_sap_delactual as $sap_delactual) {
            ...
            $tab_delsched_solist_iii = DB::table('delsched_solist')
                ->where('so_number', $val_so_num_iii)
                ->first();
            ...
            if ($rcd_status == 'O') {
                $ins_delfilter = [
                    'item_code' => $val_item_code_iii,
                    'delivery_date' => $val_delivery_date_iii,
                    'quantity' => $val_quantity_iii,
        ...
        // 1.4 Membuat summary delivered per item di tabel delsched_delsum

        $tab_delsched_delfilter = DB::table('delsched_delfilter')
            ->select('item_code')
            ->distinct()
            ->get();
        ...
        foreach ($tab_delsched_delfilter as $delsched_delfilter) {
            ...
            $sum_delsched_delfilter_qty = DB::table('delsched_delfilter')
                ->where('item_code', $val_item_code_iv)
                ->sum('quantity');

            $ins_delsum = [
                'item_code' => $val_item_code_iv,
                'quantity' => $sum_delsched_delfilter_qty,
                'total_after' => $sum_delsched_delfilter_qty,
            ];
        ...
        return Redirect::action('NewBusController@delivery_schedule_proses_2');
    }
    ...
    public function delivery_schedule_proses_2()
    {
        ...
        foreach ($tab_delsched_final as $delsched_final) {
            ...
            if ($val_so_status == 'C') {
                $update_final = DB::table('delsched_final')
                    ->where('id', $val_final_id)
                    ->update([
        ...
        return Redirect::action('NewBusController@delivery_schedule_proses_3');
    }
    ...
    public function delivery_schedule_proses_3()
    {
            ...
            if ($val_total_after <= 0) {
                ...
                if ($val_total_after >= $val_delivery_qty) {
                    ...
                    if (empty($tab_delsched_delsum)) {
                        ...
                        $update_delsum = DB::table('delsched_delsum')
                            ->where('id', $val_delsum_id)
                            ->update([
        ...
        return Redirect::action('NewBusController@delivery_schedule_proses_4');
    }

    public function delivery_schedule_proses_4()
    {
        // 4. Filter delsched dengan pengambilan stok di gudang
        ...
        foreach ($tab_delsched_final as $delsched_final) {
            ...
            $tab_delsched_stock = DB::table('delsched_stock')
                ->where('item_code', '=', $val_item_code_final)
                ->first();
            ...
            if ($val_status == 'success') {
                ...
                $update_stock = DB::table('delsched_stock')
                    ->where('id', $val_stock_id)
                    ->update([
                        'total_after' => $cal_total_after_now,
                    ]);
    ...
    {
        $tab_delivery_schedule = DB::table('delsched_finalwip')->get();

        $tab_date_delsched = DB::table('uti_date_list')->where('id', 14)->first();
        $val_last_update = $tab_date_delsched->last_update;
    ...
    public function delivery_schedule_wip_proses_1()
    {
        ...
        return Redirect::action('NewBusController@delivery_schedule_wip_proses_2');
    }
    ...
    public function delivery_schedule_wip_proses_2()
    {
        ...
        return Redirect::action('NewBusController@delivery_schedule_wip');
    }
}
