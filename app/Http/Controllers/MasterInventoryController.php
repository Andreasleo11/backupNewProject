...
class MasterInventoryController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ...
            ->with('success', 'Master Inventory created successfully.');
    }
    ...
    public function editpage($id)
    {
        ...
        return view(
            'masterinventory.edit',
            compact('data', 'depts', 'hardwareTypes', 'softwareTypes'),
        );
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('hardwares')) {
            foreach ($request->hardwares as $hardware) {
                if (! empty($hardware['brand']) && ! empty($hardware['hardware_name'])) {
                    ...
                    $existingHardware = $masterInventory
                        ->hardwares()
        ...
        if ($request->has('softwares')) {
            foreach ($request->softwares as $software) {
                if (! empty($software['software_name']) && ! empty($software['license'])) {
                    ...
                    $existingSoftware = $masterInventory
                        ->softwares()
        ...
        return redirect()
            ->route('masterinventory.index')
            ->with('success', 'Master Inventory updated successfully.');
    }
    ...
    public function detail($id)
    {
        ...
        $inventoryHistories = HeaderMaintenanceInventoryReport::with(
            'detail',
            'detail.typecategory',
        )
            ->where('master_id', $id)
    ...
    public function getAvailableItems(Request $request)
    {
        ...
        if ($type === 'hardware') {
            ...
            $items = DetailSoftware::where('master_inventory_id', $masterId)->get([
                'id as id',
    ...
    public function CreateRepair(Request $request)
    {
        ...
        return redirect()
            ->route('masterinventory.detail', ['id' => $validated['master_id']])
    ...
    public function updateHistory($id)
    {
        ...
        // Example logic:
        $inventory = MasterInventory::where('id', $repairHistory->master_id)
            ->with([
                'hardwares.hardwareType',
                'softwares.softwareType', // Assuming you have a similar relationship for softwares
            ])
        ...
        if ($repairHistory->action === 'replacement') {
            ...
            if ($repairHistory->type === 'hardware') {
                ...
                $hardwareDetail = $inventory->hardwares->firstWhere(
                    'hardware_name',
                    $repairHistory->old_part,
                );
                ...
                $softwareDetail = $inventory->softwares->firstWhere(
                    'software_name',
                    $repairHistory->old_part,
                );
                if ($softwareDetail) {
    ...
    public function destroy($id)
    {
        ...
        return redirect()
            ->route('masterinventory.index')
            ->with('success', 'Inventory Master deleted successfully!');
    }
