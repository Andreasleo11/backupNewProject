...
class CapacityByForecastController extends Controller
{
    ...
    public function step1(Request $request)
    {
        ...
        $cnt_holiday_halfday = DB::table("uti_holiday_list")
            ->whereBetween("date", [$val_start_date, $val_end_date])
            ->where("half_day", 1)
            ->count();

        $cnt_holiday_fullday = DB::table("uti_holiday_list")
            ->whereBetween("date", [$val_start_date, $val_end_date])
            ->where("half_day", 0)
            ->count();
        ...
        foreach ($tab_forecast as $fore_forecast) {
            ...
            $ins_forecast = [
                "item_code" => $val_item_no,
                "quantity" => $val_quantity,
                "quantity_next" => $val_quantity_next,
                "total" => $cal_total,
            ];
        ...
        foreach ($tab_forecast_aft as $fore_forecast_aft) {
            ...
            foreach ($tab_bom_wip as $fore_bom_wip) {
                ...
                if ($val_level == 1) {
                    ...
                    $ins_forecast_div = [
                        "father_part" => $val_item_code,
                        "item_code" => $val_wip_code,
                        "quantity" => $cal_fct_qty,
                        "level" => 1,
                    ];
                    CapForecastDiv::insert($ins_forecast_div);
                } elseif ($val_level == 2) {
    ...
    public function step1_second(Request $request)
    {
        ...
        foreach ($tab_forecast_div as $fore_forecast_div) {
            ...
            $tab_fct_inventory_fg = DB::table("sap_fct_inventory_fgs")
        ...
        $tab_forecast_sorted = DB::table("cap_forecast_sorteds")
            ->select("item_code")
            ->distinct()
            ->get();
        foreach ($tab_forecast_sorted as $fore_forecast_sorted) {
            ...
            $tab_sap_lineproduction = DB::table("sap_fct_lineproductions")
                ->where("item_code", $val_item_code_srt)
            ...
            foreach ($tab_sap_lineproduction as $fore_sap_line_production) {
                ...
                $tab_forecast_sorted_item = DB::table("cap_forecast_sorteds")
                    ->where("item_code", $val_item_code_line)
                    ->first();

                if (empty($tab_forecast_sorted_item->item_code)) {
                } else {
                    $tab_forecast_div = DB::table("cap_forecast_divs")
                        ->where("item_code", $val_item_code_line)
                        ->orderBy("level", "desc")
                        ->first();
                    ...
                    $ins_line_distribution = [
                        "bom_level" => $val_level,
                        "line_code" => $val_line_code,
                        "item_code" => $val_item_code_line,
            ...
            $tab_fct_inventory_fg_pair = DB::table("sap_fct_inventory_fgs")
                ->where("pair", $val_item_code_srt)
                ->first();
            ...
            $sum_forecast_div_selected = DB::table("cap_forecast_divs")
                ->where("item_code", $val_item_code_srt)
                ->sum("quantity");

            $tab_line_distribution_new = DB::table("cap_line_distributions")
                ->where("item_code", $val_item_code_srt)
                ->where("priority", 1)
                ->first();
            ...
            $tab_line_list_one = DB::table("inv_line_lists")
                ->where("line_code", $val_line_code_first)
                ->first();
            ...
            $con_total_prior = DB::table("cap_line_distributions")
                ->where("item_code", $val_item_code_srt)
                ->count();

            $ins_items = [
                "item_code" => $val_item_code_srt,
                "line_category" => $val_category,
                "quantity" => $sum_forecast_div_selected,
                "departement" => $val_departement,
                "cavity" => $val_cavity,
                "man_power" => $val_man_power,
                "total_forecast_time" => $cal_total_forecast_time,
                "cycle_time_raw" => $val_cycle_time_raw,
                "cycle_time" => $cal_cycle_time,
                "pair" => $val_pair,
                "counter_forecast" => $sum_forecast_div_selected,
    ...
    public function step2logic(Request $request)
    {
        ...
        foreach ($tab_cap_line_distribution as $fore_line_distribution) {
            ...
            $update_pr_items = DB::table("cap_line_distributions")
                ->where("item_code", $val_item_code)
                ->update([
        ...
        $tab_cap_line_distribution_ii = DB::table("cap_line_distributions")
            ->orderBy("total_prior", "asc")
            ->orderBy("bom_level", "asc")
        ...
        foreach ($tab_cap_line_distribution_ii as $fore_line_distribution_ii) {
            ...
            $con_total_distribution_unorder = DB::table("cap_line_distributions")
                ->where("order", 0)
                ->count();
    ...
    public function step3logic(Request $request)
    {
        ...
        for ($i = 1; $i <= 9; $i++) {
            ...
            foreach ($tab_line_capacity as $line_capacity) {
                ...
                $tab_line_distribution = DB::table("cap_line_distributions")
                    ->where("line_code", $val_line_code)
                    ->where("priority", $i)
                    ->orderBy("order", "asc")
                    ->get();
                ...
                foreach ($tab_line_distribution as $line_distribution) {
                    ...
                    if ($val_counter_forecast > 0) {
                        $tab_line_capacity_ref = DB::table("cap_line_capacities")
                            ->where("line_code", $val_line_code)
                            ->first();
                        ...
                        if ($val_time_limit > 0) {
                            ...
                            if ($cal_max_prod_u >= $val_counter_forecast) {
                                ...
                                $cal_time_limit =
                                    ($cal_max_prod_u - $val_counter_forecast) * $val_cycle_time;
                            ...
                            $update_pr_item = DB::table("cap_items")
                                ->where("id", $val_item_id)
                                ->update([
                                    "counter_forecast" => $cal_counter_forecast_new,
                                ]);

                            $update_pr_line = DB::table("cap_line_capacities")
                                ->where("line_code", $val_line_code)
                                ->update([
                                    "time_limit" => $cal_time_limit,
                                ]);
    ...
    {
        $tab_line_list_category = DB::table("inv_line_lists")
            ->select("category")
            ->distinct()
            ->get();
        ...
        foreach ($tab_line_list_category as $line_category) {
            ...
            $con_line_category = DB::table("inv_line_lists")
            ...
            $sum_req_hour = DB::table("cap_items")
                ->where("line_category", $val_category)
                ->sum("total_forecast_time");

            $cal_capacity_req_percent = ($sum_req_hour / $cal_max_capacity) * 100;
            $cal_capacity_req_percent_ceil = ceil($cal_capacity_req_percent);

            $ins_line_summary = [
                "department" => $val_departement,
                "line_category" => $val_category,
                "line_quantity" => $con_line_category,
                "work_day" => $val_work_day,
                "ready_time" => $cal_ready_time,
                "efficiency" => $val_efficiency,
                "max_capacity" => $cal_max_capacity,
                "capacity_req_hour" => $sum_req_hour,
