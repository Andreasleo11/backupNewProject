...
class NewProdController extends Controller
{
    ...
    public function pps_wizard_step_0_dept()
    {
        ...
        return view('program/new_production/pps_wizard/pps_wizard_step_0_dept', [
            'date_inj' => $val_last_updated_inj,
            'date_snd' => $val_last_updated_snd,
            'date_asm' => $val_last_updated_asm,
        ]);
    }
    ...
    public function pps_wizard_inj_step_1_scenario()
    {
        ...
        return view(
            'program/new_production/pps_wizard/injection/pps_wizard_injection_step_1_scenario',
            [
                'start_date' => $val_start_date_inj,
                'end_date' => $val_end_date_inj,
                'lead_time_fg' => $val_lead_time_fg,
                'lead_time_wip' => $val_lead_time_wip,
                'day_saving' => $val_day_saving,
                'man_power' => $val_man_power,
                'mould_change' => $val_mould_change,
                'run_forecast' => $val_run_forecast,
            ],
        );
    }
    ...
    {
        $update_date = DB::table('uti_date_list')
            ->where('id', 15)
            ->update([
        ...
        $update_lead_time_fg = DB::table('prodplan_scenario')
            ->where('id', 1)
            ->update([
                'val_int_inj' => $request->lead_time_fg,
            ]);

        $update_lead_time_wip = DB::table('prodplan_scenario')
            ->where('id', 2)
            ->update([
                'val_int_inj' => $request->lead_time_wip,
            ]);

        $update_day_saving = DB::table('prodplan_scenario')
            ->where('id', 3)
            ->update([
                'val_int_inj' => $request->day_saving,
            ]);

        $update_man_power = DB::table('prodplan_scenario')
            ->where('id', 4)
            ->update([
                'val_int_inj' => $request->man_power,
            ]);

        $update_mould_change = DB::table('prodplan_scenario')
            ->where('id', 5)
            ->update([
        ...
        $update_run_forecast = DB::table('prodplan_scenario')
            ->where('id', 6)
            ->update([
    ...
    public function pps_wizard_inj_step_2_delsched_process()
    {
        ...
        // Tarik ke delitem
        $tab_delraw_itemonly_paired = DB::table('prodplan_inj_delraw')
        ...
        $tab_delraw_itemonly_nopair = DB::table('prodplan_inj_delraw')
            ->select('item_code', 'item_pair')
            ->distinct()
            ->get();
        ...
        foreach ($tab_delraw_itemonly_nopair as $delraw_itemonly_nopair) {
            ...
            $tab_delitem = DB::table('prodplan_inj_delitem')
    ...
    {
        $tab_delraw_date = DB::table('prodplan_inj_delraw')
        ...
        foreach ($tab_delraw_date as $delraw_date) {
            ...
            foreach ($tab_delitem as $delitem) {
                ...
                $sum_del_qty = DB::table('prodplan_inj_delraw')
                    ->where('item_code', $val_item_code)
                ...
                if ($sum_del_qty > 0) {
                    if (empty($delitem->item_pair)) {
                        ...
                        $sum_del_pair_qty = DB::table('prodplan_inj_delraw')
    ...
    public function pps_wizard_inj_step_2_delsched()
    {
        ...
        return view(
            'program/new_production/pps_wizard/injection/pps_wizard_injection_step_2_delsched',
            ['delsched' => $tab_delsched],
        );
    }
    ...
    public function pps_wizard_inj_step_3_items_process()
    {
        ...
        $tab_delsched_itemonly = DB::table('prodplan_inj_delsched')
            ->select('item_code')
            ->distinct()
            ->get();
        ...
        foreach ($tab_items as $items) {
            ...
            $tab_delsched = DB::table('prodplan_inj_delsched')
                ->where('item_code', $val_item_code)
                ->first();
            ...
            $sum_outstanding = DB::table('prodplan_inj_delsched')
                ->where('item_code', $val_item_code)
                ->sum('outstanding');
    ...
    public function pps_wizard_inj_step_3_items()
    {
        ...
        return view(
            'program/new_production/pps_wizard/injection/pps_wizard_injection_step_3_items',
            ['items' => $tab_items],
        );
    }
}
