...
class MasterDataPartsImportQueued implements
    ToCollection,
    WithHeadingRow,
    WithChunkReading,
    WithBatchInserts,
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                // Minimal validation
                $reason = null;
                if ($itemNo === '')           $reason = 'item_no empty';
                elseif ($desc === '')         $reason = 'description empty';
                elseif (!is_numeric($group))  $reason = 'item_group not numeric';
            ...
            if ($payload) {
                ...
                $existingNos   = MasterDataPart::whereIn('item_no', $itemNos)->pluck('item_no')->all();
