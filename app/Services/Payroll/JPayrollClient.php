...
final class JPayrollClient implements JPayrollClientContract
{
    ...
    {
        return new self(
            config('services.jpayroll.base_url'),
            config('services.jpayroll.auth'),
        );
    }
    ...
    {
        $json = $this->post('API_View_AnnualLeave.php', ['CompanyArea' => $companyArea, 'Year' => $year]);
    ...
    private function post(string $endpoint, array $payload): array
    {
        ...
        Log::warning('JPayroll API error', ['endpoint'=>$endpoint, 'status'=>$resp->status(), 'body'=>$resp->body()]);
