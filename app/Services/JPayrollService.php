...
class JPayrollService
{
    ...
    public function syncEmployeesLeaveAndAttendanceFromApi(string $companyArea = '10000', ?int $year = null, $fromDate = null, $toDate = null): array
    {
        DB::beginTransaction();
    ...
    {
        $context['total'] += count($this->fetchMasterEmployee($context['companyArea'])['data'] ?? []);
        $context['total'] += count($this->fetchAnnualLeave($context['companyArea'], $year)['data'] ?? []);
        $context['total'] += count($this->fetchAttendance(
            $context['companyArea'],
            null,
            $fromDate,
            $toDate
    ...
    {
        $percent = $context['total'] === 0 ? 0 : intval(($context['processed'] / $context['total']) * 100);
    ...
    private function syncMasterEmployees(string $companyArea, array &$context): void
    {
        ...
        foreach ($response['data'] as $item) {
            $data = [
                ...
                'start_date' => Carbon::createFromFormat('d/m/Y', $item['StartDate'])->format('Y-m-d'),
            ...
            if ($hasNull) {
                Log::warning("Skipping employee with NIK {$item['NIK']} due to missing required fields.");
                continue;
            }
    ...
    private function syncAttendanceData(string $companyArea, ?Carbon $from, ?Carbon $to, array &$context): void
    {
        ...
        while ($cursor < $weekStart && $cursor < $to) {
            ...
            $weekEnd = $cursor->copy()->endOfWeek(Carbon::SUNDAY)->min($weekStart->copy()->subDay(), $to);

            $this->syncAttendanceRange($companyArea, $weekStart, $weekEnd, $context);

            Log::info("Syncing attendance data from {$weekStart->toDateString()} to {$weekEnd->toDateString()}");
        ...
        if ($weekStart <= $to) {
            ...
            Log::info("Syncing attendance data from {$weekStart->toDateString()} to {$effectiveTo->toDateString()}");
        }
    }

    private function syncAttendanceRange(string $companyArea, Carbon $from, Carbon $to, array &$context): void
    {
        $response = $this->fetchAttendance(
            $companyArea,
            null,
            $from,
            $to
        ...
        if (!isset($response['data']) || !is_array($response['data'])) {
            Log::warning("Attendance sync skipped — invalid response from {$from->toDateString()} to {$to->toDateString()}.");
            return;
        }
        ...
        foreach ($response['data'] as $item) {
            ...
            if (!$existing) {
                EvaluationDataWeekly::create(array_merge([
                    'NIK' => $item['NIK'],
                ], $data));
            } elseif (collect($data)->diffAssoc($existing->only(array_keys($data)))->isNotEmpty()) {
