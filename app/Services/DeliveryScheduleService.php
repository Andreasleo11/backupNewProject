...
class DeliveryScheduleService
{
    ...
    {
        DB::table('sap_delsched')->orderBy('delivery_date')->orderBy('item_code')->chunk(500, function ($records) {
            $insertData = [];

            foreach ($records as $row) {
                $inventory = DB::table('sap_inventory_fg')->where('item_code', $row->item_code)->first();
                if (!$inventory) {
                    Log::error("No sap_inventory_fg found for item_code: {$row->item_code}");
                    continue;
                }

                $customer = DB::table('sap_fg_customers')->where('item_code', $row->item_code)->first();
    ...
    {
        DB::table('sap_delso')->orderBy('doc_num')->orderBy('item_no')->chunk(500, function ($records) {
            $insertData = [];
    ...
    private function insertDelschedDelsum()
    {
        ...
        foreach ($items as $item) {
            $sum = DB::table('delsched_delfilter')->where('item_code', $item->item_code)->sum('quantity');
    ...
    public function runStep2()
    {
        ...
        foreach ($records as $row) {
            ...
            if ($so && $so->so_status === 'C') {
                DB::table('delsched_final')->where('id', $row->id)->update([
    ...
    public function runStep3()
    {
        ...
        foreach ($records as $row) {
            ...
            if ($available <= 0) {
                ...
                DB::table('delsched_delsum')->where('id', $sum->id)->update([
    ...
    public function runStep4()
    {
        ...
        foreach ($records as $row) {
            ...
            $newStatus = $deliveryDate->lessThanOrEqualTo($now)
                ? ($after >= $outstanding ? 'warning' : 'danger')
                : 'light';

            DB::table('delsched_stock')->where('id', $stock->id)->update([
