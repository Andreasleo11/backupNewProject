    ...
    private function transformData(array $data)
    {
        ...
        $data = collect($data)
            ->map(function ($item) {
                ...
                $item["StandardTime"] = number_format(
                    floatval($item["StandardTime"] ?? 0),
                    5,
                    ".",
                    "",
                );
                $item["U_SAFETYSTOCK"] = number_format(
                    floatval($item["U_SAFETYSTOCK"] ?? 0),
                    0,
                    ".",
                    "",
                );
                $item["U_DAILYLIMIT"] = number_format(
                    floatval($item["U_DAILYLIMIT"] ?? 0),
                    0,
                    ".",
                    "",
                );
