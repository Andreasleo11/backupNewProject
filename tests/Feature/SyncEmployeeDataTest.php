...
test('it syncs employees, leave, and attendance correctly', function () {
    ...
    expect($employee->Nama)->toBe('Test Employee')
        ->and($employee->Dept)->toBe('310')
        ->and($employee->employee_status)->toBe('TETAP')
        ->and($employee->jatah_cuti_tahun)->toBe(10);

    expect(EvaluationDataWeekly::count())->toBe(1);
    $evaluation = EvaluationDataWeekly::first();
    expect($evaluation->Alpha)->toBe(1)
        ->and($evaluation->Izin)->toBe(1);
});
